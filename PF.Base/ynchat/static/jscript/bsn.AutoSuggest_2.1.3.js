...
_b.AutoSuggest.prototype.doAjaxRequest = function (input)
{
	...
	var onSuccessFunc = function (req) { ynchat.hideLoadingSearchingFriend(); pointer.setSuggestions(req, input) };
	var onErrorFunc = function (status) { alert("AJAX error: "+status); ynchat.endSearchingFriend();};
...
_b.AutoSuggest.prototype.createList = function(arr)
{
	...
	// lytk: change html item 
	for (var i=0;i<arr.length;i++)
	{
        // create searching friend list - START
        var avatar = _b.DOM.cE(  "div", {className:"avatar"}, '<img src="' + arr[i].avatar + '" />', true  );
        var name = _b.DOM.cE(  "div", {className:"name"}, arr[i].full_name, true  );        
        var agent = '';
        if(arr[i].status == 'available'){
            if(arr[i].agent == 'web'){
                agent = ynchat.getLang('web'); 
            } else {
                agent = ynchat.getLang('mobile'); 
            }
        }
        agent += '<div class="status ' + arr[i].status + '"></div></div>';
        var status = _b.DOM.cE(  "div", {className: "user-status"}, agent, true  );
        ...
        ul.appendChild( li );
        // create searching friend list - END
	}
	...
	if(typeof(jsynchat) === 'undefined') {
		jsynchat = jQuery;
	}
	jsynchat( this.oP.eleParent ).append( div );
...
_b.AutoSuggest.prototype.resetTimeout = function()
{
	clearTimeout(this.toID);
	var pointer = this;
	this.toID = setTimeout(function () { pointer.clearSuggestions("fade") }, 10000);
};
