...
class Phpfox_Template
{
    ...
    public function __construct()
    {
        ...
        $this->_path_file = Phpfox::getParam('core.path_file');
        $this->_url_static_script = Phpfox::getParam('core.url_static_script');
    ...
    {
        $basePath = dirname($this->_path_file);
        switch ($mValue) {
            case 'style_script':
            ...
            default:
                if (preg_match('/app_/i', $mValue)) {
                    ...
                    if ($mKey == 'autoload.css') {
                        ...
                        $sFileName = trim(str_replace(dirname($this->_path_file), '', $this->getStyle('css', $mKey, $aParts[1], true)), '/');
    ...
    public function getHeader($bReturnArray = false)
    {
        ...
        if (!defined('PHPFOX_INSTALLER')) {
            ...
            if(!Phpfox::isAppActive('Core_Messages') && !Phpfox::isAppActive('PHPfox_IM')) {
    ...
    public function addScript($key, $value, $bReturnString = true)
    {
        switch ($value) {
            ...
            case 'static_script':
                if ($bReturnString) {
                    return '<script src="' . $this->_url_static_script . $key . '"></script>';
                }
                $this->_sAddScripts .= '<script src="' . $this->_url_static_script . $key . '"></script>';
                break;
            default:
                if (preg_match('/app_/i', $value)) {
                    ...
                    if ('.js' == substr($key, -3)) {
                        ...
                        $Theme = $this->theme()->get();
                        $sFileName = trim(str_replace(dirname($this->_path_file), '', $this->getStyle('css', $key, $aParts[1], true)), '/');
                        $sFileName = $Theme->getCssFileName($sFileName, 'app');
                    ...
                    if (isset($aParts[1]) && Phpfox::isModule($aParts[1])) {
                        if (substr($key, -3) == '.js') {
                            if ($bReturnString) {
                                return '<script src="' . $this->_path_file . 'module/' . $aParts[1] . '/static/jscript/' . $key . '"></script>';
                            }
                            $this->_sAddScripts .= '<script src="' . $this->_path_file . 'module/' . $aParts[1] . '/static/jscript/' . $key . '"></script>';
    ...
    public function getFooter($bReturnArray = false)
    {
        ...
        $this->_sFooter .= '<script src="' . Phpfox::getLib('cdn')->getUrl($this->_url_static_script) . 'bootstrap.js"></script>';
    ...
    public function getBuiltFile($sTemplate, $feed_id = null)
    {
        ...
        if (!isset($skip_layout) && isset($path)) {
            if (defined('PHPFOX_NO_TEMPLATE_CACHE') || (include $path) == FALSE) {
    ...
    public function getBuiltTemplate($sLayout, $sCacheName)
    {
        ...
        if (defined('PHPFOX_NO_TEMPLATE_CACHE') || (include $this->_getCachedName($sCacheName)) == FALSE) {
            ...
            include $this->_getCachedName($sCacheName);
        }
            ...
            if ($this->_sSetLayout) {
                ...
                if (!defined('PHPFOX_NO_TEMPLATE_CACHE')) {
                    try {
                        ...
                        $this->_sSetLayout = '';
                        $bGetTemplateCacheSucceeded = true;
                    } catch (Exception $ex) {
                    }
                }
                if (!$bGetTemplateCacheSucceeded) {
                    ...
                    $oTplCache = Phpfox::getLib('template.cache');
                    $oTplCache->compile($this->_getCachedName($sFile), $sLayoutContent, true,
                        (isset($aSubTemplate['html_data']) ? true : false));
    ...
    private function _getCachedName($sName)
    {
        ...
        return (defined('PHPFOX_TMP_DIR') ? PHPFOX_TMP_DIR : PHPFOX_DIR_CACHE) . ((defined('PHPFOX_TMP_DIR') || PHPFOX_SAFE_MODE) ? 'template_' : 'template' . PHPFOX_DS . flavor()->active->id . PHPFOX_DS) . str_replace(array(PHPFOX_DIR_SITE, PHPFOX_DIR_SITE_APPS, PHPFOX_DIR_THEME, PHPFOX_DIR_MODULE, '/', '\\'), array('', '', '', '', '_', '_'), $sName) . (Phpfox::isAdminPanel() ? '_admincp' : '') . (PHPFOX_IS_AJAX ? '_ajax' : '') . '.php';
    }
