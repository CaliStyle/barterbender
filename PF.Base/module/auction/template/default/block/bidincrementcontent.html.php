<div class="panel panel-default">
                ...
                {if isset($aForms.data_increasement) && count($aForms.data_increasement)}
                    {foreach from=$aForms.data_increasement item=aItem key=iKey}
                        <div class="bidincrement_items">
                            <div class="bidincrement_row">
                                <div class="bidincrement_from"><input class="field_from form-control" type="text" name="val[from][]" value="{$aItem.from}" readonly /></div>
                                <div class="bidincrement_to"><input class="field_to form-control" type="text" name="val[to][]" value="{$aItem.to}" {if ($iKey + 1) != count($aForms.data_increasement)} readonly {/if} /></div>
                                <div class="bidincrement_increment"><input class="field_increment form-control" type="text" name="val[increment][]" value="{$aItem.increment}" /></div>
    ...
    <div class="panel-footer">
        <input class="bidincrement_submit btn btn-primary" type="submit" value="{phrase var='submit'}" onclick="submitFormBidIncrement();" />
