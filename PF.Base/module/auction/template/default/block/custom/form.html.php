        ...
        <div class="form-group">
            ...
            <div class="table_right">
                ...
                {elseif $aField.var_type=='checkbox'}
                    {foreach from=$aField.option key=opId item=opPhrase}
                    <div class="checkbox">
                        <label for="js_jp_cf_{$aField.field_id}"><input {if $aField.is_required==1}data-isrequired="1"{else}data-isrequired="0"{/if} data-type="checkbox" id="js_jp_cf_{$aField.field_id}" type="checkbox" name="val[custom][{$aField.field_id}][]" value="{$opId}" 
                        {if isset($aField.value)}
                ...
                {elseif $aField.var_type=='radio'}
                    {foreach from=$aField.option key=opId item=opPhrase}
                            ...
                            {foreach from=$aField.value key=checked item=value_checked }
                     	<div class="radio">
                        <label for="js_jp_cf_{$aField.field_id}"><input {if $aField.is_required==1}data-isrequired="1"{else}data-isrequired="0"{/if} data-type="radio" id="js_jp_cf_{$aField.field_id}" type="radio" name="val[custom][{$aField.field_id}][]" value="{$opId}" {if $opId == $checked} checked {/if} /> {phrase var=$opPhrase}</label></div>
                            {/foreach}
                            {else}
                        <div class="radio">
                    	<label for="js_jp_cf_{$aField.field_id}"><input {if $aField.is_required==1}data-isrequired="1"{else}data-isrequired="0"{/if} data-type="radio" id="js_jp_cf_{$aField.field_id}" type="radio" name="val[custom][{$aField.field_id}][]" value="{$opId}" /> {phrase var=$opPhrase}</label></div>
                            {/if}
                    {/foreach}
                {/if}
            </div>
