...
class Comment_Service_Process extends Phpfox_Service
{
    ...
    public function add($aVals, $iUserId = null, $sUserName = null)
    {
        ...
        if (Phpfox::isModule('feed') && ((isset($aVals['is_via_feed']) && !empty($aVals['is_via_feed'])) || !empty($aVals['is_api']))) {
            ...
            if (!empty($aFeed) && !Phpfox::getUserParam('privacy.can_comment_on_all_items')) {
                if (isset($aFeed['privacy_comment']) && !empty($aFeed['privacy']) && !empty($aFeed['user_id']) && $aFeed['user_id'] != $iUserId) {
                    if ($aFeed['privacy_comment'] == 1 && Phpfox::isModule('friend') && Phpfox::getService('friend')->isFriend($iUserId, $aFeed['user_id']) != true) {
                        ...
                        if ($aFeed['privacy_comment'] == 2 && Phpfox::isModule('friend') && Phpfox::getService('friend')->isFriendOfFriend($iUserId) != true) {
                            ...
                            if ($aFeed['privacy_comment'] == 3 && ($aFeed['user_id'] != $iUserId) && !(Phpfox::isModule('feed') && Phpfox::getService('feed')->checkTaggedUser($aVals['item_id'], $aVals['type'], $iUserId))) {
                                return Phpfox_Error::display(_p('unable_to_post_a_comment_on_this_item_due_to_privacy_settings'));
                            } else {
                ...
                if (isset($aFeed['privacy']) && !empty($aFeed['privacy']) && !empty($aFeed['user_id']) && $aFeed['user_id'] != $iUserId) {
                    if ($aFeed['privacy'] == 1 && Phpfox::isModule('friend') && Phpfox::getService('friend')->isFriend($iUserId,
                        ...
                        if ($aFeed['privacy'] == 2 && Phpfox::isModule('friend') && Phpfox::getService('friend')->isFriendOfFriend($iUserId) != true) {
                            ...
                            if ($aFeed['privacy'] == 3 && ($aFeed['user_id'] != Phpfox::getUserId()) && !(Phpfox::isModule('feed') && Phpfox::getService('feed')->checkTaggedUser($aVals['item_id'], $aVals['type'], $iUserId))) {
                                return Phpfox_Error::display(_p('unable_to_post_a_comment_on_this_item_due_to_privacy_settings'));
                            } else {
        ...
        if ($aVals['type'] != 'app' && Phpfox::hasCallback($aVals['type'], 'getCommentItem')) {
        ...
        if (Phpfox::getParam('feed.top_stories_update') != 'like') {
            ...
            (Phpfox::isModule('feed') ? Phpfox::getService('feed.process')->clearCache($aVals['type'], $aVals['item_id']) : null);
            $this->database()->update(Phpfox::getT($sFeedPrefix . 'feed'), ['time_update' => PHPFOX_TIME], 'type_id = \'' . $this->database()->escape($sNewTypeId) . '\' AND item_id = ' . (int)$aVals['item_id']);

            if (!empty($sFeedPrefix)) {
                $this->database()->update(Phpfox::getT('feed'), ['time_update' => PHPFOX_TIME], 'type_id = \'' . $this->database()->escape($sNewTypeId) . '\' AND item_id = ' . (int)$aVals['item_id']);
            }
        }
    ...
    public function updateText($iId, $sText)
    {
        if (Phpfox::getService('comment')->hasAccess($iId, 'edit_own_comment', 'edit_user_comment')) {
            ...
            $this->database()->update(Phpfox::getT('comment'),
                ['update_time' => PHPFOX_TIME, "update_user" => Phpfox::getUserBy("full_name")],
                "comment_id = " . (int)$iId);
    ...
    public function deleteInline($iId, $iTypeId, $bForce = false)
    {
        ...
        if (Phpfox::isAppActive('Core_Pages') && Phpfox_Request::instance()->get('type_id') == 'pages') {
        ...
        if ((($iUserId = Phpfox::getService('comment')->hasAccess($iId, 'delete_own_comment',
            ...
            if (empty($aCommentRow['parent_id']) && Phpfox::hasCallback($iTypeId, 'deleteComment')) {
    ...
    public function delete($iId)
    {
        ...
        $comment = $this->database()->select('type_id, parent_id')
            ->from(Phpfox::getT('comment'))
            ->where('comment_id = ' . (int)$iId)
            ->execute('getSlaveRow');

        $commentType = $comment['type_id'] . '_comment'; // type of comment on feed table
        (Phpfox::isModule('feed') ? Phpfox::getService('feed.process')->delete($commentType, (int)$iId) : null);
        ...
        if ($comment['parent_id']) {
            $iTotalChild = $this->database()->select('COUNT(*)')->from(Phpfox::getT('comment'))->where('parent_id =' . (int)$comment['parent_id'])->execute('getField');
            $this->database()->update(Phpfox::getT('comment'), ['child_total' => $iTotalChild], 'comment_id =' . (int)$comment['parent_id']);
        }
        // get all replies
        $replies = $this->database()->select('comment_id')
            ->from(Phpfox::getT('comment'))
            ->where('parent_id = ' . (int)$iId)
            ->execute('getSlaveRows');
        foreach ($replies as $reply) {
            (Phpfox::isModule('feed') ? Phpfox::getService('feed.process')->delete($commentType, $reply['comment_id']) : null);
            $this->database()->delete(Phpfox::getT('comment'), "comment_id = " . $reply['comment_id']);
            $this->database()->delete(Phpfox::getT('comment_text'), "comment_id = " . $reply['comment_id']);
            $this->database()->delete(Phpfox::getT('comment_rating'), 'comment_id = ' . $reply['comment_id']);
        }
    ...
    public function moderate($iId, $sAction, $bIsAdmin = false)
    {
        ...
        $aItem = Phpfox::hasCallback($aComment['type_id'], 'getCommentItem') ? Phpfox::callback($aComment['type_id'] . '.getCommentItem', $aComment['item_id']) : [];
        ...
        if ($sAction == 'approve') {
            ...
            if ($bIsAdmin) {
                ...
                // force for add feed when enable "add feed for new comment"
                define('FEED_FORCE_USER_ID', $aComment['user_id']);

                // force for check when add notification
                define('COMMENT_FORCE_USER_ID', $aComment['user_id']);
                define('COMMENT_FORCE_USER_FULL_NAME', $aComment['full_name']);
            ...
            // notify tagged users
            $mentions = Phpfox::getService('user.process')->getIdFromMentions($aComment['text'], true);
            if (Phpfox::isModule('notification')) {
                $sName = 'comment_';
                if ($aVals['type'] == 'photo_album' || (strpos($aVals['type'], 'music') !== false) || ($aVals['type'] == 'user_status')) {
                ...
                foreach ($mentions as $userId) {
                    Phpfox::getService('notification.process')->add($sName, $aComment['comment_id'], $userId, $aComment['user_id'], true);
                }
            }
        } else {
        ...
        if (isset($aVals['comment_view_id']) && $aVals['comment_view_id'] == 2) {
            if ($bIsAdmin) {
                ...
                if (Phpfox::hasCallback($aComment['type_id'], 'processCommentModeration')) {
