...
{	
    ...
    public function get($sSelect, $aConds, $sSort = 'cmt.time_stamp DESC', $iRange = '', $sLimit = '', $iCnt = null, $bIncludeOwnerDetails = false)
	{
		...
		foreach ($aRows as $iKey => $aRow)
		{
			...
			{
                if (Phpfox::getUserBy('profile_page_id') > 0 && Phpfox::isAppActive('Core_Pages')) {
	...
	public function getPendingComments()
	{
        ...
        if (false === ($aComments = $this->cache()->get($sCacheId))) {
            ...
            foreach ($aComments as $iKey => $aComment) {
                ...
                $aComments[$iKey]['item_message'] = _p('user_link_left_a_comment_on_your_item', [
                    'user'      => $aComment,
                    'item_name' => Phpfox::hasCallback($aComment['type_id'], 'getCommentItemName') ? Phpfox::callback($aComment['type_id'] . '.getCommentItemName') : '',
