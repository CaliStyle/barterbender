...
class Notification_Service_Notification extends Phpfox_Service
{
    ...
    public function getForBrowse($iPage = 0, $iPageTotal = 10)
    {
        ...
        if (!empty($sIds)) {
            $this->database()->update(Phpfox::getT('notification'), ['is_seen' => '1'], 'notification_id IN(' . $sIds . ')');
        }

        return [$iCnt, $aNotifications];
    }
    ...
    public function get()
    {
        ...
        $this->database()->update($this->_sTable, ['is_seen' => '1'], array_merge([
    ...
    {
        $fullName= Phpfox::getLib('parse.output')->shorten(Phpfox::getLib('parse.output')->clean($aNotification['full_name']), 0);
        if (isset($aNotification['extra_users']) && is_array($aNotification['extra_users']) && count($aNotification['extra_users'])) {
            ...
            foreach ($aNotification['extra_users'] as $aUser) {
                ...
                $sStr .= '<span class="drop_data_user">' . ((isset($aUser['user_id']) && $aUser['user_id'] == Phpfox::getUserId()) ? _p('you') : Phpfox::getLib('parse.output')->shorten(Phpfox::getLib('parse.output')->clean($aUser['full_name']), 0)) . '</span>, ';
            }
            ...
            if ($bRemove) {
                ...
                if (isset($sExtraTotalUsers)) {
                    if ($sExtraTotalUsers === 1) {
                        $sStr = $sStr . ' <span class="drop_data_user">' . ((isset($aNotification['user_id']) && $aNotification['user_id'] == Phpfox::getUserId()) ? 'You' : $fullName) . '</span> ' . _p('and') . ' ' . $sExtraTotalUsers . ' ' . _p('other') . ' ';
                    } else {
                        $sStr = $sStr . ' <span class="drop_data_user">' . ((isset($aNotification['user_id']) && $aNotification['user_id'] == Phpfox::getUserId()) ? 'You' : $fullName) . '</span> ' . _p('and') . ' ' . $sExtraTotalUsers . ' ' . _p('others') . ' ';
                    }
                } else {
                    $sStr = (empty($sStr) ? '' : rtrim($sStr, ', ') . ' ' . _p('and')) . ' <span class="drop_data_user">' . ((isset($aNotification['user_id']) && $aNotification['user_id'] == Phpfox::getUserId()) ? 'You' : $fullName) . '</span>';
                }
            }
        ...
        $sStr = '<span class="drop_data_user">' . ((isset($aNotification['user_id']) && $aNotification['user_id'] == Phpfox::getUserId()) ? _p('you') : $fullName) . '</span>';
