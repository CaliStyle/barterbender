...
class Contactimporter_Component_Controller_Queuemes extends Phpfox_Component
{
	public function process()
	{
		...
		if (!Phpfox::isUser())
		{
			...
			$this -> url() -> send('login', null, _p('need_to_be_logged_in_for_showing_your_queue_mes'));
		}
		...
		if ($iQueue = $this -> request() -> getInt('del'))
		{
			...
			$url  = Phpfox ::getLib( 'url')-> makeUrl('contactimporter.queuemes');
			if($iPage)	
				$url  = Phpfox ::getLib( 'url')-> makeUrl('contactimporter.queuemes').'page_'.$iPage;
			$this -> url() -> send($url, null, _p('queue_delete_succesful'));
		...
		elseif ($sQueue = $this -> request() -> get('selectedVals'))
		{
			...
			$this -> url() -> send('contactimporter.queuemes', null, _p('queue_delete_succesful'));
		}
		
		
		$iPageSize = (int)Phpfox::getParam('contactimporter.how_many_pendings_to_show_per_page');
	
		list($iCnt, $aQueues) = Phpfox::getService('contactimporter') -> getQueue(array('iUser' => Phpfox::getUserId(), 'iPage' => $iPage, 'iPageSize' => $iPageSize, 'sType' => 'social'));
		...
		$this -> template() -> setTitle(_p('queue_messages')) -> setBreadcrumb(_p('queue_messages')) -> assign(array(
			'aQueues' => $aQueues,
			'iPage' => $iPage,
