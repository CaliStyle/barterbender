...
class gmail extends openinviter_base
	...
	public function login($user,$pass)
        ...
        $res=$this->post("https://www.google.com/accounts/ClientLogin",$post_elements,false);
        
        if ($this->checkResponse("login_post",$res))
			$this->updateDebugBuffer('login_post',"https://www.google.com/accounts/ClientLogin",'POST',true,$post_elements);
		else
        {			
            $post_elements=array('accountType'=>'HOSTED_OR_GOOGLE','Email'=>$user,'Passwd'=>$pass,'service'=>'cl','source'=>'OpenInviter-OpenInviter-'.$this->base_version);
            $res=$this->post("https://www.google.com/accounts/ClientLogin",$post_elements,false);
            $post_elements=array('accountType'=>'HOSTED_OR_GOOGLE','Email'=>$user,'Passwd'=>$pass,'service'=>'cp','source'=>'OpenInviter-OpenInviter-'.$this->base_version);
            $res=$this->post("https://www.google.com/accounts/ClientLogin",$post_elements,false);
            if ($this->checkResponse("login_post",$res))
	...
	public function getMyContacts()
		...
		$res=$this->get("https://www.google.com/m8/feeds/contacts/default/full?max-results=10000",true,false,true,false,array("Authorization"=>"GoogleLogin auth={$auth}"));
		if ($this->checkResponse("contact_xml",$res))
			$this->updateDebugBuffer('contact_xml','https://www.google.com/m8/feeds/contacts/default/full?max-results=10000','GET');
		else
			{
			$this->updateDebugBuffer('contact_xml','https://www.google.com/m8/feeds/contacts/default/full?max-results=10000','GET',false);
			$this->debugRequest();
			$this->stopPlugin();
			return false;
			}
