...
class youtube extends openinviter_base
{
    ...
    public $debug_array = array(
        ...
        'url_redirect' => 'XSRF_TOKEN', //'XSRF_TOKEN',//'gXSRF_token',
    ...
    public function login($user, $pass)
    {
        ...
        //secho $res;die('222');
        $re = $this->getHiddenElements($res);
        //print_r($re);//die('22');
        $form_action = "https://www.google.com/accounts/ServiceLoginAuth?service=youtube";
        $post_elements = array('ltmpl' => 'sso',
            'continue' => 'https://www.youtube.com/signin?action_handle_signin=true&nomobiletemp=1&hl=en_US&next=%2Findex',
            'next' => '/',
            'service' => 'youtube',
            'uilel' => 3,
            'ltmpl' => 'sso',
        ...
        // echo "<pre>".print_r($post_elements,true)."</pre>"; 
        //$log->lwrite(print_r($post_elements,true));
        ...
        $this->login_ok = 'https://www.youtube.com/address_book';
        return true;
    }
    ...
    public function getMyContacts()
    {
        ...
        $log->lwrite('get url_addressbook');
        $log->lwrite($res);
        ...
        $form_action = 'https://www.youtube.com/address_book?action_ajax=1';
        ...
        if (preg_match_all("#fid\"\: \"(.+)\"\}#U", $res, $matchesArray))
        {
            if (!empty($matchesArray[1]))
                foreach ($matchesArray[1] as $dummy => $id)
                {
                    $emailsArray = array();
                    $namesArray = array();
                    $res = $this->get("https://www.youtube.com/address_book?action_display_contact_details=1&fid={$id}");
                    if (preg_match_all("#mailto\:(.+)\"#U", $res, $emailsArray))
                    {
                        if (preg_match_all("#\"\/user\/(.+)\"#U", $res, $namesArray))
                            $contacts[$emailsArray[1][0]] = array('first_name' => $namesArray[1][0], 'email_1' => $emailsArray[1][0]);
                        else
                            $contacts[$emailsArray[1][0]] = array('first_name' => $emailsArray[1][0], 'email_1' => $emailsArray[1][0]);
                    }
                }
        }
        foreach ($contacts as $email => $dummy)
    ...
    public function logout()
    {
        if (!$this->checkSession())
            return false;
        $res = $this->get('https://www.youtube.com/index', true);
        $post_elements = array('action_logout' => 1,
            'session_token' => $this->getElementString($res, "YT_php_support('", "'"),
        );
        $res = $this->post('https://www.youtube.com/index', $post_elements, true);
        $this->debugRequest();
        $this->resetDebugger();
        $this->stopPlugin();
        return true;
