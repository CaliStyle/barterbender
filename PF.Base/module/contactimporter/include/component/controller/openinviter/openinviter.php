...
class openinviter
{
    ...
    public function __construct()
    {
        ...
        $cookiePath = PHPFOX_DIR_CACHE;
        $this->settings['cookie_path'] = $cookiePath;
        // echo $cookiePath;
    }
    ...
    private function checkConfig()
    {
        ...
        foreach ($this->configStructure as $option => $details)
        {
            if (!isset($this->settings[$option]))
                $to_add[$option] = $details['default'];
            if ($ok)
                if ($details['required'] AND empty($this->settings[$option]))
                {
                    $this->internalError = "`{$option}` is not defined in config.php";
                    $ok = false;
                }
        }
        if (!empty($to_add))
        {
            ...
            foreach ($to_add as $option => $value)
    ...
    private function statsCheck()
    {
        ...
        if (!file_exists($db_file))
        {
            ...
            {
                $this->internalError = "Unable to write stats. " . $this->basePath . " is not writable";
                return false;
            }
            if (!$this->statsOpenDB())
            {
                $this->internalError = "Unable to create the stats database.";
                return false;
            }
        ...
        elseif (!is_readable($db_file))
        {
            $this->internalError = "Unable to open stats database. {$db_file} is not readable.";
            return false;
        }
        elseif (!is_writable($db_file))
        {
            $this->internalError = "Unable to write stats. {$db_file} is not writable";
            return false;
        }
        elseif (!$this->statsOpenDB())
        {
            $this->internalError = "Unable to open the stats database.";
            return false;
        }
        return true;
    }
    ...
    private function statsOpenDB()
    {
        ...
        if (function_exists('sqlite_open'))
            if ($this->statsDB = sqlite_open($this->basePath . '/openinviter_stats.sqlite', 0666))
                return true;
        return false;
    }
    ...
    public function startPlugin($plugin_name, $getPlugins = false)
    {
        ...
        elseif (file_exists($this->basePath . "/plugins/{$plugin_name}.plg.php"))
        {
            ...
            $this->plugin->base_path = $this->basePath;
            $this->currentPlugin = $this->availablePlugins[$plugin_name];
            if (file_exists($this->basePath_ . "/conf/{$plugin_name}.conf"))
            {

                include($this->basePath_ . "/conf/{$plugin_name}.conf");
                if (empty($enable))
                    $this->internalError = "Invalid service provider";
                if (!empty($messageDelay))
                    $this->plugin->messageDelay = 2; else
                    $this->plugin->messageDelay = 2;
                if (!empty($maxMessages))
                    $this->plugin->maxMessages = 10000; else
                    $this->plugin->maxMessages = 10000;
            }
            else
            {
                $this->plugin->messageDelay = 2;
    ...
    public function writePlConf($name_file, $type)
    {
        ...
        if (!file_exists($this->basePath_ . "/conf"))
            mkdir($this->basePath_ . "/conf", 0755, true);
        if ($type == 'social')
            file_put_contents($this->basePath_ . "/conf/{$name_file}.conf", '<?php $enable=true;$autoUpdate=true;$messageDelay=1;$maxMessages=10;?>');
        elseif ($type == 'email')
            file_put_contents($this->basePath_ . "/conf/{$name_file}.conf", '<?php $enable=true;$autoUpdate=true; ?>');
        elseif ($type == 'hosted')
            file_put_contents($this->basePath_ . "/conf/{$name_file}.conf", '<?php $enable=false;$autoUpdate=true; ?>');
    }
            ...
            if ($update == TRUE OR $this->settings['hosted'] == FALSE)
            {
                ...
                if (count($array_file) > 0)
                {
                    ...
                    foreach ($modified_files as $plugin_key => $file)
                        if (file_exists($this->basePath_ . "/conf/{$plugin_key}.conf"))
                        {
                            include_once($this->basePath_ . "/conf/{$plugin_key}.conf");
    ...
    private function checkLoginCredentials($user)
    {
        ...
        if ($this->currentPlugin['requirement'])
        {
            ...
            {
                $this->internalError = _p('please_enter_the_full_email_not_just_the_username');
                return false;
            }
            elseif ($this->currentPlugin['requirement'] == 'user' AND $is_email)
            {
                $this->internalError = _p('please_enter_just_the_username_not_the_full_email');
                return false;
            }
        }
        if ($this->currentPlugin['allowed_domains'] AND $is_email)
        {
            ...
            if (!$temp)
            {
                $this->internalError = _p('strong_user_domain_strong_is_not_a_valid_domain_for_this_provider', array('user_domain' => $user_domain));
                return false;
            }
        }
        return true;
