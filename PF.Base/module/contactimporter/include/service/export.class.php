...
class Contactimporter_Service_Export extends Phpfox_Service
{
	public function exportCSV($userID)
	{
		...
		$aRows = $this -> database()
				...
				-> where('fr.friend_user_id = ' . $userID. ' AND u.profile_page_id = 0 AND fr.is_page = 0')
		...
		if (count($aRows) > 0)
		{
			...
			foreach ($aInserts as $aRow)
			{
				...
				{
                    $name_arr = array(0,1,2);
                    if(count($aRow['full_name']) < 3) {
                        if($key2 == 2)
                            $key2 = 1;
                        else if($key2 == 1)
                            $name_arr = array();
                    }
					if (in_array($key2, $name_arr) && isset($aRow['full_name'][$key2]))
					{
		...
		{
			$error = _p('your_friend_contact_is_empty') . '.';
		}
		if ($error != '')
		{
			return $error;
