...
<p class="description" style="margin-bottom:5px;">{_p var='the_following_people_are_not_your_friends'}.</p>
...
<div class="wrapper-list">
	<div id="div_list_view" style="width:100%;padding-left:0px;">
		...
		<?php $is_contact=false;?>
			...
			{/if}
			{/foreach}
			<?php if($is_contact==true) echo "<tr class='thTableOddRow'><td align='center' style='padding:20px;' colspan='3'>"._p('there_is_no_contact_in_your_account')."</td></tr>";?>
		</table>
	</div>
</div>
<div class="clear"></div>
<div style="float:left;margin:10px;0px;position:absolute;z-index:1000;">
    {if {$sLinkPrev}}<a href="{$sLinkPrev}">{_p var='view_number_contacts_previous' number=5000}</a> | {/if}
    {if {$sLinkNext}}<a href="{$sLinkNext}">{_p var='view_number_contacts_next' number=5000}</a>{/if}
</div>
{pager}
<div class="clear"></div>
...
<form method="post" action="{url link='contactimporter.addcontact'}" class="global_form" name='openinviter' enctype="application/x-www-form-urlencoded" onsubmit="return check_select();">
	<div class="form-wrapper" id="message-wrapper" style="clear:both">
		...
		<div class="form-label" id="message-label" style="width: 120px;text-align: left;">
			<label class="optional" for="message">{_p var='custom_message_title'}
	...
	<span>
		<input class="button" type='submit' id='submit' name='send' value="{_p var='send_invites'}"/>
		{if isset($friends_count) && $friends_count >= $max_invitation}<input class="button" type='button' name='send' value="{_p var='send_all' contacts=$friends_count}" onclick="sendallPopup('{$provider_box}', {$friends_count})"/>{/if}
		<input class="button" type='button' id='' name='send' value ="{_p var='skip'} &gt;&gt;" onclick="document.getElementById('skip').submit();">
	</span>
