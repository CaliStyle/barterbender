<div id="openinviter">
    ...
    <table class='thTableOddRow' align='left' cellspacing='0' cellpadding='5px' style=" width: 98%;border-left:2px solid #EDEDED;" >
        ...
        <div id="sort">
            <ul style="text-align: center;">
                ...
                for ($letter = ord('A'); $letter <= ord('Z'); $letter++)
                    echo '<li id="id_letter_' . chr($letter) . '"><a rel="letter_' . chr($letter) . '" class="letter" href="javascript:void(0);">' . chr($letter) . '</a></li>';
                ?>
            </ul>
        </div>
        *}
        <div id="div_list_view" style="width:98%;padding-left:0px;">
            <table class='thTableOddRow' align='left' cellspacing='0' cellpadding='5px' style="width:100%; border:0px solid">
                ...
                <tr class='thTableOddRow' id="row_<?php echo $counter ?>">
                    <td align="left" width="10px">
                        <input id='check_<?php echo $counter ?>' name='items[]' onclick='check_toggle(<?php echo $counter ?>,document.getElementById("row_<?php echo $counter ?>"),false);' value='{$invite_list.email}' type='checkbox' class='thCheckbox'>
                ...
                {/if}
                {/foreach}
				<?php if ($is_contact) echo "<tr class='thTableOddRow'><td align='center' style='padding:20px;' colspan='3'>" . _p('there_is_no_contact_in_your_account') . "</td></tr>"; ?>
            </table>
        </div>
    </div>
    <div class="clear"></div>
    ...
    <form method="post" action="{url link='contactimporter.invite'}" class="global_form" id='openinviterform' name='openinviterform' enctype="application/x-www-form-urlencoded" onsubmit="return check_select_invite();">
        <div class="form-wrapper" id="message-wrapper" style="clear:both;margin-top:5px">
            ...
            <div class="form-label" id="message-label" style="width: 120px;text-align: left;"><br/>
                <label class="optional" for="message" style="margin-top:5px">
                    ...
                    <textarea rows="6" cols="45" id="message" name="message">{_p var='default_invite_message_text'}</textarea>
                </label>
            </div>
            {else}
        ...
        <span>
            <button class="btn btn-sm btn-primary" type="submit" name="mysubmit" value="{_p var='send_invites'}" id='submit'>{_p var='send_invites'}</button>
            <button class="btn btn-sm btn-default" type='button' id='' name='send' value ="{_p var='skip'} &gt;&gt;" onclick="document.getElementById('skip').submit();">{_p var='skip'} &gt;&gt;</button>
        </span>
        {if Phpfox::getUserId() }
