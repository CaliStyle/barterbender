...
class FoxFeedsPro_Component_Controller_Addfeed extends Phpfox_Component
{
	/**
	...
	private function validate($aVals, $bIsEdit =false)
	{
		...
		else
		{
			...
			if(isset($aVals['item_count']) && $aVals['item_count'] == 0)
			{
				//Phpfox_Error::set(_p('foxfeedspro.the_rss_provider_url_is_not_supported'));
				//return false;
			}
		}
		...
		if(isset($aVals['sYnFfFrom']) && 
			...
			&& strlen(trim($aVals['time_delete_news'])) > 0
			&& (!is_numeric($aVals['time_delete_news']) || ctype_digit($aVals['time_delete_news']) == false)
		)
		{
			Phpfox_Error::set(_p('foxfeedspro.the_number_of_delete_news_after_every_x_days_is_invalid'));
			return false;
		}
	...
	public function process ()
	{
		...
		Phpfox::getUserParam("foxfeedspro.can_add_rss_provider",TRUE);
		...
		if($iEditId = $this->request()->getInt('feed'))
		{
            ...
            if (!empty($aFeed['logo_file'])) {
                $aFeed['current_image'] = Phpfox::getLib('image.helper')->display(
                    array(
                        'server_id' => 0,
                        'path' => 'core.url_pic',
                        'file' => $aFeed['logo_file'],
                        'suffix' => '_120',
                        'return_url' => true
                    )
                );
			...
			else
			{
				...
				if($aFeed['feed_logo'])
				{
					$aFeed['feed_logo'] = Phpfox::getParam('core.url_pic')."foxfeedspro/".$aFeed['feed_logo'];
				}
			}
		...
		if('profile' == $sYnFfFrom || 'pages' == $sYnFfFrom)
		{
			if('profile' == $sYnFfFrom){
				$breadcrumbUrl = (!$bIsEdit ? $this->url()->makeurl('profile.foxfeedspro.profileaddrssprovider') 
											: $this->url()->makeurl('profile.foxfeedspro.profileaddrssprovider.feed_'.$iEditId));
			} else {								
				$breadcrumbUrl = (!$bIsEdit ? $aParentModule['url'] . 'foxfeedspro/profileviewrss/go_profileaddrssprovider/' 
											: $aParentModule['url'] . 'foxfeedspro/profileviewrss/go_profileaddrssprovider/feed_' . $iEditId);
			}
		} else {
			$breadcrumbUrl = (!$bIsEdit ? $this->url()->makeurl('foxfeedspro.addfeed') : $this->url()->makeurl('foxfeedspro.addfeed.feed_'.$iEditId));
		}						
			...
			if( $this->validate($aVals, $bIsEdit) )
			{
					...
					if(Phpfox::getService('foxfeedspro.process')->editFeed($aVals))
					{
						if(isset($aVals['sYnFfFrom']) && 
							...
							if('profile' == $aVals['sYnFfFrom']){
								$this->url()->send(Phpfox::getLib('url')->makeUrl('profile.foxfeedspro.profilemanagerssprovider'), null, _p('foxfeedspro.the_feed_name_was_edited_successfully',array('feed_name' =>substr($aVals['feed_name'], 0, 200))));
							} else {								
								$this->url()->send($aParentModule['url'] . 'foxfeedspro/profileviewrss/go_profilemanagerssprovider/', null, _p('foxfeedspro.the_feed_name_was_edited_successfully',array('feed_name' =>substr($aVals['feed_name'], 0, 200))));
							}
						} else {
							$this->url()->send('foxfeedspro.feeds', null, _p('foxfeedspro.the_feed_name_was_edited_successfully',array('feed_name' =>substr($aVals['feed_name'], 0, 200))));
						}						
					}	
					else
					{
				...
				elseif($iId =  Phpfox::getService('foxfeedspro.process')->addFeed($aVals))
				{
					...
					if(isset($aVals['sYnFfFrom']) && 
						...
						if('profile' == $aVals['sYnFfFrom']){
							$this->url()->send(Phpfox::getLib('url')->makeUrl('profile.foxfeedspro.profilemanagerssprovider'), null,_p('foxfeedspro.the_feed_name_was_created_successfully',array('feed_name'=>substr($aVals['feed_name'], 0, 200))));
						} else {								
							$this->url()->send($aParentModule['url'] . 'foxfeedspro/profileviewrss/go_profilemanagerssprovider/', null,_p('foxfeedspro.the_feed_name_was_created_successfully',array('feed_name'=>substr($aVals['feed_name'], 0, 200))));
						}
					} else {
						$this->url()->send('foxfeedspro.feeds', null,_p('foxfeedspro.the_feed_name_was_created_successfully',array('feed_name'=>substr($aVals['feed_name'], 0, 200))));
					}					
				}
				else
				{
