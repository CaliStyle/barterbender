	...
	<ul class="action">
		...
		<li class="{if $iCategoryItemView == $aCategory.category_id} active{/if}"><a href="{$aCategory.url}" class="ajax_link">{$aCategory.name|convert|clean}</a>
			...
			<ul>
				...
				<li class="{if $iCategoryItemView == $aSubCategory.category_id} active{/if} {if isset($sModule)}{$sModule}_{/if}subcategory_{$aCategory.category_id} special_subcategory" {if $iKey >= Phpfox::getParam('foxfeedspro.setting_subcategories_to_show_at_first')}style="display:none;"{/if}>
				...
				{if $iKey >= Phpfox::getParam('foxfeedspro.setting_subcategories_to_show_at_first') && Phpfox::getParam('foxfeedspro.setting_subcategories_to_show_at_first') > 0}
