 ...
 * @version        3.01
 */
class Coupon_Service_FAQ_FAQ extends Phpfox_Service
{
        ...
        {
                $this->_sTable = Phpfox::getT('coupon_faq');
        }
        ...
        public function get()
        {
                $sCacheId = $this->cache()->set('coupon_faq_display_' . $this->_sDisplay . '_' . Phpfox::getLib('locale')->getLangId());
        ...
        private function _get($iParentId, $iActive = null)
        {
                ...
                if (count($aFAQs))
                {
                        ...
                        foreach ($aFAQs as $iKey => $aFaq)
                        {
                                ...
                                {
                                        $sOutput .= '<li><img src="' . Phpfox::getLib('template')->getStyle('image', 'misc/draggable.png') . '" alt="" /> <input type="hidden" name="order[' . $aFaq['faq_id'] . ']" value="' . $aFaq['ordering'] . '" class="js_mp_order" /><a href="#?id=' . $aFaq['faq_id'] . '" class="js_drop_down">' . strip_tags(Phpfox::getLib('parse.input')->prepare($aFaq['question'])) . '</a>' . $this->_get($aFaq['faq_id'], $iActive) . '</li>' . "\n";
                                } else
                                {
        ...
        {
            return $this->database()->select('question_parsed as question, answer_parsed as answer')->from($this->_sTable)->order('ordering ASC')->execute('getRows');
        }
        ...
        public function __call($sMethod, $aArguments)
        {
                 ...
                 */
                if ($sPlugin = Phpfox_Plugin::get('coupon.service_faq_faq__call'))
                {
                        return eval($sPlugin);
                }
