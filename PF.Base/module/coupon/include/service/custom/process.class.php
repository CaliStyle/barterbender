...
class Coupon_Service_Custom_Process extends Phpfox_service
{
    ...
    public function addValue($aValues, $iCouponId)
    {
        $aInsert = array('coupon_id' => $iCouponId);
    ...
    public function updateValue($aValues, $iCouponId)
    {
        $aUpdate = array();
        ...
        foreach($aValues as $iFieldId => $aValue)
        {
            ...
            $aFields = $this->database()->select('cv.value_id')
                ...
                ->where('cv.field_id = '. $iFieldId .' AND cv.coupon_id = '.$iCouponId)
                ->execute('getSlaveRows');

            if(count($aFields) <= 0) {
                $this->addValue( array( $iFieldId => $aValue), $iCouponId);
                continue;
            }
            ...
            if(is_array($aValue))
            {
               ...
               $this->database()->delete($this->_sTableValue,'field_id = '. $iFieldId .' AND coupon_id = '.$iCouponId);
            ...
            else
            {
                ...
                $this->database()->update($this->_sTableValue, $aUpdate,' coupon_id = ' . $iCouponId . ' AND field_id = '.$iFieldId);
            }
        }
    ...
    private function _addOptions($iFieldId, &$aVals)
    {
        ...
        if (isset($aVals['current']))
        {
            ...
            $aExisting[] = str_replace('core.coupon_cf_option_' . $aVals['field_id'] . '_','',$sVarName);
        }
        ...
        foreach ($aVals['option'] as $iKey => $aOptions)
        {
            ...
            foreach ($aOptions as $sLang => $aOption)
            {
                ...
                $sPhraseVar = 'coupon_cf_option_' . $iFieldId . '_' . $iSeqNumber;
