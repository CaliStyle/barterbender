...
$Behavior.ajxCatSearchActions = function(){
	$("#advmarketplace_ajx_catspros").change(function(){
		$.ajaxCall("advancedmarketplace.showSubcats", "pid=" + $(this).val());
	});
}
	...
	update: function()
	{
		...
		$('.js_option_holder').each(function()
		{
			...
			$(this).find('input').each(function()
			{
				...
				{
					$(this).attr('name', $(this).attr('name').replace('#', (iCnt-1)));
				}
	...
	action: function(oObj, sAction)
	{
		...
		switch (sAction)
		{
			case 'edit':
				...
				{
					window.location.href = this.sUrl + 'addcustomgroup/id_' + aParams['id'] + '/';
				}
				else
				{
					window.location.href = this.sUrl + 'addcustomfield/id_' + aParams['id'] + '/';
				}
				break;
			case 'delete':
				if (confirm(oTranslations['core.are_you_sure']))
				{
					...
					{
						$.ajaxCall('advancedmarketplace.deleteField', 'id=' + aParams['id']);
					}					
				}
				break;
			default:
				if (aParams['type'] == 'group')
				{
					$.ajaxCall('advancedmarketplace.toggleActiveGroup', 'id=' + aParams['id']);
				}
				else
				{
					$.ajaxCall('advancedmarketplace.toggleActiveField', 'id=' + aParams['id']);
				}				
				break;
		}
...
{	
	...
	{
		if (confirm(oTranslations['coupon.are_you_sure_you_want_to_delete_this_custom_option']))
		{
			aParams = $.getParams(this.href);
			
			$.ajaxCall('advancedmarketplace.deleteOption', 'id=' + aParams['id']);
		}
