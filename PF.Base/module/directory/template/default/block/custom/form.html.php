{if count($aCustomFields)}
    {foreach from=$aCustomFields item=aField}
        <div class="form-group">
            <label>{if $aField.is_required==1}{required}{/if}{phrase var=$aField.phrase_var_name}</label>
            ...
            {elseif $aField.var_type=='checkbox'}
                {foreach from=$aField.option key=opId item=opPhrase}
                    <div class="checkbox">
                        <label><input {if $aField.is_required==1}data-isrequired="1"{else}data-isrequired="0"{/if} data-type="checkbox" id="js_jp_cf_{$aField.field_id}" type="checkbox" name="val[custom][{$aField.field_id}][]" value="{$opId}"
                        {if isset($aField.value)}
            ...
            {elseif $aField.var_type=='radio'}
                {foreach from=$aField.option key=opId item=opPhrase}
                        ...
                        <div class="radio">
                            <label>
                                <input {if $aField.is_required==1}data-isrequired="1"{else}data-isrequired="0"{/if} data-type="radio" id="js_jp_cf_{$aField.field_id}" type="radio" name="val[custom][{$aField.field_id}][]" value="{$opId}" {if $opId == $checked} checked {/if} />
                                {phrase var=$opPhrase}
                            </label>
                        </div>
                                {/foreach}
                        ...
                        <div class="radio">
                            <label>
                                <input {if $aField.is_required==1}data-isrequired="1"{else}data-isrequired="0"{/if} data-type="radio" id="js_jp_cf_{$aField.field_id}" type="radio" name="val[custom][{$aField.field_id}][]" value="{$opId}" />
                                {phrase var=$opPhrase}
                            </label>
                        </div>
                        {/if}
