...
class Directory_Service_Custom_Group extends Phpfox_service
{
    ...
    {
        $this->_sTable = Phpfox::getT('directory_custom_group');
    }
    ...
    public function getGroupForEdit($iGroupId){
        ...
        $aMappingCategoriesSql =  $this->database()->select('category_id')
                    ->from(Phpfox::getT('directory_category_customgroup_data'))
                    ->where('group_id = '.$iGroupId)
                    ->execute('getSlaveRows');
        ...
        $aMappingCustomFields =  Phpfox::getService('directory.custom')->getCustomFieldByGroupId($iGroupId);
    ...
    public function addGroup($aGroup){
        ...
        if (empty($sParsedVar))
        {
            return Phpfox_Error::set(_p('directory.group_name_cannot_be_empty'));
        }
        ...
        if(isset($aGroup['customfield']) && count($aGroup['customfield']) > 0){
            foreach ($aGroup['customfield'] as $iFieldId) {
                $this->database()->update(Phpfox::getT('directory_custom_field'),
                            array(
                                "group_id" => $iGroupId
                            ),
                            'field_id = '.$iFieldId
        ...
        //remove old mapping
        $this->database()->delete(Phpfox::getT("directory_category_customgroup_data"), 'group_id = '.$iGroupId);

        //add mapping category
        if(isset($aGroup['categories']) && count($aGroup['categories']) > 0){
            foreach ($aGroup['categories'] as $aCategory) {
                    $iGroupId = $this->database()->insert(Phpfox::getT("directory_category_customgroup_data"), array(
    ...
    public function updateGroup($iGroupId,$aGroup){
        ...
        if(isset($aGroup['categories']) && count($aGroup['categories']) > 0){
            foreach ($aGroup['categories'] as $aCategory) {
                    $this->database()->insert(Phpfox::getT("directory_category_customgroup_data"), array(
                        "category_id" => $aCategory,
                        "group_id" => $iGroupId,                
                    ));                
            }
    ...
    public function getForListing()
    {
        $aFields = $this->database()->select('cf.*')
            ->from(PhpFox::getT('directory_custom_field'), 'cf')
        ...
        $aGroups = $this->database()->select('cg.*')
            ->from(Phpfox::getT('directory_custom_group'), 'cg')
    ...
    public function deleteGroup($iGroupId){
        ...
        $this->database()->update(Phpfox::getT('directory_custom_field'), array('group_id' => 0), 'group_id = ' . $aGroup['group_id']);
