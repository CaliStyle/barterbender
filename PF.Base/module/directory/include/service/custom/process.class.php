...
class Directory_Service_Custom_Process extends Phpfox_service
{
    ...
    {
        $this->_sTable = Phpfox::getT('directory_custom_field');
        $this->_sTableOption = Phpfox::getT('directory_custom_option');
        $this->_sTableValue = Phpfox::getT('directory_custom_value');
    }
    ...
    public function addValue($aValues, $iBusinessId)
    {
        $aInsert = array('business_id' => $iBusinessId);
    ...
    public function updateValue($aValues, $iBusinessId)
    {
        $aUpdate = array();
        ...
        foreach($aValues as $iFieldId => $aValue)
        {
            ...
            $aFields = $this->database()->select('cv.value_id')
                ...
                ->where('cv.field_id = '. $iFieldId .' AND cv.business_id = '.$iBusinessId)
                ->execute('getSlaveRows');

            if(count($aFields) <= 0) {
                $this->addValue( array( $iFieldId => $aValue), $iBusinessId);
                continue;
            }
            ...
            if(is_array($aValue))
            {
               ...
               $this->database()->delete($this->_sTableValue,'field_id = '. $iFieldId .' AND business_id = '.$iBusinessId);
            ...
            else
            {
                ...
                $this->database()->update($this->_sTableValue, $aUpdate,' business_id = ' . $iBusinessId . ' AND field_id = '.$iFieldId);
            }
        }
