...
class Directory_Service_Custom_Custom extends Phpfox_service
{
    ...
    public function getCustomFieldByBusinessId($iBusinessId)
    {
        if((int)$iBusinessId == 0){
            return array();
        }
        $aFields = $this->database()->select('cf.*, cv.value,cv.business_id')
            ->from($this->_sTable, 'cf')
            ->leftJoin($this->_sTableValue, 'cv', 'cv.field_id = cf.field_id AND cv.business_id = '.$iBusinessId)
            ->group('cf.field_id')
            ...
            foreach($aFields as $k=>$aField)
            {
                if(in_array($aField['var_type'], $this->_aHasOption))
                {
                    ...
                    $aOptions = $this->database()->select('co.*')
                        ...
                        ->where('co.field_id = '.$aField['field_id'].' AND cv.business_id = '.$iBusinessId)
