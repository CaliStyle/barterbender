...
class Directory_Component_Block_detailvideoslist extends Phpfox_Component {
	...
	public function process() {
		...
		$aVideos = array();
		$iCountVideos = 0;
		list($aVideos, $iCountVideos) = Phpfox::getService('directory')->getVideoByBusinessId($aBusiness['business_id'], $aConds, $aExtra, true);

		foreach ($aVideos as $iKey => $aRow)
		{	
			$aVideos[$iKey]['link'] = Phpfox::permalink($sModuleId, $aRow['video_id'], $aRow['title']);
            Phpfox::getService('directory')->convertImagePath($aVideos[$iKey], 120);
		}
