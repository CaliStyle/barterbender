...
class Directory_Component_Block_DetailFollowersLists extends Phpfox_Component {
	...
	public function process() {
		...
		$iPage = 1;
		$aConds = array(' 1=1 ');
		$aExtra['order'] = 'df.time_stamp DESC';
			...
			if(isset($aVals['filterinbusiness_sort']) && $aVals['filterinbusiness_sort']) {
				switch ($aVals['filterinbusiness_sort']){
					case 'newest': 
						...
						break;
					case 'oldest': 
						$aExtra['order'] = "df.time_stamp ASC";
						break;
					case 'a_z': 
		...
		$aFollowers = array();
		$iCountFollowers = 0;
		list($aFollowers, $iCountFollowers) = Phpfox::getService('directory')->getFollowersByBusinessId($aBusiness['business_id'], $aConds, $aExtra, true);				

		$this->setParam('aPagingParams', array(
			'total_all_result' => $iCountFollowers,
			'total_result' => count($aFollowers),
			'page' => $iPage,
			'limit' => $iItemPerPage
		));
		...
		$this->template()->assign(array(
				...
				'iCountFollowers' => $iCountFollowers, 
				'sLink' => $sLink, 
