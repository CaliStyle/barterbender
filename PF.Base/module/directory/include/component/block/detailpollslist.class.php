...
class Directory_Component_Block_detailpollslist extends Phpfox_Component {
	...
	public function process() {
		...
		$iPage = 1;
		$aConds = array(' 1=1 ');
		$aExtra['order'] = 'poll.time_stamp DESC';
		...
		if($aVals = $this->getParam('aQueryParam')) {
			...
			if(isset($aVals['keyword']) && $aVals['keyword']) {
				...
				$aConds[] = 'poll.question like \'%' . $sKeywordParse . '%\' ';
			}
			...
			if(isset($aVals['filterinbusiness_sort']) && $aVals['filterinbusiness_sort']) {
				switch ($aVals['filterinbusiness_sort']){
					case 'latest': 
						...
						break;
					case 'most_viewed': 
						$aExtra['order'] = "poll.total_view DESC";
						break;
					case 'most_liked': 
						$aExtra['order'] = "poll.total_like DESC";
						break;
					case 'most_discussed': 
						$aExtra['order'] = "poll.total_comment DESC";
						break;
				}					
			}				
		...
		$iCountPolls = 0;
		$aPollsTmp = array();
		list($aPolls, $iCountPolls) = Phpfox::getService('directory')->getPollByBusinessId($aBusiness['business_id'], $aConds, $aExtra, true);				
		Phpfox::getService('directory')->processPollRows($aPolls);

		$this->setParam('aPagingParams', array(
			'total_all_result' => $iCountPolls,
			'total_result' => count($aPolls),
			'page' => $iPage,
			'limit' => $iItemPerPage
		));
