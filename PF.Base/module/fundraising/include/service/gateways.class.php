...
Phpfox::getService('fundraising.classhelper')->getAPIClass('fundraising.include.service.gatewayinterface');
class Fundraising_Service_Gateways extends Phpfox_Service
{
	...
	public function load($sGateway, $aSettings = null)
	{
		...
		{
			$sFilePath = PHPFOX_DIR_MODULE . 'fundraising' . PHPFOX_DS . 'include' . PHPFOX_DS . 'service' .PHPFOX_DS . 'api' . PHPFOX_DS . $sGateway . '.class.php';
                        $aGatewaySetting = Phpfox::getService('fundraising.gateway')->getActiveGatewaySetting($sGateway);
                        if($aGatewaySetting)
                        {
                            $this->_aObject[$sGateway] = (file_exists($sFilePath) ? Phpfox::getService('fundraising.classhelper')->getAPI('fundraising.include.service.api.' . $sGateway) : false);
	...
	{ 
		...
		$sUrl = Phpfox::getLib('phpfox.url')->makeUrl('fundraising.gateway.callback', array($sGateway));
