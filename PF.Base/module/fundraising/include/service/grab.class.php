...
class Fundraising_Service_Grab extends Phpfox_Service
{
    ...
    {
        $sCacheId = $this->cache()->set('fundraising_api_embed_ly');
        $sCacheId2 = $this->cache()->set('fundraising_api_embed_ly_site');
        $aSites = array();
    ...
    public function get($sUrl)
    {
        ...
        if (substr($sUrl, 0, 7) == 'http://' || substr($sUrl, 0, 8) == 'https://') {
            if (strpos($sUrl, 'youtube') || (preg_match('/http:\/\/youtu\.be\/(.*)/i', $sUrl, $aMatches) && isset($aMatches[1]))) {
                ...
                if (isset($aMatches) && $aMatches[1]) {
                    $sUrl = 'https://www.youtube.com/watch?v=' . $aMatches[1];
                }
            }
            ...
            if (is_array($this->_aRegex) && count($this->_aRegex)) {
                ...
                if (!$this->_aData['site_id']) {
                    ...
                    return Phpfox_Error::set(_p('not_a_valid_site_valid_sites_asites', array('aSites' => implode(', ', $aSites))));
                }
            }
        ...
        return Phpfox_Error::set(_p('please_provide_a_valid_url_for_your_video'));
    }
    ...
    public function parse()
    {
        ...
        // Plugin call
        if ($sPlugin = Phpfox_Plugin::get('fundraising.service_grab_parse__start')) {
            eval($sPlugin);
        }
        ...
        if ($bSent === false) {
            ...
            if (is_array($this->_aRegex) && count($this->_aRegex)) {
                $oVideo = json_decode(Phpfox::getLib('request')->send('http://api.embed.ly/1/oembed?format=json&maxwidth=400&url=' . urlencode($this->_aData['url']), array(), 'GET', $_SERVER['HTTP_USER_AGENT']));
                ...
                switch ($this->_aData['site_id']) {
                    case 'youtube':
                        ...
                        $xVideo = Phpfox::getLib('request')->send($url, array(), 'GET');
                        $data = json_decode($xVideo);
                        if (empty($data->items)) {
                            return false;
                        }
                        $this->_aData['html'] = $data->items[0];
                        break;
                    case 'myspace':
                    case 'break':
                    case 'metacafe':
        ...
        if ($sPlugin = Phpfox_Plugin::get('fundraising.service_grab_parse__end')) {
            eval($sPlugin);
        }
    ...
    public function title()
    {
        ...
        switch ($this->_aData['site_id']) {
            // YouTube
            case 'youtube':
                if (isset($this->_aData['html']->snippet->title)) {
                    $sTitle = $this->_aData['html']->snippet->title;
                }
                break;
            case 'myspace':
    ...
    public function image($iId, $sModule = 'fundraising', $sImageName)
    {
        if (isset($this->_aCustom['default_image'])) {
            ...
            switch ($this->_aData['site_id']) {
                // YouTube
                case 'youtube':
                    $sImage = $this->_aData['html']->snippet->thumbnails->medium->url;
                    $thumbnails = end($this->_aData['html']->snippet->thumbnails);
                    $sImageForSlide = $thumbnails->url;
                    break;
                case 'myspace':
    ...
    public function description()
    {
        ...
        switch ($this->_aData['site_id']) {
            // YouTube
            case 'youtube':
                if (isset($this->_aData['html']->snippet->description)) {
                    $sDescription = $this->_aData['html']->snippet->description;
                }
                break;
            case 'myspace':
    ...
    public function duration()
    {
        ...
        switch ($this->_aData['site_id']) {
            // YouTube
            case 'youtube':
                if (isset($this->_aData['html']->contentDetails)) {
                    $start = new DateTime('@0'); // Unix epoch
                    $start->add(new DateInterval($this->_aData['html']->contentDetails->duration));
                    $sDuration = '';
                    if ((int)$start->format('H') > 0)
                        $sDuration .= $start->format('H') . ':';
                    $sDuration .= $start->format('i') . ':' . $start->format('s');
                }
                break;
            default:
    ...
    public function getTimeStamp()
    {
        switch ($this->_aData['site_id']) {
            // YouTube
            case 'youtube':
                if (isset($this->_aData['html']->snippet->publishedAt)) {
                    $sTimeStamp = strtotime($this->_aData['html']->snippet->publishedAt);
                }
                break;
            default:
    ...
    public function embed()
    {
        ...
        switch ($this->_aData['site_id']) {
            ...
            case 'youtube':
                ...
                parse_str($aUrl['query'], $aStr);
                $sEmbed = '<object width="425" height="344"><param name="wmode" value="transparent"></param><param name="movie" value="https://www.youtube.com/v/' . $aStr['v'] . (($bAutoPlay = false) ? '&amp;autoplay=1' : '') . (($bFullScreen = false) ? '&amp;fs=1' : '') . (($bDisableYouTubeRelateVideos = true) ? '&amp;rel=0' : '') . '"></param><param name="allowFullScreen" value="true"></param><param name="allowscriptaccess" value="always"></param><embed wmode="transparent" src="https://www.youtube.com/v/' . $aStr['v'] . (($bAutoPlay = false) ? '&amp;autoplay=1' : '') . (($bFullScreen = false) ? '&amp;fs=1' : '') . (($bDisableYouTubeRelateVideos = true) ? '&amp;rel=0' : '') . '" type="application/x-shockwave-flash" allowscriptaccess="always" allowfullscreen="true" width="425" height="344"></embed></object>';
                break;
            case 'myspace':
            ...
            case 'break':
                ...
                //return '<embed src="'.$aMatches[0].'" width="'.Phpfox::getParam('fundraising.player_width').'" height="'.Phpfox::getParam('fundraising.player_height').'"></embed>';
            ...
            case 'metacafe':
                ...
                foreach ($aMatches[0] as $sMatch) {
                    ...
                    if (strpos($sMatch, $aVidName[1]) !== false) {
                        ...
                        //$sEmbed = '<embed flashVars="playerVars=showStats=no|autoPlay='.(Phpfox::getParam('fundraising.embed_auto_play') ? 'yes' : 'no').'" src="'.$sMatch.'" width="'.Phpfox::getParam('fundraising.player_width').'" height="'.Phpfox::getParam('fundraising.player_height').'" wmode="transparent" pluginspage="http://www.macromedia.com/go/getflashplayer" type="application/x-shockwave-flash"> </embed>';
                        $sEmbed = '<embed class="metacafe_video_player" flashVars="playerVars=showStats=no|autoPlay=' . (Phpfox::getParam('fundraising.embed_auto_play') ? 'yes' : 'no') . '" src="' . $sMatch . '" width="400" height="348" wmode="transparent" pluginspage="http://www.macromedia.com/go/getflashplayer" type="application/x-shockwave-flash"> </embed>';
    ...
    public function __call($sMethod, $aArguments)
    {
         ...
         */
        if ($sPlugin = Phpfox_Plugin::get('fundraising.service_grab__call')) {
            return eval($sPlugin);
        }
