...
<style type="text/css">
    ...
    .yn_profilepopup_cover img {
        width:535px;
    }
</style>
{/literal}
<div class="no-popup uiContextualDialogContent">
    <div class="yn_profilepopup_hovercard_stage" {if empty($aEvent.image_path)} style="padding-top: 10px;" {/if}>
        <!-- event not found -->
        ...
        <div class="yn_profilepopup_hovercard_content">
            ...
            <div class="yn_profilepopup_cover">
                {img server_id=$aEvent.server_id title=$aEvent.title path='event.url_image' file=$aEvent.image_path suffix=''}
            ...
            <div class="yn-profilepopup_basic_info" {if !Phpfox::getParam('profilepopup.enable_thumbnails')}style="margin-left: 10px;"{/if}>
                ...
                <div class="yn_profilepopup_image row_banner">
                    <header>
                ...
                {/if}
                <div class="yn_profilepopup_main_title {if !Phpfox::getParam('profilepopup.enable_thumbnails')}yn_profilepopup_title_nophoto{/if} ">
                <a href="{permalink module='event' id=$aEvent.event_id title=$aEvent.title}" class="link">{$aEvent.title|clean|split:25}</a>
                </div>
            </div>
        ...
        <div class="yn_profilepopup_hovercard_content">
            ...
            <div class="yn-profilepopup_basic_info" {if !Phpfox::getParam('profilepopup.enable_thumbnails')}style="margin-left: 10px;"{/if}>
                ...
                {/if}
                <div class="yn_profilepopup_main_title {if !Phpfox::getParam('profilepopup.enable_thumbnails')}yn_profilepopup_title_nophoto{/if} "> <a href="{permalink module='event' id=$aEvent.event_id title=$aEvent.title}" class="link">{$aEvent.title|clean|split:25}</a></div>
            </div>
            ...
            <div class="yn_profilepopup_main" {if !Phpfox::getParam('profilepopup.enable_thumbnails')}style="margin-left: 10px;"{/if}>    
                <div>
                    ...
                    <div class="yn_profilepopup_info">
                        <div class="yn_profilepopup_info_left">{$aItem.lang_name}:&nbsp;</div>
                        <div class="yn_profilepopup_info_right">{phrase var='profilepopup.from'}: {$aEvent.start_date}<br/>{phrase var='profilepopup.to'}: {$aEvent.end_date}&nbsp;</div>
                    </div>
                    {/if}
                    <!-- Location -->
                ...
                <div class="yn_profilepopup_mutual">
                    <a href="#" onclick="$Core.box('profilepopup.getJoinedFriends', 300, '&item_type=event&item_id={$aEvent.event_id}');return false;">{phrase var='profilepopup.joined_friends_total' total=$iJoinedFriendTotal}</a>                                        
    ...
    <div class="yn_profilepopup_hovercard_footer">
        {if Phpfox::isUser() &&  isset($iIsEvent) && $iIsEvent == 1 && isset($iIsCanView) && $iIsCanView == 1}
            <ul class="yn_profilepopup_list_horizontal">
                {if Phpfox::isModule('foxfavorite') && Phpfox::isUser() && isset($sFFModule) && isset($iFFItemId) && $sFFModule == 'event'}
                ...
                {if $aEvent.view_id == 0 && ($aEvent.user_id == Phpfox::getUserId() && Phpfox::getUserParam('event.can_edit_own_event')) || Phpfox::getUserParam('event.can_edit_other_event')}
                    <li class="yn_profilepopup_list_item" >
                        <a title="{phrase var='profilepopup.event_invite'}" class="yn_profilepopup_icon_invite" href="{url link='event.add.invite' id=$aEvent.event_id tab=invite}">
                ...
                {if !isset($aEvent.rsvp_id) || $aEvent.rsvp_id != 1}
                    <li class="yn_profilepopup_list_item" >
                        <a title="{phrase var='profilepopup.event_join'}" onclick="ynfbpp.closePopup(); $.ajaxCall('profilepopup.joinEvent', 'id={$aEvent.event_id}&amp;rsvp=1'); return false;" class="yn_profilepopup_icon_join" href="#">
                            <i class="fa fa-plus-square-o"></i>&nbsp;{phrase var='profilepopup.event_join'}
                        </a>
                    </li>
                {else}
                    <li class="yn_profilepopup_list_item" >
                        <a title="{phrase var='profilepopup.event_leave'}" onclick="ynfbpp.closePopup(); ynfbpp.leaveEvent({$aEvent.event_id}, '{phrase var='profilepopup.event_confirm_leave'}'); return false;" class="yn_profilepopup_icon_leave" href="#">
