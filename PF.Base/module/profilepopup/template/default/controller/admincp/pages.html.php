...
{literal}
        $Behavior.ynppInitPages = function()
        {
                $('#update').click(function(event)
                {
                        ...
                        $('#update').ajaxCall('profilepopup.updateGlobalSettings', '&item_type=pages' + '&' + $('#globalSettings').serialize());
                }); 
...
{if count($aAllItems)}
    <div class="panel panel-default">
        <form id="globalSettings" method="post" action="{url link='admincp.profilepopup.pages'}">
            <div class="panel-heading">
                <div class="panel-title">
                    <div class="p_4"><span style="font-weight: bold; text-decoration: underline; font-size: 15px;">{phrase var='profilepopup.notice'}</span>: {phrase var='profilepopup.pages_global_settings_info'}</div>
                </div>
            </div>
...
{else}
        <div class="p_4">
                {phrase var='profilepopup.no_pages_global_settings'}
        </div>
{/if}
