...
class Videochannel_Service_Grab extends Phpfox_Service
{
    ...
    {
        $sCacheId = $this->cache()->set('videochannel_api_embed_ly');
        $sCacheId2 = $this->cache()->set('videochannel_api_embed_ly_site');
        $aSites = array();
    ...
    public function get($sUrl)
    {
        ...
        if (substr($sUrl, 0, 7) == 'http://' || substr($sUrl, 0, 8) == 'https://') {
            if (strpos($sUrl, 'youtube') || (preg_match('/https?:\/\/youtu\.be\/(.*)/i', $sUrl, $aMatches) && isset($aMatches[1]))) {
                $this->_aRegex = false;
            ...
            if (is_array($this->_aRegex) && count($this->_aRegex)) {
                ...
                if (!$this->_aData['site_id']) {
                    ...
                    return Phpfox_Error::set(_p('videochannel.not_a_valid_site_valid_sites_asites', array('aSites' => implode(', ', $aSites))));
                }
            }
        ...
        return Phpfox_Error::set(_p('videochannel.please_provide_a_valid_url_for_your_video'));
    }
    ...
    public function parse()
    {
        ...
        // Plugin call
        if ($sPlugin = Phpfox_Plugin::get('videochannel.service_grab_parse__start')) {
            eval($sPlugin);
        }
        ...
        if ($sPlugin = Phpfox_Plugin::get('videochannel.service_grab_parse__end')) {
            eval($sPlugin);
        }
    ...
    public function image($iId, $sModule = 'videochannel', $sAddMore480ImageLocation = null)
    {
        if (isset($this->_aCustom['default_image']) && $sAddMore480ImageLocation == null) {
            $sImage = $this->_aCustom['default_image'];
        } else {
            switch ($this->_aData['site_id']) {
                // YouTube
                case 'youtube':
                    if(isset($this->_aData['html']->snippet->thumbnails)) {
        ...
        if (isset($sImage)) {
            if ($sAddMore480ImageLocation == null) {
                ...
                Phpfox::getLib('cdn')->put(sprintf($sImageLocation, '_480'));
                ...
                $sTempImage1 = 'video_temporal_image1_' . $iId;
                Phpfox::getLib('file')->writeToCache($sTempImage1, $oImage1);
                @copy(PHPFOX_DIR_CACHE . $sTempImage1, $sAddMore480ImageLocation);
                unlink(PHPFOX_DIR_CACHE . $sTempImage1);
            }
        }
    ...
    public function embed()
    {
        ...
        switch ($this->_aData['site_id']) {
            ...
            case 'youtube':
                ...
                $sEmbed = '<object width="425" height="344"><param name="wmode" value="transparent"></param><param name="movie" value="https://www.youtube.com/v/' . $aStr['v'] . (Phpfox::getParam('videochannel.embed_auto_play') ? '&amp;autoplay=1' : '') . (Phpfox::getParam('videochannel.full_screen_with_youtube') ? '&amp;fs=1' : '') . '"></param><param name="allowFullScreen" value="true"></param><param name="allowscriptaccess" value="always"></param><embed wmode="transparent" src="https://www.youtube.com/v/' . $aStr['v'] . (Phpfox::getParam('videochannel.embed_auto_play') ? '&amp;autoplay=1' : '') . (Phpfox::getParam('videochannel.full_screen_with_youtube') ? '&amp;fs=1' : '') . (Phpfox::getParam('videochannel.disable_youtube_related_videos') ? '&amp;rel=0' : '') . '" type="application/x-shockwave-flash" allowscriptaccess="always" allowfullscreen="true" width="425" height="344"></embed></object>';
                break;
            case 'myspace':
    ...
    public function __call($sMethod, $aArguments)
    {
         ...
         */
        if ($sPlugin = Phpfox_Plugin::get('videochannel.service_grab__call')) {
            return eval($sPlugin);
        }
