...
<div class="label_flow_menu">	
	<ul>
		<li class="label_flow_menu_active"><a href="#videochannel.upload{if defined('PHPFOX_GROUP_VIEW')}?bIsGroup=true{/if}">{phrase var='videochannel.add_a_video'}</a></li>
		<li class="last"><a href="#videochannel.share{if defined('PHPFOX_GROUP_VIEW')}?bIsGroup=true{/if}">{phrase var='videochannel.share_a_video'} </a></li>
	</ul>
	<br class="clear" />
</div>	
...
{/if}	
	...
	<div id="js_video_process" style="display:none;">
		<div class="message">
			{img theme='ajax/small.gif' alt='' class='v_middle'} <span id="js_upload_command">{phrase var='videochannel.uploading'}</span>: <span id="js_upload_file_name"></span>
		</div>
	</div>	
	
    <form method="post" action="{url link='videochannel.frame'}" id="js_video_form" enctype="multipart/form-data" target="js_upload_frame">
	{if $sModule}
	...
	{/if}
		...
		<div id="js_upload_inner_form">
			<div class="message" style="font-weight:normal;" id="copyright_notice">
				<p>
					{phrase var='videochannel.upload_copyrights_notice'}
				</p>
				<p>
					{phrase var='videochannel.copyright_consequences_notice'}
				</p>
			</div>	
			...
			<div class="table form-group">
				<div class="table_left">
					...
					{required}{phrase var='videochannel.select_video'}:
					{/if}
				</div>
				<div class="table_right">				
						...
						<div id="method_massuploader" class="upload_method" style="display:none;">
							...
							<span id="swfUploadText">{phrase var='videochannel.select_video'}</span>
					...
					<div class="extra_info">
						{phrase var='videochannel.you_can_upload_a_sfileext_file' sFileExt=$sFileExt}
					</div>		
					<div class="extra_info">
						<strong>{phrase var='videochannel.max_file_size_iuploadlimit' iUploadLimit=$iUploadLimit}</strong>
					</div>
			...
			<div class="table_clear">
				<input type="submit" value="Upload" class="button btn-primary" />
			</div>
		</div>
        
	</form>	
	<div id="js_upload_frame_holder"></div>
{if PHPFOX_IS_AJAX && !$bHideSwitchMenu}
</div>
{/if}