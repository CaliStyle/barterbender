    ...
    <div class="sub_section_menu">
        <ul class="action">
            {foreach from=$aCategories item=aCategory}
                <li class="{if $iCategoryChannelView == $aCategory.category_id} active{/if}"><a href="{$aCategory.url}" class="ajax_link">{$aCategory.name|convert|clean}</a>
                    {if isset($aCategory.sub) && count($aCategory.sub)}
                        <ul class="action">
                            {foreach from=$aCategory.sub item=aSubCategory key=iKey}
                                <li class="{if $iCategoryChannelView == $aSubCategory.category_id} active{/if} {if isset($sModule)}{$sModule}_{/if}subcategory_{$aCategory.category_id} special_subcategory" {if $iKey >= Phpfox::getParam('videochannel.subcategories_to_show_at_first')}style="display:none;"{/if}>
