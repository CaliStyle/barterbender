 ...
 */
class Donation_Service_Api_2checkout implements Donation_gatewayinterface
{
	/**
	...
	{		
		return array(
			'2co_id' => array(
				'phrase' => _p('core.2checkout_vendor_id_number'),
				'phrase_info' => _p('core.your_numerical_vendor_id'),
				'value' => (isset($this->_aParam['2co_id']) ? $this->_aParam['2co_id'] : '')
			),
			'2co_secret' => array(
				'phrase' => _p('core.2checkout_secret_word'),
				'phrase_info' => _p('core.the_secret_word_as_set_within_the_look_and_feel_page_of_your_2checkout_account'),
				'value' => (isset($this->_aParam['2co_secret']) ? $this->_aParam['2co_secret'] : '')
			)
		);
	}	
	...
	public function getForm()
	{
		...
		$aForm = array(
			...
			'param' => array(
				...
				'x_receipt_link_url' => Phpfox::getService('donation.gateways')->url('2checkout')
			)
		);     
		...
		if ($this->_aParam['recurring'] > 0)
		{
			...
			if($this->_aParam['recurring_cost'] > 0)
			{
				...
				if($this->_aParam['amount'] > 0)
				{
					...
					$aForm['param']['li_1_name'] = _p('subscribe.recurring_price');
	...
	public function callback()
	{
		...
		else 
		{
			if (isset($this->_aParam['message_type']) && isset($this->_aParam['key']))			
			{
				...
				$sHash = strtoupper(md5($this->_aParam['invoice_id'] . $this->_aParam['2co_id'] . $this->_aParam['invoice_id'] . $this->_aParam['2co_secret']));
				Phpfox::log('Refund hash.');
			}
			else 
			{
				$sHash = strtoupper(md5($this->_aParam['2co_secret'] . $this->_aParam['2co_id'] . $this->_aParam['order_number'] . $this->_aParam['total']));
