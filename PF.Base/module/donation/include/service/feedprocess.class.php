 ...
 */
class Donation_Service_Feedprocess extends Phpfox_Service {
    ...
    public function add($sType, $iItemId, $iPrivacy = 0, $iPrivacyComment = 0, $iParentUserId = 0, $iOwnerUserId = null, $bIsTag = 0)
    {
        //Plugin call
        ...
        if ((defined('PHPFOX_SKIP_FEED') && PHPFOX_SKIP_FEED))
        {
            return false;
        }
        ...
        if ($this->_bIsNewLoop)
        {
            if ($this->database()->isField(Phpfox::getT('feed'), 'feed_reference'))
            {
        ...
        {
            $aInsert['parent_user_id'] = $iParentUserId;
        ...
        if ($this->_bIsCallback && $this->_aCallback['module'] == 'pages' && !$this->_bIsNewLoop && $iParentUserId > 0)
        {
            ...
            $this->_aCallback = [];
            (isset($aUser['user_id']) && Phpfox::getUserId() == $aUser['user_id'] ? $this->add($sType, $iItemId, $iPrivacy, $iPrivacyComment) :  $this->add($sType, $iItemId, $iPrivacy, $iPrivacyComment, $iParentUserId, $aInsert['user_id']));
        }
    ...
    public function addComment($aVals)
    {
        ...
        {
            Phpfox::getLib('mail')
                    ->to($aVals['parent_user_id'])
                    ->subject(_p('feed.full_name_wrote_a_comment_on_your_wall', array('full_name' => Phpfox::getUserBy('full_name'))))
                    ->message(_p('feed.full_name_wrote_a_comment_on_your_wall_message', array('full_name' => Phpfox::getUserBy('full_name'), 'link' => $sLink)))
