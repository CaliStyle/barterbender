 ...
 */
class Donation_Service_Mail extends Phpfox_Service
{
    ...
    {		
		...
		if (!empty($sIds))
		{
			...
			{
				$this->database()
                        ->select('un.user_notification, ')
                        ->leftJoin(Phpfox::getT('user_notification'), 'un', "un.user_id = u.user_id AND un.user_notification = '" . $this->database()->escape($this->_sNotification) . "'");
			}		
			...
			if ($this->_aUsers === null)
		    {
				$aUsers = $this->database()
                        ->select('u.user_id, u.email, u.language_id, u.full_name, u.user_group_id')
			...
			{			    
				foreach ($aUsers as $aUser)
				{
					...
					{						
						...
						{
							$sMessage = _p($this->_aMessage[0], isset($this->_aMessage[1]) ? array_merge($aUser, $this->_aMessage[1]) : $aUser, false, null, $aUser['language_id']);
						}
						else
						{
						...
						{
							$sMessagePlain = _p($this->_aMessagePlain[0], isset($this->_aMessagePlain[1]) ? array_merge($aUser, $this->_aMessagePlain[1]) : $aUser, false, null, $aUser['language_id']);
						}
						else
						{
						...
						{
							$sSubject = _p($this->_aSubject[0], isset($this->_aSubject[1]) ? array_merge($aUser,$this->_aSubject[1]) : $aUser, false, null, $aUser['language_id']);
						}
						else
						{
						...
						$sMessage = preg_replace('/\{setting var=\'(.*)\'\}/is', "'' . Phpfox::getParam('\\1') . ''", $sMessage);
						$sMessage = preg_replace('/\{phrase var=\'(.*)\'\}/is', "'' . _p('\\1',{$this->_sArray}, false, null, '".$aUser['language_id']."') . ''", $sMessage);
						$sMessagePlain = preg_replace('/\{phrase var=\'(.*)\'\}/is', "'' . _p('\\1',{$this->_sArray}, false, null, '".$aUser['language_id']."') . ''", $sMessagePlain);
						$sMessagePlain = preg_replace('/\{setting var=\'(.*)\'\}/is', "'' . Phpfox::getParam('\\1') . ''", $sMessagePlain);
						$sSubject = preg_replace('/\{setting var=\'(.*)\'\}/is', "'' . Phpfox::getParam('\\1') . ''", $sSubject);
						$sSubject = preg_replace('/\{phrase var=\'(.*)\'\}/is', "'' . _p('\\1',{$this->_sArray}, false, null, '".$aUser['language_id']."') . ''", $sSubject);
		...
		{			
			...
			foreach ($aEmails as $sEmail)
			{
				...
				if (is_array($this->_aMessage))
				{
					$sMessage = _p($this->_aMessage[0], $this->_aMessage[1], false, null, Phpfox::getParam('core.default_lang_id'));
				}
				else
				{
					$sMessage = $this->_aMessage;
				...
				if (is_array($this->_aMessagePlain))
				{
					$sMessagePlain = _p($this->_aMessagePlain[0], $this->_aMessagePlain[1], false, null, Phpfox::getParam('core.default_lang_id'));
				}
				else
				{
					$sMessagePlain  = $this->_aMessagePlain;
				...
				if (is_array($this->_aSubject))
				{
					$sSubject = _p($this->_aSubject[0], $this->_aSubject[1], false, null, Phpfox::getParam('core.default_lang_id'));
				}
				else
				{
					$sSubject = $this->_aSubject;
				...
				$sEmailSig = "";
				$sMessagePlain = preg_replace('/\{phrase var=\'(.*)\'\}/is', "'' . _p('\\1', {$this->_sArray}, false, null, '". Phpfox::getParam('core.default_lang_id')."') . ''", $sMessagePlain);
				$sMessage = preg_replace('/\{phrase var=\'(.*)\'\}/is', "'' . _p('\\1', {$this->_sArray}, false, null, '". Phpfox::getParam('core.default_lang_id')."') . ''", $sMessage);
				$sSubject = preg_replace('/\{phrase var=\'(.*)\'\}/is', "'' . _p('\\1', {$this->_sArray}, false, null, '". Phpfox::getParam('core.default_lang_id')."') . ''", $sSubject);
