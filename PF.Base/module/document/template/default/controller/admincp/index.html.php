        ...
        <table class="table table-bordered" id="js_drag_drop" cellpadding="0" cellspacing="0">
            ...
            <tr>
                ...
                <td class="t_center w60">
                    ...
                    <div class="link_menu">
                        <ul>
                            <li><a href="{url link='admincp.document.add' id=$aCategory.category_id}">{phrase
                                    var='edit'}</a>
                            </li>
                            {if isset($aCategory.categories) && ($iTotalSub = count($aCategory.categories))}
                            <li><a href="{url link='admincp.document' sub={$aCategory.category_id}">{phrase
                            ...
                            <li><a href="" class="jsWarning" data-title="{_p('notice')}"
                                   data-message="{phrase var='you_can_not_delete_this_category_because_there_are_many_items_related_to_it'}">{phrase
                                    var='delete'}</a></li>
                            {else}
                            <li><a href="{url link='admincp.document' delete=$aCategory.category_id}" class="sJsConfirm"
                                   data-message="{phrase var='are_you_sure'}">{phrase var='delete'}</a></li>
                            {/if}
                        </ul>
                    </div>
                </td>
                ...
                <td class="t_center w140">
                    {if isset($aCategory.categories) && ($iTotalSub = count($aCategory.categories))}
                        <a href="{url link='admincp.document' sub={$aCategory.category_id}" class="">{$iTotalSub}</a>
                    {else}
                        0
                    {/if}
                </td>
                <td class="t_center">
                    <div class="js_item_is_active" style="{if !$aCategory.is_active}display:none;{/if}">
                        <a href="#?call=document.updateActivity&amp;id={$aCategory.category_id}&amp;active=0"
                    ...
                    <div class="js_item_is_not_active" style="{if $aCategory.is_active}display:none;{/if}">
                        <a href="#?call=document.updateActivity&amp;id={$aCategory.category_id}&amp;active=1"
