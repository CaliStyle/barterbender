 ...
 */
class Document_Service_Category_Category extends Phpfox_Service 
{
    private $_sOutput = '';
    ...
    {        
        $sCacheId = $this->cache()->set('document_category_browse' . ($sCategory === null ? '' : '_' . md5($sCategory)) . '_' . Phpfox::getLib('locale')->getLangId());
         if (!($aCategories = $this->cache()->get($sCacheId)))
        {
            $aCategories = $this->database()->select('mc.category_id, mc.name, mc.name_url')
                ...
                ->where('mc.parent_id = ' . ($sCategory === null ? '0' : (int) $sCategory) . ' AND mc.is_active = 1')
                ->order('mc.ordering ASC')
                ->execute('getRows');
              
            foreach ($aCategories as $iKey => $aCategory)
            {
                $aCategories[$iKey]['url'] = Phpfox::permalink('document.category', $aCategory['category_id'], ((Phpfox::isPhrase($aCategory['name'])) ? _p($aCategory['name']) : Phpfox::getLib('locale')->convert($aCategory['name'])));
                $aCategories[$iKey]['name'] = ((Phpfox::isPhrase($aCategory['name'])) ? _p($aCategory['name']) : Phpfox::getLib('locale')->convert($aCategory['name']));
                ...
                {
                    $aCategories[$iKey]['sub'][$iSubKey]['url'] = Phpfox::permalink('document.category', $aSubCategory['category_id'], Phpfox::getLib('locale')->convert($aSubCategory['name']));
                    $aCategories[$iKey]['sub'][$iSubKey]['name'] = Phpfox::getLib('locale')->convert($aSubCategory['name']);
                }
            }
            $this->cache()->save($sCacheId, $aCategories);
            $this->cache()->group('document_categories', $sCacheId);
        }
    ...
    public function get()
    {
        $sCacheId = $this->cache()->set('document_category_display_' . $this->_sDisplay . '_' . Phpfox::getLib('locale')->getLangId());
    ...
    {        
        $sCacheId = $this->cache()->set('document_parent_breadcrumb_' . md5($sCategory));
    ...
    public function getCategoriesById($iId = null, &$aCategories = null)
    {
        ...
        if ($aCategories === null)
        {
            $aCategories = $this->database()->select('pc.parent_id, pc.category_id, pc.name, pc.name_url')
                ->from(Phpfox::getT('document_category_data'), 'pcd')
                ->join($this->_sTable, 'pc', 'pc.category_id = pcd.category_id')
                ->where('pcd.document_id = ' . (int) $iId)
        ...
        {            
            ...
            {                
                $aBreadcrumb[] = array(((Phpfox::isPhrase($aCategory['name'])) ? _p($aCategory['name']) : Phpfox::getLib('locale')->convert($aCategory['name'])), Phpfox::permalink('document.category', $aCategory['category_id'], $aCategory['name']));
            }
            
        }        
        else 
        {            
            $aBreadcrumb[] = array(((Phpfox::isPhrase($aCategories[0]['name'])) ? _p($aCategories[0]['name']) : Phpfox::getLib('locale')->convert($aCategories[0]['name'])), Phpfox::permalink('document.category', $aCategories[0]['category_id'], ((Phpfox::isPhrase($aCategories[0]['name'])) ? _p($aCategories[0]['name']) : Phpfox::getLib('locale')->convert($aCategories[0]['name']))));
        }
    ...
    {
        $sCacheId = $this->cache()->set('document_category_childern_' . $sCategory);
    ...
    {
        $sCacheId = $this->cache()->set('document_category_parent_' . $sCategory);
    ...
    public function __call($sMethod, $aArguments)
    {
         ...
         */
        if ($sPlugin = Phpfox_Plugin::get('document.service_category_category__call'))
        {
            return eval($sPlugin);
        }
    ...
    private function _get($iParentId, $iActive = null)
    {
        ...
        if (count($aCategories))
        {
            ...
            else 
            {
                ...
                $this->_sOutput .= '<select name="val[category][]" class="js_mp_category_list form-control" id="js_mp_id_' . $iParentId . '">' . "\n";
            ...
            foreach ($aCategories as $iKey => $aCategory)
            {
                ...
                elseif ($this->_sDisplay == 'admincp')
                {
                    $sOutput .= '<li><img src="' . Phpfox::getLib('template')->getStyle('image', 'misc/draggable.png') . '" alt="" /> <input type="hidden" name="order[' . $aCategory['category_id'] . ']" value="' . $aCategory['ordering'] . '" class="js_mp_order" /> &nbsp;<a href="#?id=' . $aCategory['category_id'] . '" class="js_drop_down">' . ((Phpfox::isPhrase($aCategory['name'])) ? _p($aCategory['name']) : Phpfox::getLib('locale')->convert($aCategory['name'])) . $this->_get($aCategory['category_id'], $iActive) . '</a> </li>' . "\n";
                    // $sOutput .= '<li><img src="' . Phpfox::getLib('template')->getStyle('image', 'misc/drag.png') . '" alt="" /> <input type="hidden" name="order[' . $aCategory['category_id'] . ']" value="' . $aCategory['ordering'] . '" class="js_mp_order" /> &nbsp;<a href="#?id=' . $aCategory['category_id'] . '" class="js_drop_down"><img style="cursor:pointer;" src="' . Phpfox::getLib('template')->getStyle('image', 'misc/bullet_arrow_down.png') . '" alt="" border="0" /></a> ' . ((Phpfox::isPhrase($aCategory['name'])) ? _p($aCategory['name']) : Phpfox::getLib('locale')->convert($aCategory['name'])) . $this->_get($aCategory['category_id'], $iActive) . '</li>' . "\n";
                }
                else 
                {                
    ...
    private function _getParentsUrl($iParentId, $bPassName = false)
    {
        ...
        if ($iCnt === 1)
        {
            ...
            $sCacheId = $this->cache()->set('document_category_url' . ($bPassName ? '_name' : '') . '_' . $iParentId);
    ...
    {
        return $this->database()->select('COUNT(ecd.document_id)')
            ->from(Phpfox::getT('document_category_data'), 'ecd')
    ...
    {
        $aCategorie = $this->database()->select('name')
            ->from(Phpfox::getT('document_category'))
