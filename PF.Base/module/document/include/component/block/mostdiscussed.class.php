...
class Document_Component_Block_MostDiscussed extends Phpfox_Component
{
	/**
	...
	public function process()
	{
        ...
        $aRows = Phpfox::getService('document')->getMostDiscussed($sCondition, $limit);
		...
		$this->template()->assign(array(
				'sHeader' => _p('most_discussed_documents'),
        ...
        if ($count_rows >= $limit) {
            $this->template()->assign([
                'aFooter' => [
                    _p('view_all') => $this->url()->makeUrl($sLink, array('sort' => 'most-talked'))
                ]
            ]);
        }
    ...
    public function getSettings()
    {
        return [
            [
                'info' => _p('Most Discussed Documents Limit'),
                'description' => _p('Define the limit of how many most discussed documents can be displayed when viewing the blog section. Set 0 will hide this block.'),
                'value' => 4,
                'type' => 'integer',
                'var_name' => 'limit',
            ]
    ...
    public function getValidation()
    {
        return [
            'limit' => [
                'def' => 'int',
                'min' => 0,
                'title' => '"Most Discussed Documents Limit" must be greater than or equal to 0'
            ]
        ];
    }
	...
	{
		(($sPlugin = Phpfox_Plugin::get('document.component_block_mostdiscussed_clean')) ? eval($sPlugin) : false);
	}
}
