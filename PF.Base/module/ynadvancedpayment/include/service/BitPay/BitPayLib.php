...
class Ynadvancedpayment_Api_BitPay_BitPayLib
{
	

/**
 *
 * Writes $contents to a log file specified in the $this->bp_options file or, if missing,
 * defaults to a standard filename of '$this->bplog.txt'.
 *
      ...
      if($uname) {
        ...
        if($responseString == false) {
          ...
          if($bpOptions['useLogging'])
            $this->bpLog('Error: ' . curl_error($curl));
        } else {
          if(function_exists('json_decode'))
            $response = json_decode($responseString, true);
          else
            $response = $this->bpJSONdecode($responseString);

          if (!$response) {
            $response = array('error' => 'invalid json');
            if($bpOptions['useLogging'])
              $this->bpLog('Error - Invalid JSON.');
          }
        }
        ...
        curl_close($curl);
        if($bpOptions['useLogging'])
          $this->bpLog('Invalid data found in apiKey value passed to $this->bpCurl. (Failed: base64_encode(apikey))');
        return array('error' => 'Invalid data found in apiKey value passed to $this->bpCurl. (Failed: base64_encode(apikey))');
      }
    } catch (Exception $e) {
      @curl_close($curl);
      if($bpOptions['useLogging'])
        $this->bpLog('Error: ' . $e->getMessage());
      return array('error' => $e->getMessage());
    }
  } else {
    // Invalid parameter specified
    if($bpOptions['useLogging'])
      $this->bpLog('Error: You must supply non-empty url and apiKey parameters.');
...
/**
 *
 * Creates BitPay invoice via $this->bpCurl.
 *
 ...
 *
 */
function bpCreateInvoice($orderId, $price, $posData, $bpOptions, $options = array(),$apiUrl = 'https://bitpay.com/api/invoice/') {
  ...
  //
  // $price: by default, $price is expressed in the currency you set in $this->bp_options.php.  The currency can be
  // changed in $options.
  //
  ...
  //
  // If a given option is not provided here, the value of that option will default to what is found in $this->bp_options.php
  // (see api documentation for information on these options).


  try {
    $options = array_merge($bpOptions, $options);  // $options override any options found in $this->bp_options.php
    ...
    if(function_exists('json_encode'))
      ...
      $options['posData'] = $this->bpJSONencode($pos);
    ...
    */
    ...
    if(function_exists('json_encode'))
      ...
      $post = $this->bpJSONencode($post);

    $response = $this->bpCurl($apiUrl, $options['apiKey'], $post);
...
function bpVerifyNotification($apiKey = false) {
  ...
  try {
    ...
    if(function_exists('json_decode'))
      ...
      $posData = $this->bpJSONdecode($json['posData']);

    if($bpOptions['verifyPos'] and $posData['hash'] != $this->bpHash(serialize($posData['posData']), $apiKey))
 ...
 *
 */
function bpGetInvoice($invoiceId, $apiKey=false,$apiUrl = 'https://bitpay.com/api/invoice/') {
  global $bpOptions;
  ...
  try {
    ...
    $response = $this->bpCurl($apiUrl.$invoiceId, $apiKey);
    ...
    if(function_exists('json_decode'))
      ...
      $response['posData'] = $this->bpJSONdecode($response['posData']);
...
function bpDecodeResponse($response) {
  ...
  try {
    ...
    if(function_exists('json_decode'))
      ...
      return $this->bpJSONdecode($response);
...
function bpCurrencyList() {
  ...
  try {
    if(function_exists('json_decode'))
      ...
      $clist = $this->bpJSONdecode(file_get_contents($rate_url));
...
function bpJSONencode($data) {
  if(is_array($data)) {
    ...
    foreach($data as $key => $value) {
      ...
      if(is_array($value))
        $jsondata .= $this->bpJSONencode($value) . ', ';
