...
<form action="{url link='admincp.younetcore'}" method="post">
	<div class="error_message" style="font-size:11px;"> {_p var='younetcore.if_there_are_any_problems_related_younet_core_plugins_and_others_developed_by_younet_company_please'}.
	<input type="hidden" name="recheck" value="Check Problems" />
	<a href="javascript:void(0);" onclick = "$(this).parent().parent().submit();">{_p var='younetcore.check_problems'}</a>
	</div>
	
</form>	
<div id="container">
	<ul class="menu">
		<li id="yoursplugin" class="active title">{_p var='younetcore.yours_plugins'}</li>
		<li id="younetplugins" class="title">{_p var='younetcore.younet_plugins'}</li>
		<li id="license" class="title">{_p var='younetcore.license_terms'}</li>
	</ul>
	<div class="clear"></div>
