            ...
            {foreach from=$aCategories key=iKey item=aCategory}
                <tr class="checkRow{if is_int($iKey/2)} tr{else}{/if}">
                    ...
                    <td class="t_center">
                        ...
                        <div class="link_menu">
                            <ul>
                                <li><a href="{url link='admincp.ecommerce.category.add' id=$aCategory.category_id}">{phrase var='edit'}</a></li>
                                {if isset($aCategory.categories) && ($iTotalSub = count($aCategory.categories))}
                                <li><a href="{url link='admincp.ecommerce.category' sub={$aCategory.category_id}">{phrase var='manage_sub_categories_total' total=$iTotalSub}</a></li>
                                {/if}
                                ...
                                {else}
                                <li><a href="{url link='admincp.ecommerce.category' delete=$aCategory.category_id}" class="sJsConfirm" data-message="{phrase var='are_you_sure'}">{phrase var='delete'}</a></li>
                                {/if}
                            </ul>
                        </div>
                    </td>
                    <td>
                        {img server_id=$aCategory.server_id path='core.url_pic' file=$aCategory.image_path suffix='_16'}
                        &nbsp;
                    ...
                    <td class="t_center">
                        ...
                        <a href="#?call=ecommerce.updateActivity&amp;id={$aCategory.category_id}&amp;active=0&amp;sub={if $bSubCategory}1{else}0{/if}" class="js_item_active_link" title="{phrase var='deactivate'}"></a>
                        </div>
                        <div class="js_item_is_not_active" style="{if $aCategory.is_active}display:none;{/if}">
                        <a href="#?call=ecommerce.updateActivity&amp;id={$aCategory.category_id}&amp;active=1&amp;sub={if $bSubCategory}1{else}0{/if}" class="js_item_active_link" title="{phrase var='activate'}"></a>
                        </div>
                    </td>
                </tr>
            {/foreach}
