...
class Ecommerce_Service_Mail_Process extends Phpfox_Service
{
        ...
        public function addEmailTemplate($aVals)
        {
            ...
            $aRow = $this->database()->select('*')->from(Phpfox::getT('ecommerce_email_template_data'))->where('email_template_id = ' . $aVals['email_template_id'] .' AND language_id = "'.$aVals['language_id'].'"')->execute('getSlaveRow');
            ...
            if ($aRow)
            {
                    ...
                    $this->database()->update(Phpfox::getT('ecommerce_email_template_data'), $aUpdate, 'email_template_id = ' . $aVals['email_template_id'] .' AND language_id = "'.$aVals['language_id'].'"');
            } else
            {
                    $aInsert = array(
                            'language_id'           => $aVals['language_id'],                        
                    ...
                    $iId = $this->database()->insert(Phpfox::getT('ecommerce_email_template_data'), $aInsert);
            }
        }
        ...
        public function sendEmailTo($iTemplateType = 0, $iProductId = 0, $aReceivers = array(), $aCustomEmail = array())
		{
                ...
                $defaultLanguage = Phpfox::getService('ecommerce.helper')->getDefaultLanguage();

                if (!$aCustomEmail)
                {
                   $aCustomEmail = Phpfox::getService('ecommerce.mail')->getEmailMessageFromTemplate($iTemplateType , $defaultLanguage , $iProductId, $iReceiverId);
                }
        ...
        {
                $this->database()->insert(Phpfox::getT('ecommerce_email_queue'), $aVal);
        }
}
