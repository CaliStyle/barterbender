...
class Ecommerce_Service_Custom_Group extends Phpfox_service {
    ...
    {
        $this->_sTable = Phpfox::getT('ecommerce_custom_group');
    }
    ...
    public function getGroupForEdit($iGroupId)
    {
        ...
        $aMappingCategoriesSql = $this->database()->select('category_id')
                ->from(Phpfox::getT('ecommerce_category_customgroup_data'))
                ->where('group_id = ' . $iGroupId)
                ->execute('getSlaveRows');
        ...
        $aMappingCustomFields = Phpfox::getService('ecommerce.custom')->getCustomFieldByGroupId($iGroupId);
    ...
    public function addGroup($aGroup)
    {
        ...
        if (isset($aGroup['customfield']) && count($aGroup['customfield']) > 0)
        {
            ...
            {
                $this->database()->update(Phpfox::getT('ecommerce_custom_field'), array(
                    "group_id" => $iGroupId
                        ), 'field_id = ' . $iFieldId
                );
            }
        ...
        //remove old mapping
        $this->database()->delete(Phpfox::getT("ecommerce_category_customgroup_data"), 'group_id = ' . $iGroupId);
        ...
        if (isset($aGroup['categories']) && count($aGroup['categories']) > 0)
        {
            ...
            {
                $iGroupId = $this->database()->insert(Phpfox::getT("ecommerce_category_customgroup_data"), array(
    ...
    public function updateGroup($iGroupId, $aGroup)
    {
        ...
        if (isset($aGroup['categories']) && count($aGroup['categories']) > 0)
        {
            foreach ($aGroup['categories'] as $aCategory)
            {
                $this->database()->insert(Phpfox::getT("ecommerce_category_customgroup_data"), array(
                    "category_id" => $aCategory,
                    "group_id" => $iGroupId,
                ));
            }
    ...
    public function getForListing()
    {
        $aFields = $this->database()->select('cf.*')
                ->from(PhpFox::getT('ecommerce_custom_field'), 'cf')
        ...
        $aGroups = $this->database()->select('cg.*')
                ->from(Phpfox::getT('ecommerce_custom_group'), 'cg')
    ...
    public function deleteGroup($iGroupId)
    {
        ...
        $this->database()->update(Phpfox::getT('ecommerce_custom_field'), array('group_id' => 0), 'group_id = ' . $aGroup['group_id']);
