...
<div class="{if $sPrivacyFormType == 'mini'}privacy_setting_mini{else}privacy_setting{/if} privacy_setting_div">
    ...
    <a role="button" data-toggle="dropdown" class="privacy_setting_active{if $sPrivacyFormType == 'mini'} js_hover_title{/if} btn btn-md btn-default btn-icon btn-sm">
    ...
    <ul class="dropdown-menu dropdown-menu-checkmark">
        ...
        <li role="presentation">
            <a role="button" {if isset($aPrivacyControl.onclick)} onclick="{$aPrivacyControl.onclick} return false;"{else}data-toggle="privacy_item"{/if} rel="{$aPrivacyControl.value}" {if (isset($aPrivacyControl.is_active)) || (isset($bNoActive) && $bNoActive && $phpfox.iteration.privacycontrol == 1)}class="is_active_image"{/if}>{$aPrivacyControl.phrase}
            </a>
        </li>
        {/foreach}
    </ul>
