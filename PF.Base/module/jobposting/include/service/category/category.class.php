...
class Jobposting_Service_Category_Category extends Core_Service_Systems_Category_Category
{
	private $_sOutput = '';
	...
	{	
        ...
        $this->_sTableData = Phpfox::getT('jobposting_category_data');
	}
	...
	public function getForBrowse($iCategoryId = null)
	{
        ...
        {
            $aCategories[$iKey]['url'] = Phpfox::permalink('jobposting.category', $aCategory['category_id'], $aCategory['name']);
            ...
            //if ($sCategory === null)
            {
                ...
                {
                    $aCategories[$iKey]['sub'][$iSubKey]['url'] = Phpfox::permalink('jobposting.category', $aSubCategory['category_id'], $aSubCategory['name']);
                }
            }
        }
		return $aCategories;
	...
	{
		$sCacheId = $this->cache()->set('jobposting_category_display_' . $this->_sDisplay . '_' . Phpfox::getLib('locale')->getLangId());
	...
	{		
		$sCacheId = $this->cache()->set('jobposting_parent_breadcrumb_' . md5($sCategory));
	...
	public function getCategoriesById($iId = null, &$aCategories = null)
	{
		...
		if ($aCategories === null)
		{
			$aCategories = $this->database()->select('pc.parent_id, pc.category_id, pc.name')
				->from(Phpfox::getT('jobposting_category_data'), 'pcd')
				->join($this->_sTable, 'pc', 'pc.category_id = pcd.category_id')
				->where('pcd.company_id = ' . (int) $iId)
		...
		{			
			...
			{				
				$aBreadcrumb[] = array(((Phpfox::isPhrase($aCategory['name'])) ? _p($aCategory['name']) : Phpfox::getLib('locale')->convert($aCategory['name'])), Phpfox::permalink('jobposting.category', $aCategory['category_id'], $aCategory['name']));
			}
		}		
		else 
		{			
			$aBreadcrumb[] = array(((Phpfox::isPhrase($aCategories[0]['name'])) ? _p($aCategories[0]['name']) : Phpfox::getLib('locale')->convert($aCategories[0]['name'])), Phpfox::permalink('jobposting.category', $aCategories[0]['category_id'], $aCategories[0]['name']));
		}
	...
	{
		$sCacheId = $this->cache()->set('jobposting_category_childern_' . $sCategory);
	...
	{
		$sCacheId = $this->cache()->set('jobposting_category_parent_' . $sCategory);
	...
	private function _get($iParentId, $iActive = null,$type = 1)
	{
		...
		if (count($aCategories))
		{
			...
			else 
			{
				...
				$this->_sOutput .= '<option value="">' . ($iParentId === 0 ? _p('select') : _p('select_a_sub_industry')) . ':</option>' . "\n";
			}
			...
			foreach ($aCategories as $iKey => $aCategory)
			{
				...
				if ($this->_sDisplay == 'option')
				{
					if(!PHpfox::isAdmin() || ($aCategory['parent_id']==0 && PHpfox::isAdmin()))
					{
						if($type!=3)
						{
							$this->_sOutput .= '<option value="' . $aCategory['category_id'] . '" id="js_mp_category_item_' . $aCategory['category_id'] . '">' . ($this->_iCnt > 0 ? str_repeat('&nbsp;', ($this->_iCnt * 2)) . ' ' : '') . ((Phpfox::isPhrase($aCategory['name'])) ? _p($aCategory['name']) : Phpfox::getLib('locale')->convert($aCategory['name'])) . '</option>' . "\n";
						}	
						else
						{
							
							$this->_sOutput .= '<option value="' . $aCategory['category_id'] . '" id="popup_js_mp_category_item_' . $aCategory['category_id'] . '">' . ($this->_iCnt > 0 ? str_repeat('&nbsp;', ($this->_iCnt * 2)) . ' ' : '') . ((Phpfox::isPhrase($aCategory['name'])) ? _p($aCategory['name']) : Phpfox::getLib('locale')->convert($aCategory['name'])) . '</option>' . "\n";
						}	
				...
				elseif ($this->_sDisplay == 'admincp')
				{
					$sOutput .= '<li><img src="' . Phpfox::getLib('template')->getStyle('image', 'misc/draggable.png') . '" alt="" /> <input type="hidden" name="order[' . $aCategory['category_id'] . ']" value="' . $aCategory['ordering'] . '" class="js_mp_order" /><a href="#?id=' . $aCategory['category_id'] . '" class="js_drop_down">' . ((Phpfox::isPhrase($aCategory['name'])) ? _p($aCategory['name']) : Phpfox::getLib('locale')->convert($aCategory['name'])) . '</a>' . $this->_get($aCategory['category_id'], $iActive) . '</li>' . "\n";
				}
				else 
				{
					if($type!=3)
					{
						$this->_sOutput .= '<option value="' . $aCategory['category_id'] . '" id="js_mp_category_item_' . $aCategory['category_id'] . '">' . ((Phpfox::isPhrase($aCategory['name'])) ? _p($aCategory['name']) : Phpfox::getLib('locale')->convert($aCategory['name'])) . '</option>' . "\n";
					}				
					else {
						$this->_sOutput .= '<option value="' . $aCategory['category_id'] . '" id="popup_js_mp_category_item_' . $aCategory['category_id'] . '">' . ((Phpfox::isPhrase($aCategory['name'])) ? _p($aCategory['name']) : Phpfox::getLib('locale')->convert($aCategory['name'])) . '</option>' . "\n";
					}
				}
			}
	...
	private function _getParentsUrl($iParentId, $bPassName = false)
	{
		...
		if ($iCnt === 1)
		{
			...
			$sCacheId = $this->cache()->set('jobposting_category_url' . ($bPassName ? '_name' : '') . '_' . $iParentId);
   	...
   	private function _getForAdd($iNo, $iParentId, $iActive = null, $aCats = array())
	{
		...
		if (count($aCategories))
		{
			...
			$this->_sOutput .= '<div class="js_mp_parent_holder" id="js_mp_holder_' . $iNo . '_' . $iParentId . '" style="' . $sStyle . '">';
			$this->_sOutput .= '<select name="val[category][' . $iNo . '][' . $iParentId . ']" class="form-control js_mp_jobindustry_category_list" id="js_mp_id_' . $iNo . '_' . $iParentId . '">' . "\n";
			$this->_sOutput .= '<option value="">' . ($iParentId === 0 ? _p('select') : _p('select_a_sub_industry')) . ':</option>' . "\n";
			...
			foreach ($aCategories as $iKey => $aCategory)
			{
				...
				//if($type!=3)
					$this->_sOutput .= '<option value="' . $aCategory['category_id'] . '" id="js_mp_category_item_' . $iNo . '_' . $aCategory['category_id'] . '"' . $sSelect . '>' . ((Phpfox::isPhrase($aCategory['name'])) ? _p($aCategory['name']) : Phpfox::getLib('locale')->convert($aCategory['name'])) . '</option>' . "\n";
				//else
				//	$this->_sOutput .= '<option value="' . $aCategory['category_id'] . '" id="popup_js_mp_category_item_' . $iNo . '_' . $aCategory['category_id'] . '"' . $sSelect . '>' . ((Phpfox::isPhrase($aCategory['name'])) ? _p($aCategory['name']) : Phpfox::getLib('locale')->convert($aCategory['name'])) . '</option>' . "\n";
			}
	...
	public function getCategoryData($iId, $iLimit = 3)
	{
		...
		$aCats = $this->database()->select('no, category_id')->from($this->_sTableData)->where('company_id = '.$iId)->execute('getSlaveRows');
	...
	public function getPhraseCategory($company_id, $show_link = false){
		$str = "";
		$aCategory = Phpfox::getService('jobposting.category')->getCategoryData($company_id);

		foreach($aCategory as $key=>$Category){
			if(isset($Category[0]) && $Category[0]>0)
			{
				
				$aData1 = Phpfox::getService('jobposting.category')->getForEdit($Category[0]);
				
				if(isset($aData1['category_id']))
				{
                    $name = (Phpfox::isPhrase($aData1['name']) ? _p($aData1['name']) : Phpfox_Locale::instance()->convert($aData1['name']));
                    if ($show_link) {
                        $link = Phpfox::permalink('jobposting.company.category', $aData1['category_id'], $name);
                        $str .= sprintf('<a href="%1$s" title="%2$s">%2$s</a>', $link, $name);
                    } else {
                        $str .= $name;
                    }

					if(isset($Category[$Category[0]]) && $Category[$Category[0]]>0){
						$aData2 = Phpfox::getService('jobposting.category')->getForEdit($Category[$Category[0]]);
						
						if(isset($aData2['category_id'])){
						    $name_sub = Phpfox::isPhrase($aData2['name']) ? _p($aData2['name']) : Phpfox_Locale::instance()->convert($aData2['name']);
                            if ($show_link) {
                                $link_sub = Phpfox::permalink('jobposting.company.category', $aData2['category_id'], $name_sub);
                                $str .= " &#8250; " . sprintf('<a href="%1$s" title="%2$s">%2$s</a>', $link_sub, $name_sub);
                            } else {
                                $str .= " &#8250; " . $name_sub;
                            }
						}
