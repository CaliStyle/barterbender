        ...
        if(is_array($aPackages) && count($aPackages))
        {
            foreach($aPackages as $k => $aPackage)
            {
                $sHtml .= '<tr'.(($k%2 != 0) ? ' class="on"' : '').'>
                        ...
                        <td class="t_center">'.(($aPackage['post_number'] == 0) ? _p('unlimited') : $aPackage['remaining_post']).'</td>
    ...
    public function buildHtmlToBuyPackages($iCompanyId)
    {
        ...
        if(is_array($aPackages) && count($aPackages))
        {
            foreach($aPackages as $k => $aPackage)
            {
                ...
                $sHtml .= $aPackage['name'];
                $sHtml .= ' - $'.$aPackage['fee'];
                $sHtml .= ' - '.(($aPackage['post_number']==0) ? _p('unlimited') : _p('remaining').$aPackage['post_number'].' '._p('job_posts'));
