...
class JobPosting_Service_Custom_Process extends Phpfox_service
{
    ...
    public function add($aVals)
    {
        ...
        $aVals['type'] = isset($aVals['type']) ? $aVals['type'] : 2;
		
		if($aVals['type'] == 1)
		{
			$sVarName = 'company_' . $sVarName;
		}
		else{
			 $sVarName = 'job_' . $sVarName;
		}
		
		
		$iSameField = $this->database()->select('COUNT(*)')
            ->from($this->_sTable)
            ->where("field_name = '".$sFieldName. "' AND type = ". (int)$aVals['type'])
            ->execute('getField');
		
		if ($iSameField)
		{
			return Phpfox_Error::set(_p('custom.name_of_this_custom_field_is_already_in_use'));
		}
        ...
        $aSql = array(
            ...
            'type' => (isset($aVals['type']) ? (int)$aVals['type'] : null),
    ...
    public function addValue($aValues, $iApplicationId, $type = NULL)
    {
	...
	public function updateValue($aValues, $objectId, $type = NULL)
    {
		
        $aInsert = array('application_id' => $objectId);
		if(!is_null($type))
		{
			$aInsert['type'] = $type;
		}
        ...
        foreach($aValues as $iFieldId => $aValue)
        {
            ...
            if(is_array($aValue))
            {
            	...
            	$this->database()->delete($this->_sTableValue, 'field_id = ' . $aInsert['field_id'].' AND application_id ='.$objectId);
            ...
            else
            {
				...
				$fieldId = $this->database()->select('field_id')->from($this->_sTableValue)->where('field_id = '.$aInsert['field_id']. ' AND application_id ='.$objectId)->execute('getSlaveRow');
				if($fieldId)
				{
					$this->database()->update($this->_sTableValue, $aInsert, 'field_id ='.$aInsert['field_id']. ' AND application_id ='.$objectId);
				}
				else{
					 $this->database()->insert($this->_sTableValue, $aInsert);
				}
