 ...
 */
class Jobposting_Service_Catjob_Process extends Core_Service_Systems_Category_Process
{
    /**
     * Class constructor
     */
    ...
    public function __construct()
    {
        ...
        $this->_sModule = 'jobposting_job';
    }

    public function checkDelete($iId)
    {
        return $this->database()->select('count(*)')->from(Phpfox::getT('jobposting_job_category_data'))->where('category_id = ' . $iId)->execute('getField');
    }

    public function delete($iId)
    {
        if ($this->checkDelete($iId) > 0) {
            Phpfox_Error::set(_p('you_can_only_delete_empty_cat'));
            return false;
        }
        ...
        if (true /* Phpfox::getParam('jobposting.keep_[jobposting_after_category_delete') */) {
            ...
            $this->database()->delete(Phpfox::getT('jobposting_job_category_data'), 'category_id = ' . (int)$iId);
            $this->cache()->removeGroup('jobposting_job');
            return true;
        }
