    ...
    public function getFromCache($mAllowCustom = false, $sUserSearch = false, $bIncludeCurrentUser = false, $userId = null)
    {
        ...
        if ($aRows === false) {
            ...
            if ($sUserSearch != false) {
                $this->database()->join($this->_sTable, 'f',
                    'u.user_id = f.friend_user_id AND f.user_id=' . $userId);
            ...
            foreach ($aRows as $iKey => $aRow) {
                ...
                $aRows[$iKey]['full_name'] = html_entity_decode($pareOutput->split($pareOutput->clean($aRow['full_name']), 20), null, 'UTF-8');
        ...
        if ($bIncludeCurrentUser && !in_array($userId, array_column($aRows, 'user_id'))) {
            $aCurrentUser = db()->select(Phpfox::getUserField())
                ...
                ->execute('getSlaveRow');
            $aCurrentUser['full_name'] = html_entity_decode($pareOutput->split($pareOutput->clean($aCurrentUser['full_name']), 20), null, 'UTF-8');
            $aCurrentUser['user_profile'] = ($aCurrentUser['profile_page_id'] ? Phpfox::getService('pages')->getUrl($aCurrentUser['profile_page_id'],
                '', $aCurrentUser['user_name']) : Phpfox_Url::instance()->makeUrl($aCurrentUser['user_name']));
            $aCurrentUser['is_page'] = ($aCurrentUser['profile_page_id'] ? true : false);
            $aCurrentUser['user_image'] = Phpfox::getLib('image.helper')->display([
                'user' => $aCurrentUser,
                'suffix' => '_50_square',
                'max_height' => 32,
                'max_width' => 32,
            ...
            $aCurrentUser['user_image_actual'] = Phpfox::getLib('image.helper')->display([
                'user' => $aCurrentUser,
                'suffix' => '_50_square',
                'max_height' => 32,
                'max_width' => 32
            ]);
            $aCurrentUser['has_image'] = isset($aCurrentUser['user_image']) && $aCurrentUser['user_image'];
            $aRows[] = $aCurrentUser;
        }
