$Core.Gmap = {
  ...
  initGoogle: function (sCallback, oCallbackParams) {
    ...
    script.src = sAddr + "maps.google.com/maps/api/js?libraries=places&key=" + sGoogleKey + "&language=" + getParam('sLanguage') + "&callback=$Core.Gmap." + sCallback;
  ...
  initMapView: function (oParams) {
    try {
      ...
      if (href.query) {
        this.sView = href.query.replace(/.*[&|?]view=([^&]*).*/g, '$1');
      }
      ...
      $(document).on('mousedown', '.gm-ui-hover-effect', function () {
        var infoWD = $(this).siblings('.gm-style-iw-d').find('.js-info_window');
        if (infoWD.length) {
          var id = infoWD.data('item-id');
        }
      });
