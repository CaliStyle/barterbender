...
<?php echo Phpfox::getLib('phpfox.image.helper')->display(array('theme' => 'ajax/small.gif')); ?>
	...
	<textarea id="js_admincp_note" name="admincp_note" class="form-control" rows="8" onfocus="$('#js_share_user_status').show();" placeholder="<?php echo _p('save_your_notes_here'); ?>" data-value="<?php echo $this->_aVars['sAdminNote']; ?>"><?php echo $this->_aVars['sAdminNote']; ?></textarea>
	<div class="p_4 t_right" id="js_share_user_status" style="display:none;">
		<input type="button" value="<?php echo _p('save'); ?>" class="btn btn-primary" onclick="$('#js_share_user_status').hide(); $('#js_save_note').show(); $('#js_admincp_note').ajaxCall('core.admincp.updateNote'); return false;" />
		<input type="button" name="null" value="<?php echo _p('cancel'); ?>" onclick="$('#js_admincp_note').val($('#js_admincp_note').data('value'));$('#js_share_user_status').hide(); return false;" class="btn btn-default" />
	</div>	
</div>
