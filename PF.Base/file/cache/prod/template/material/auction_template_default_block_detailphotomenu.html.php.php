...
<?php endif; ?>
    ...
    <a class="ynauction-profile-avatar" href="<?php echo Phpfox::permalink('auction.detail', $this->_aVars['aAuction']['product_id'], $this->_aVars['aAuction']['name'], false, null, (array) array (
)); ?>" title="<?php echo Phpfox::getLib('phpfox.parse.output')->clean($this->_aVars['aAuction']['name']); ?>">
<?php echo Phpfox::getLib('phpfox.image.helper')->display(array('user' => $this->_aVars['aAuction'],'suffix' => '','max_width' => 150,'max_height' => 150,'class' => 'v_middle')); ?>
    </a>
</div>
<div class="ynauction-username-holder">
    <span><?php echo _p('by'); ?>: </span><a href="<?php echo Phpfox::getLib('phpfox.url')->makeUrl($this->_aVars['aAuction']['user_name']); ?>" title="<?php echo Phpfox::getLib('phpfox.parse.output')->clean($this->_aVars['aAuction']['full_name']); ?>"><?php echo Phpfox::getLib('phpfox.parse.output')->split(Phpfox::getLib('phpfox.parse.output')->shorten(Phpfox::getLib('phpfox.parse.output')->clean($this->_aVars['aAuction']['full_name']), 75, '...'), 10); ?></a>
</div>
<div class="ynauction-userinfo-holder">
<?php if ($this->_aVars['aAuction']['location'] != ''): ?>
    <div class="ynauction-location">
        <span class="ynauction-location-icon">

        </span><?php echo Phpfox::getLib('phpfox.parse.output')->clean($this->_aVars['aAuction']['location']); ?>
    </div>
<?php endif; ?>

    <div class="ynauction-bid">
        <span class="ynauction-bid-icon"></span>
        <a href="<?php echo Phpfox::getLib('phpfox.url')->makeUrl($this->_aVars['aAuction']['user_name']); ?>auction">
<?php if ($this->_aVars['aAuction']['total_auction'] > 1): ?>
<?php echo _p('number_auctions', array('number' => $this->_aVars['aAuction']['total_auction'])); ?>
<?php else: ?>
<?php echo _p('number_auction', array('number' => $this->_aVars['aAuction']['total_auction'])); ?>
<?php endif; ?>
        </a>
    </div>
</div>
