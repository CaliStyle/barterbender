...
<div class="layout_profile_completeness_profile_completeness">
    <ul class="profile_project_content">
        <li>
            <span class="percent"><?php echo $this->_aVars['iPercent']; ?>%</span> <?php echo _p('profilecompleteness.profile_completeness'); ?>
        </li>
        <li>
            <div class="profile_completed_box"
                 style="background-color: <?php if ($this->_aVars['colorbackground'] != ''):  echo $this->_aVars['colorbackground']; ?>;<?php else: ?>#EEEEEE;<?php endif; ?>">
                <div class="profile_completed" style="width: <?php echo $this->_aVars['iPercent']; ?>%;">
                </div>
            </div>
        </li>
<?php if ($this->_aVars['PercentTotal'] != 100): ?>
<?php if ($this->_aVars['iGroup_id'] == 'basic'): ?>
        <li>
<?php echo _p('profilecompleteness.next'); ?>: <a
                    href="<?php if ($this->_aVars['isPhoTo'] == 1):  echo Phpfox::getLib('phpfox.url')->makeUrl('user.profile');  else:  echo Phpfox::getLib('phpfox.url')->makeUrl('user.photo');  endif; ?>">+ <?php echo $this->_aVars['Key']; ?>
                (+<?php echo $this->_aVars['PercentValue']; ?>%)</a>
        </li>
<?php else: ?>
        <li>
<?php echo _p('profilecompleteness.next'); ?>: <a
                    href="<?php if ($this->_aVars['isPhoTo'] == 1):  echo Phpfox::getLib('phpfox.url')->makeUrl('user.profile', array('group' => $this->_aVars['iGroup_id']));  else:  echo Phpfox::getLib('phpfox.url')->makeUrl('user.photo');  endif; ?>">+
<?php echo $this->_aVars['Key']; ?> (+<?php echo $this->_aVars['PercentValue']; ?>%)</a>
        </li>
<?php endif; ?>
        <li class="text-center">
            <a class="btn btn-success btn-sm" href="<?php echo Phpfox::getLib('phpfox.url')->makeUrl('user.profile'); ?>"><?php echo _p('profilecompleteness.update_profile'); ?></a>
        </li>
