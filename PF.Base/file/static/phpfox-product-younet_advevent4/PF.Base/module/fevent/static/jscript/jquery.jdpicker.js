...
jdPicker.prototype = {
  build: function() {
    ...
    var monthNav = $('<p class="month_nav">' +
      '<span class="button prev"><i class="fa fa-angle-left"></i></span>' +
      ' <span class="month_name"></span> ' +
      '<span class="button next"><i class="fa fa-angle-right"></i></span>' +
      '</p>');
    ...
    var yearNav = $('<p class="year_nav">' +
      '<span class="button prev"><i class="fa fa-angle-double-left"></i></span>' +
      ' <span class="year_name" id="year_name"></span> ' +
      '<span class="button next"><i class="fa fa-angle-double-right"></i></span>' +
      '</p>');
		...
		if (!this.currentMonth || !(this.currentMonth.getFullYear() == newMonth.getFullYear() &&
		  ...
		  for (var i = 0; i <= numDays; i++) {
			...
			if ((this.select_week == 0 && currentDay.getMonth() == date.getMonth() && this.isNewDateAllowed(currentDay) && !this.isHoliday(currentDay)) || (this.select_week==1 && currentDay.getMonth() == date.getMonth() && this.isNewDateAllowed(firstDayOfWeek))) {
			  ...
			  if(typeof(POOL.search[this.dateToString(currentDay)])!='undefined'){
                ...
                if (currentFEDay.length > 1) {
                    if(typeof(currentFEDay[1].full_name) == 'undefined') {
                        if (currentFEDay[0].isrepeat == -1) {
                            $sClasscurrentFEDay0 = 'one-time';
                        } else {
                            $sClasscurrentFEDay0 = 'repeat';
                        }

                        if (currentFEDay[1].isrepeat == -1) {
                            $sClasscurrentFEDay1 = 'one-time';
                        } else {
                            $sClasscurrentFEDay1 = 'repeat';
                        }
                        tooltip = 'onmouseover="$(\'#tooltip\').html(\'<div class=' + $sClasscurrentFEDay0 + '>' + currentFEDay[0].from + ' - ' + currentFEDay[0].event_name + '</div><div class=' + $sClasscurrentFEDay1 + '>' + currentFEDay[1].from + ' - ' + currentFEDay[1].event_name + '</div>' + (sPhrase?'<div>' + sPhrase + '</div>':'') + '\');$(\'#tooltip\').show();" onmouseout="$(\'#tooltip\').hide();" onmousemove="var _x = event.pageX ? event.pageX : event.clientX; var _y = event.pageY ? event.pageY : event.clientY + $(document).scrollTop(); $(\'#tooltip\').css(\'left\', _x-120); $(\'#tooltip\').css(\'top\', _y+22);"';
                    }
                    else {
                        if (currentFEDay[0].isrepeat == -1) {
                            $sClasscurrentFEDay0 = 'one-time';
                        ...
                        $sClasscurrentFEDay1 = 'birthday';
                        tooltip = 'onmouseover="$(\'#tooltip\').html(\'<div class=' + $sClasscurrentFEDay0 + '>' + currentFEDay[0].from + ' - ' + currentFEDay[0].event_name + '</div><div class=' + $sClasscurrentFEDay1 +'>' + currentFEDay[1].full_name + '</div>' + (sPhrase?'<div>' + sPhrase + '</div>':'') + '\');$(\'#tooltip\').show();" onmouseout="$(\'#tooltip\').hide();" onmousemove="var _x = event.pageX ? event.pageX : event.clientX; var _y = event.pageY ? event.pageY : event.clientY + $(document).scrollTop(); $(\'#tooltip\').css(\'left\', _x-120); $(\'#tooltip\').css(\'top\', _y+22);"';
                    }
                }
                else {
                    if(typeof(currentFEDay[0].full_name)=='undefined') {
                        if (currentFEDay[0].isrepeat == -1) {
                            $sClasscurrentFEDay0 = 'one-time';
                        } else {
                            $sClasscurrentFEDay0 = 'repeat';
                        }
                        tooltip = 'onmouseover="$(\'#tooltip\').html(\'<div class=' + $sClasscurrentFEDay0 + '>' + currentFEDay[0].from + ' - ' + currentFEDay[0].event_name + '</div>' + (sPhrase?'<div>' + sPhrase + '</div>':'') + '\');$(\'#tooltip\').show();" onmouseout="$(\'#tooltip\').hide();" onmousemove="var _x = event.pageX ? event.pageX : event.clientX; var _y = event.pageY ? event.pageY : event.clientY + $(document).scrollTop(); $(\'#tooltip\').css(\'left\', _x-120); $(\'#tooltip\').css(\'top\', _y+22);"';
                    }
                    else {
                        $sClasscurrentFEDay0 = 'birthday';
                        tooltip = 'onmouseover="$(\'#tooltip\').html(\'<div class=' + $sClasscurrentFEDay0 + '>' + currentFEDay[0].full_name + '</div>' + (sPhrase?'<div>' + sPhrase + '</div>':'') + '\');$(\'#tooltip\').show();" onmouseout="$(\'#tooltip\').hide();" onmousemove="var _x = event.pageX ? event.pageX : event.clientX; var _y = event.pageY ? event.pageY : event.clientY + $(document).scrollTop(); $(\'#tooltip\').css(\'left\', _x-120); $(\'#tooltip\').css(\'top\', _y+22);"';
                    }
                }
			  }
			  dayCells += '<td '+ tooltip +' class="selectable_day'+(isAvailable ? ' available' : '') + (isToday ? ' today' : '') + '" date="' + this.dateToString(currentDay) + '"><span>' + currentDay.getDate() + '</span></td>';
			} else {
		  ...
		  if(this.select_week == 0){
			  $(".selectable_day", this.tbody).click(this.bindToObj(function(event) {
                ...
                url = url.split("?")[0];
                // console.log($(event.target), $(event.target).attr('date'));
                url += '?date='+$(event.target).parent().attr("date").replace(/\//g,'-')+'&';
