{if count($aUpcoming)>0}
	<div class="ync-mode-view-container fevent-block-upcoming-js">
        ...
        <span class="ync-mode-view-btn map" data-mode="map" data-callback-js="fevent.showMapView('upcoming');" title="{_p var='map_view'}"><i class="ico ico-map"></i></span>
    </div>
    <div class="item-container ynfevent-content-item-list ync-listing-container col-2 full-border ync-view-modes-js" data-mode-view="" data-mode-view-default="grid">
		{foreach from=$aUpcoming  name=event item=aItem}
			<article class="ync-item">{template file='fevent.block.event-item'}</article>
		{/foreach}
		<div id="fevent_upcoming_map" class="fevent-map" style="width: 100%; height:400px;"></div>
	</div>
	{if $iTotal>$iLimit}
		<div class="ync-viewmore">
			<a href="{url link='fevent' when='upcoming' view='all'}" class="item-viewmore">{_p var='core.view_more'}</a>
		</div>
	{/if}
{else}
