    ...
    {if Phpfox::isModule('invite') && Invite_Service_Invite::instance()->isInviteOnly()}
        <div class="sign-up-invitation">
            ...
            <form method="post" class="form" action="{url link='user.register'}">
                ...
                <div class="form-group">
                    <button type="submit" class="btn btn-primary">{_p var='create_account'}</button>
                </div>
            </form>
        </div>
        {else}
        ...
        <div id="js_registration_holder">
            <div class="custom-header-title">
                <div class="custom-title">{ _p var='create_an_account' }</div>
                <div class="custom-info">{ _p var='it_free_and_always_will_be' }</div>
            </div>
            
            <div class="register_container_scroll">
                <form method="post" class="form" action="{url link='user.register'}" id="js_form" enctype="multipart/form-data" data-step="1">
                    ...
                    <div class="form-button-group">
                        <div class="form-group">
                            {if isset($bIsPosted) || !Phpfox::getParam('user.multi_step_registration_form')}
                                <button type="submit" class="btn btn-success btn-lg" id="js_registration_submit">{_p var='create_account'}</button>
                            {else}
                                <input type="button" value="{_p var='create_account'}" class="btn btn-success text-uppercase" id="js_registration_submit" onclick="$Core.registration.submitForm();" />
                            {/if}
                        </div>
