...
class Ynchat_Service_Ynchat extends Phpfox_Service
{
	...
	public function addBan($aVals, &$aBanFilter = null)
	{
		...
		$aForm = array(
			...
			'find_value' => array(
				'type' => 'string:required',
				'message' => _p('ynchat.filter_value_is_required')
			),
			'reason' => array(
				'type' => 'string'
			),
		...
		if ($aBanFilter !== null && isset($aBanFilter['replace']))
		{
			$aForm['replacement'] = array(
				'type' => 'string:required',
				'message' => _p('ynchat.filter_replacement_is_required')
			);
		}
		...
		{
			return Phpfox_Error::set(_p('ynchat.you_cannot_ban_yourself'));
		}
		...
		{
			$aVals['reason'] = !Phpfox::isPhrase($aVals['reason']) ? Phpfox::getLib('parse.input')->clean($aVals['reason']) : $aVals['reason'];
    ...
    public function removeCache(){
        ...
        $this->cache()->remove('ynchat_getblocklistbysidemoreinfo_to_' . Phpfox::getUserId());
        $this->cache()->remove('ynchat_getallowlistbysidemoreinfo_to_' . Phpfox::getUserId());
        $this->cache()->remove('ynchat');
    }
    ...
    public function getLangAndConfig(){
        ...
        return array(
            'lang' => array(
                'your_browser_does_not_support_javascript' => html_entity_decode(_p('ynchat.your_browser_does_not_support_javascript')),
                'nothing_friend_s_found' => html_entity_decode(_p('ynchat.nothing_friend_s_found')),
                'connection_timed_out_please_try_again' => html_entity_decode(_p('ynchat.connection_timed_out_please_try_again')),
                'search' => html_entity_decode(_p('ynchat.search')),
                'too_big_image_file' => html_entity_decode(_p('ynchat.too_big_image_file')),
                'please_try_another_image' => html_entity_decode(_p('ynchat.please_try_another_image')),
                'upload' => html_entity_decode(_p('ynchat.upload')),
                'add' => html_entity_decode(_p('ynchat.add')),
                'view_old_conversation' => html_entity_decode(_p('ynchat.view_old_conversation')),
                'yesterday' => html_entity_decode(_p('ynchat.yesterday')),
                'days' => html_entity_decode(_p('ynchat.days')),
                'month' => html_entity_decode(_p('ynchat.month')),
                'show_message_from' => html_entity_decode(_p('ynchat.show_message_from')),
                'advanced_settings' => html_entity_decode(_p('ynchat.advanced_settings')),
                'close_all_chat_tabs' => html_entity_decode(_p('ynchat.close_all_chat_tabs')),
                'go_offline' => html_entity_decode(_p('ynchat.go_offline')),
                'go_online' => html_entity_decode(_p('ynchat.go_online')),
                'play_sound_on_new_message' => html_entity_decode(_p('ynchat.play_sound_on_new_message')),
                'yes' => html_entity_decode(_p('ynchat.yes')),
                'no' => html_entity_decode(_p('ynchat.no')),
                'enter_name' => html_entity_decode(_p('ynchat.enter_name')),
                'searching' => html_entity_decode(_p('ynchat.searching')),
                'turn_on_chat_except' => html_entity_decode(_p('ynchat.turn_on_chat_except')),
                'turn_on_chat_for_only_some_friends' => html_entity_decode(_p('ynchat.turn_on_chat_for_only_some_friends')),
                'save' => html_entity_decode(_p('ynchat.save')),
                'chat' => html_entity_decode(_p('ynchat.chat')),
                'mobile' => html_entity_decode(_p('ynchat.mobile')),
                'web' => html_entity_decode(_p('ynchat.web')),
                'message_history' => html_entity_decode(_p('ynchat.message_history')),
                'video' => html_entity_decode(_p('ynchat.video')),
                'photo' => html_entity_decode(_p('ynchat.photo')),
                'add_files' => html_entity_decode(_p('ynchat.add_files')),
                'upload_photo' => html_entity_decode(_p('ynchat.upload_photo')),
                'choose_a_sticker_or_emoticon' => html_entity_decode(_p('ynchat.choose_a_sticker_or_emoticon')),
                'please_try_again_make_sure_you_are_uploading_a_valid_photo' => html_entity_decode(_p('ynchat.please_try_again_make_sure_you_are_uploading_a_valid_photo')),
                'unable_to_connect_to_chat_check_your_internet_connection' => html_entity_decode(_p('ynchat.unable_to_connect_to_chat_check_your_internet_connection')),
            ),
            'config' => array(
    ...
    public function getMessageByFriendId($aData){
        if(Phpfox::isUser() == false){
            return array(
                'result' => FALSE,
                'error_message' => _p('ynchat.please_login_and_try_again'),
                'error_code' => 1
            );
        }
            ...
            if ($iNotCompleted === 0)
            {
                ...
                if ($sAction == 'upload_photo_via_share')
                {
                    ...
                    else
                    {
                        ...
                        if (!isset($aFeeds[0]))
                        {
                            Phpfox::addMessage(_p('feed.this_item_has_successfully_been_submitted'));
                        }
                    }

                    Phpfox::addMessage(_p('photo.photo_successfully_uploaded'));
                }
                else
                {
                    ...
                    {
                        Phpfox::addMessage((count($aImages) == 1 ? _p('photo.photo_successfully_uploaded') : _p('photo.photos_successfully_uploaded')));
                    }
    ...
    public function attachVideo($aData){
        if(!$this->isEnableVideoAction()){
            return array(
                'result' => FALSE,
                'error_message' => _p('ynchat.cannot_attach_video_link'),
                'error_code' => 1
            );
        }
        if(Phpfox::isUser() == false){
        ...
        if(empty($sUrl)){
            return array(
                'result' => FALSE,
                'error_message' => _p('ynchat.please_input_valid_link'),
                'error_code' => 1
            );
        }
    ...
    public function attachLink($aData){
        if(!$this->isEnableLinkAction()){
            return array(
                'result' => FALSE,
                'error_message' => _p('ynchat.cannot_attach_link'),
                'error_code' => 1
            );
        }
        if(Phpfox::isUser() == false){
    ...
    public function upload($params) {
        ...
        if (!$sender || !$receiver) {
            return array(
                'error_message' => _p('ynchat.invalid_receiver'),
                'error_code' => 1
            );
        }
