...
class Ynchat_Component_Controller_Admincp_Banword extends Phpfox_Component
{
	...
	{
		$aBanFilter = array(
				'title' => _p('ynchat.words'),
				'type' => 'word',
				'url' => 'admincp.ynchat.banword',
				'form' => _p('ynchat.words'),
				'replace' => true
        );
		...
		if (($iDeleteId = $this->request()->getInt('delete')))
		{
			...
			{
				$this->url()->send($aBanFilter['url'], null, _p('ynchat.filter_successfully_deleted'));
			}
		}
		...
		if (($this->request()->get('add_ban')))
		{
			...
			{
				$this->url()->send($aBanFilter['url'], null, _p('ynchat.filter_successfully_added'));
			}
		}
		...
		$this->template()->setTitle(_p('ynchat.words'))
			->setBreadcrumb(_p('ynchat.words'))
			->setBreadcrumb(_p('ynchat.words'), null, true)
			->assign(array(
					'aFilters' => $aFilters,
