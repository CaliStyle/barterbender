var $Material = {
  ...
  updateMainNav: function() {
    ...
    $('.js-btn-collapse-main-nav-selected').each(function() {
  ...
  setColumnFixed: function(left_right, elem){
    ...
    checkFixedColumn = function() {
      if ($Core.exists('body#page_yncphotoworld_landing') || $('body').css('position') == 'fixed') {
        if ($Core.exists('body#page_yncphotoworld_landing')) {
      ...
      var offset_footer = 0;
      var height_footer = 0;
      var bottom_elems = ['#section-footer', '#bottom_placeholder', '._block.location_8'];
      ...
      if(elem_height < max_height && !$('#main').hasClass('empty-' + left_right)) {
        ...
        if (elem_total >= offset_footer) {
          ...
          if (elem_height < wd_height - top_space - bottom) {
            ...
            if(($('._is_profile_view  .profiles-menu').length && $('._is_profile_view  .profiles-menu').css('position') !== 'fixed') || ($('#section-header .sticky-bar').length && $('#section-header .sticky-bar').css('position') !== 'fixed' && $('#js_block_border_core_menusub').length)) {
    ...
    $(window).off('scroll', checkFixedColumn).on('scroll', checkFixedColumn);
  }
};
...
$Ready(function() {
  ...
  $('.nav-mask-modal,.yncphotoworld-main-menu-toggle .btn-close').on("click touchend", function(){
    $(this).removeClass("in");
  ...
  $(".site-menu-small .ajax_link, .site-logo-link, .yncphotoworld-main-menu-toggle .btn-close").on("click", function(){
  ...
  $(document).mouseup(function(e) {
    ...
    {
        $('.sticky-bar-inner.overlap').removeClass('overlap');
        $( "#search-panel input" ).fadeIn( "slow" );
    }
  });
  ...
  if(!(/Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent) )){
    ...
    //Init scrollbar
    $(".yncphotoworld-main-menu-toggle .site-menu-small,.fixed-main-navigation .dropdown-menu, .user-sticky-bar .panel-items, .friend-search-invite-container, #js_main_mail_thread_holder .mail_messages, .js_box_content .item-membership-container, #welcome_message .custom_flavor_content, .dropdown-menu-limit, .attachment-form-holder .js_attachment_list").mCustomScrollbar({
      theme: "minimal-dark",
    }).addClass('dont-unbind-children');

    $(".yncphotoworld-main-nav-dropdown .dropdown-menu").mCustomScrollbar({
      theme: "minimal-dark",
  ...
  if ($('.js-btn-collapse-main-nav-selected:not(.built)').length > 0) {
    $Material.updateMainNav();
    $('.js-btn-collapse-main-nav-selected').addClass('built');
  }
