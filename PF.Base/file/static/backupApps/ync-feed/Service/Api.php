 ...
 */
class Api extends Feed_Service_Api
{
    /**
    ...
    public function delete($params)
    {
        ...
        $result = Phpfox::getService('ynfeed.process')->deleteFeed($iFeedId);
        if (!$result)
        {
    ...
    {
        $iFeedId = $params['id'];
        $aFeed = Phpfox::getService('ynfeed')->getUserStatusFeed([], $iFeedId);
        if (empty($aFeed) || empty($aFeed['feed_id']))
        {
            return $this->error(_p('The {{ item }} cannot be found.', ['item' => _p('feed__l')]));
        }
    ...
    public function gets()
    {
        ...
        $aRows = Phpfox::getService('ynfeed')->callback($aCallback)->get($aParams, null);
    ...
    public function post()
    {
        ...
        if ($iId = Phpfox::getService('ynfeed.process')->callback($aCallback)->addComment($aVals))
        {
            ...
            Phpfox::getService('ynfeed')->callback($aCallback);
        ...
        if ($aVals['type'] == '2')
        {
            ...
            else
            {
                ...
                foreach ($aVals['friends'] as $iFriendId)
                {
                    ...
                    {
                        $ids[] = Phpfox::getService('ynfeed.process')->addComment($aShare);
                    }
                    else
                    {
            ...
            $aFeeds = Phpfox::getService('ynfeed')->get();
            $results = [];
