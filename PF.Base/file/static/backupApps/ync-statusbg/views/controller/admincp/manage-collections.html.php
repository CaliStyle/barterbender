...
<form method="post" id="manage_reactions" action="{url link='admincp.yncstatusbg.manage-collections'}">
    <div class="panel panel-default">
        <div class="panel-heading">
            <div class="panel-title" style="position: relative">
                <a href="{url link='admincp.app' id='YNC_StatusBg'}">{_p var='manage_collections'}</a>
            </div>
        </div>
        <div class="table-responsive">
            <table class="table table-bordered">
                ...
                <tr>
                    ...
                    <th class="t_center w80">{_p var='active__u'}</th>
                </tr>
                </thead>
                <tbody>
                    ...
                    <tr class="tr">
                        ...
                        <td class="t_center w60">
                            ...
                            <div class="link_menu">
                                <ul>
                                    <li><a href="{url link='admincp.yncstatusbg.add-collection' id=$aItem.collection_id}">{_p var='edit'}</a></li>
                                    {if !$aItem.is_default}
                                    <li><a href="{url link='admincp.yncstatusbg.manage-collections' default=$aItem.collection_id}" class="sJsConfirm" data-message="{_p var='are_you_sure_if_set_as_default_this_collection_is_activated_and_the_current_default_one_will_be_set_inactive'}">{_p var='set_as_default'}</a></li>
                                    {/if}
                                    {if $aItem.view_id != 1 && $aItem.is_default == 0}
                                        <li><a href="{url link='admincp.yncstatusbg.manage-collections' delete=$aItem.collection_id}" class="sJsConfirm" data-message="{_p var='are_you_sure_you_want_to_delete_this_collection_permanently'}">{_p var='delete'}</a></li>
                                    {/if}
                                </ul>
                            </div>
                        </td>
                        ...
                        <td class="w160 t_center">
                            {if !empty($aItem.full_path)}
                                <span class="ync-statusbg-main-image" style="background-image: url('{$aItem.full_path}')"></span>
                            {/if}
                        </td>
                        ...
                        <td class="t_center w140" {if $aItem.is_default}style="opacity:0.5; pointer-events: none;"{/if}>
                            <div class="js_item_is_active"{if !$aItem.is_active} style="display:none;"{/if}>
                                <a href="#?call=yncstatusbg.toggleActiveCollection&amp;id={$aItem.collection_id}&amp;active=0" class="js_item_active_link" title="{_p var='Deactivate'}"></a>
                            </div>
                            <div class="js_item_is_not_active"{if $aItem.is_active} style="display:none;"{/if}>
                                <a href="#?call=yncstatusbg.toggleActiveCollection&amp;id={$aItem.collection_id}&amp;active=1" class="js_item_active_link" title="{_p var='Activate'}"></a>
                            </div>
                        </td>
                    </tr>
                    {/foreach}
...
{literal}
<style>
 #manage_reactions .ync-statusbg-main-image{
    width: 100%;
