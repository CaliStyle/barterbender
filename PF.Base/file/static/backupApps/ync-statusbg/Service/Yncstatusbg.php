...
class Yncstatusbg extends Phpfox_Service
{
    ...
    public function getForManage($iLimit = 5, $iPage = 1, &$iCnt)
    {
        ...
        if ($iCnt) {
            $aItems = db()->select('c.*, b.image_path, b.server_id')
                ...
                ->order('c.is_default DESC, c.is_active DESC')
    ...
    public function getBackgroundImage(&$aBackground, $sSuffix = 300, $sName = 'full_path')
    {
        if (!empty($aBackground['image_path'])) {
            if ($aBackground['view_id'] > 0) {
                $aBackground[$sName] = Phpfox::getParam('core.path_actual') . 'PF.Site/Apps/ync-statusbg/assets/images/default-collection/' . ($sSuffix == 48 ? str_replace('-min','-sm',$aBackground['image_path']) : $aBackground['image_path']);
            } else {
    ...
    public function getUploadParams($aParams = null)
    {
        ...
        $aEvents = [
            'sending' => 'yncstatusbg_admin.dropzoneOnSending',
            'success' => 'yncstatusbg_admin.dropzoneOnSuccess',
            'queuecomplete' => 'yncstatusbg_admin.dropzoneQueueComplete',
            'removedfile' => 'yncstatusbg_admin.dropzoneOnRemoveFile',
            'error' => 'yncstatusbg_admin.dropzoneOnError',
            'init' => 'yncstatusbg_admin.dropzoneOnInit'
        ];
        return [
            'max_size' => ($iMaxFileSize === 0 ? null : $iMaxFileSize),
            'upload_url' => Phpfox::getLib('url')->makeUrl('admincp.yncstatusbg.frame-upload'),
            'component_only' => true,
            ...
            'upload_dir' => Phpfox::getParam('core.dir_pic') . 'yncstatusbg/',
            'upload_path' => Phpfox::getParam('core.url_pic') . 'yncstatusbg/',
            'update_space' => true,
            'no_square' => true,
            'type_list' => ['jpg', 'png'],
            'style' => '',
            ...
            'thumbnail_sizes' => Phpfox::getParam('yncstatusbg.thumbnail_sizes'),
            'max_size_description' => _p('the_file_size_limit_is_file_size_if_your_upload_does_not_work_try_uploading_a_smaller_image',['file_size' => Phpfox_File::instance()->filesize($iMaxFileSize * 1048576)])
        ];
    }
    ...
    public function getCollectionsList($bActive = true, $bGetBg = true)
    {
        ...
        $sCacheId = $this->cache()->set('yncstatusbg_collection_listing_' . md5($sCond . '_' . $bGetBg));
        $this->cache()->group('yncstatusbg', $sCacheId);
    ...
    public function getFeedStatusBackground($iItemId, $sType, $iUserId, $bNoImage = false)
    {
        if (!$iItemId || !$sType) {
            return false;
        }
        $aBackground = db()->select('b.*')
