...
class FrameUploadController extends Phpfox_Component
{
    ...
    public function process()
    {
        ...
        $aParams = Phpfox::getService('yncstatusbg')->getUploadParams();
        $aParams['type'] = 'yncstatusbg';
        ...
        $iImageId = db()->insert(Phpfox::getT('yncstatusbg_backgrounds'), array(
            'time_stamp' => $sTimeStamp,
            'collection_id' => $iId,
            'image_path' => 'yncstatusbg/' . $aFile['name'],
            'server_id' => Phpfox_Request::instance()->getServer('PHPFOX_SERVER_ID'),
            'ordering' => 99
        ));
        if ($iId && $iImageId) {
            $aCollection = Phpfox::getService('yncstatusbg')->getForEdit($iId);
            db()->updateCounter('yncstatusbg_collections', 'total_background', 'collection_id', $iId);
            if (empty($aCollection['main_image_id'])) {
                db()->update(':yncstatusbg_collections', ['main_image_id' => $iImageId],
