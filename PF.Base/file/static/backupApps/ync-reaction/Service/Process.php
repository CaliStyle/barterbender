...
class Process extends Phpfox_Service
{
    ...
    public function toggleActiveReaction($iId, $iActive)
    {
        ...
        $iActive = (int)$iActive;
        if ($iActive == 1 && Phpfox::getService('yncreaction')->countReactions(true) >= 6) {
        ...
        $this->cache()->removeGroup('yncreaction');
        return true;
    }
    ...
    {
        Phpfox::isAdmin(true);
        $aReaction = Phpfox::getService('yncreaction')->getReactionById($iId);
    ...
    public function add($aVals, $sName = 'title', $bIsEdit = false)
    {
        ...
        $sPicStorage = Phpfox::getParam('core.dir_pic') . 'yncreaction/';
        $sIconName = '';
        if ($bIsEdit) {
            $aReaction = Phpfox::getService('yncreaction')->getForEdit($aVals['id']);
            if (!$aReaction) {
        ...
        if ($bIsEdit) {
            $iId = $aVals['id'];
            if (!empty($sIconName)) {
                $aInsert['icon_path'] = 'yncreaction/' . $sIconName;
            ...
            if (Phpfox::getService('yncreaction')->countReactions(true) >= 6) {
                $aInsert['is_active'] = 0;
            }
            $aExtra = [
                'view_id' => 0,
                'server_id' => Phpfox::getLib('request')->getServer('PHPFOX_SERVER_ID'),
                'icon_path' => 'yncreaction/' . $sIconName,
            ];
    ...
    protected function addPhrase($aVals, $sName = 'name', $bVerify = true)
    {
        ...
        $phrase_var_name = 'yncreaction_react_title_' . md5('yncreaction_react_title' . $name . PHPFOX_TIME);
