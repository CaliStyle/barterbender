...
class EventList extends Phpfox_Component
{
    public function process()
    {
        ...
        switch ($sDataSource) {
            case 'ongoing':
                list($iTotal, $events) = Phpfox::getService('fevent')->getOnHomepageByType('ongoing', $iLimit, $bIsPage, false, false, $pageID);
                $aFooter = array(_p('view_more') => $this->url()->makeUrl('fevent', array('when' => 'ongoing', 'view' => 'all')));
                $hideStatus = true;
                break;
            case 'upcoming':
                list($iTotal, $events) = Phpfox::getService('fevent')->getOnHomepageByType('upcoming', $iLimit, $bIsPage, false, false, $pageID);
            ...
            case 'invited':
                ...
                break;
            case 'most-liked':
                list($iTotal, $events) = Phpfox::getService('fevent')->getTopEvent('liked', $iLimit, $bIsPage, false, false, $pageID);
                $aFooter = array(_p('view_more') => $this->url()->makeUrl('fevent', array('sort' => 'most-liked')));
                break;
            case 'most-viewed':
                list($iTotal, $events) = Phpfox::getService('fevent')->getTopEvent('viewed', $iLimit, $bIsPage, false, false, $pageID);
                $aFooter = array(_p('view_more') => $this->url()->makeUrl('fevent', array('sort' => 'most-viewed')));
                break;
            case 'most-discussed':
                list($iTotal, $events) = Phpfox::getService('fevent')->getTopEvent('discussed', $iLimit, $bIsPage, false, false, $pageID);
