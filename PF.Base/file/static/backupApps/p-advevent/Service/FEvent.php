...
class FEvent extends Phpfox_Service
{
    ...
    public function checkBlockExistOutOfApp($controller, $component = 'event-list') {
        $check = db()->select('block_id')
                    ...
                    ->where('m_connection = "'. $controller .'" AND component = "'. $component .'" AND is_active = 1')
    ...
    {
        $row = db()->select('is_featured, is_sponsor, view_id')
            ->from($this->_sTable)
    ...
    public function getBirthdayBackgroundImage()
    {
        ...
        if (($rowParsed = $this->cache()->get($cacheId)) === false) {
            $row = db()->select('name, default_value')
                ->from(Phpfox::getT('fevent_setting'))
                ->where('name = "fevent_birthday_photo_image_path" OR name = "fevent_birthday_photo_server_id"')
    ...
    public function checkSentWishes($userIds = [])
    {
        ...
        $rows = db()->select('u.user_id, fbw.birthday_wish_id AS is_send_wish, fbw.message' . $extraSelect)
            ->from(Phpfox::getT('user'), 'u')
            ->leftJoin(Phpfox::getT('fevent_birthday_wish'), 'fbw', 'fbw.target_user_id = u.user_id AND fbw.user_id = ' . Phpfox::getUserId() . ' AND (fbw.time_stamp BETWEEN ' . $startTime . ' AND ' . $endTime . ')')
            ->where('u.user_id IN (' . implode(',', $userIds) . ')')
            ->execute('getSlaveRows');
    ...
    public function getBirthdaysInCurrentYear($iUser)
    {
        ...
        if (false === ($aBirthdays = $this->cache()->get($sCacheId, 5 * 60 * 60))) // cache is in 5 hours
        {
            ...
            $aBirthdays = $this->database()->select(Phpfox::getUserField() . ', u.custom_gender, uf.dob_setting, fb.birthday_user_receiver')
    ...
    public function getBlockData($type, $isPage = false, $isProfile, $limit = 3, $params = [])
    {
            ...
            case 'suggest':
                {
                    ...
                    $events = db()->select('e.*, fi.rsvp_id, fi.user_id AS inviter_id, fi.invited_user_id AS invitee_id, ft.description_parsed, ' . Phpfox::getUserField())
                        ->join($this->_sTable, 'e', 'e.event_id = fevent.event_id')
                        ->join($userTalble, 'u', 'u.user_id = e.user_id')
                        ->join($textTable, 'ft', 'ft.event_id = fevent.event_id')
                        ->leftJoin($inviteTable, 'fi', 'fi.event_id = fevent.event_id AND fi.invited_user_id = ' . Phpfox::getUserId())
                        ->limit($limit)
                        ->execute('getSlaveRows');
                    break;
                }
            case 'popular':
                {
                    ...
                    db()->select('COUNT(*) AS total_friend, 0 AS total_mutual_friend, 0 AS total_people, fi.event_id')
                        ->from($inviteTable, 'fi')
                        ->join($friendTable, 'f', 'f.friend_user_id = fi.invited_user_id')
                        ->where('fi.rsvp_id = 1 AND f.is_page = 0 AND f.user_id = ' . Phpfox::getUserId())
                        ->group('fi.event_id')
                        ->union();

                    $whereInFriendList = ':field :in_condition (SELECT friend_user_id from :friend WHERE is_page=0 AND user_id=:user_id)';
                    db()->select('0 AS total_friend , COUNT(*) AS total_mutual_friend, 0 AS total_people, fi.event_id')
                        ->from($inviteTable, 'fi')
                        ->join($friendTable, 'f', 'f.user_id = fi.invited_user_id')
                        ->join($friendTable, 'sf', 'sf.friend_user_id = f.friend_user_id AND sf.user_id = ' . Phpfox::getUserId())
                        ->where('fi.rsvp_id = 1 AND (f.user_id != ' . Phpfox::getUserId() . ' AND ' . strtr($whereInFriendList, [
                                ':friend' => $friendTable,
                                ':user_id' => Phpfox::getUserId(),
                                ':field' => 'f.user_id',
                                ':in_condition' => 'NOT IN'
                            ]) . ') AND ' . strtr($whereInFriendList, [
                                ':friend' => $friendTable,
                                ':user_id' => Phpfox::getUserId(),
                                ':field' => 'f.friend_user_id',
                                ':in_condition' => 'IN'
                            ]))
                        ->group('f.friend_user_id')
                        ->union();

                    db()->select('0 AS total_friend , 0 AS total_mutual_friend, COUNT(*) AS total_people, fi.event_id')
                        ->from($inviteTable, 'fi')
                        ->where('fi.rsvp_id = 1')
                        ->group('fi.event_id')
                        ->union();
                    ...
                    $events = db()->select('SUM(fevent.total_friend) AS total_friend, SUM(total_mutual_friend) AS total_mutual_friend, SUM(total_people) AS total_people, e.*, fi.rsvp_id, fi.user_id AS inviter_id, fi.invited_user_id AS invitee_id, ft.description_parsed, ' . Phpfox::getUserField())
                        ->join($eventTable, 'e', 'e.event_id = fevent.event_id AND e.view_id = 0')
                        ->join($userTalble, 'u', 'u.user_id = e.user_id')
                        ->join($textTable, 'ft', 'ft.event_id = fevent.event_id')
                        ->leftJoin(Phpfox::getT('fevent_invite'), 'fi', 'fi.event_id = fevent.event_id AND fi.invited_user_id = ' . Phpfox::getUserId())
                        ->group('fevent.event_id')
                        ->order('total_people DESC, total_friend DESC, total_mutual_friend DESC, e.start_time ASC, e.end_time ASC')
                        ->limit($limit)
                        ->execute('getSlaveRows');
                    break;
                }
            case 'related':
                {
                    ...
                    db()->select('fcd.category_id')
                        ->from($eventTable, 'f')
                        ->join(Phpfox::getT('fevent_category_data'), 'fcd', 'fcd.event_id = f.event_id')
                        ->where('f.event_id = ' . (int)$params['event_id'] . ' AND f.view_id = 0')
                        ->union()
                        ->unionFrom('fevent');

                    $events = db()->select('f.*, fi.rsvp_id, fi.user_id AS inviter_id, fi.invited_user_id AS invitee_id, ft.description_parsed, ' . Phpfox::getUserField())
                        ->join(Phpfox::getT('fevent_category'), 'fc', '(fevent.category_id IS NOT NULL OR fevent.category_id != "") AND fevent.category_id = fc.category_id')
                        ->join(Phpfox::getT('fevent_category_data'), 'fcd', 'fcd.category_id = fc.category_id')
                        ->join($eventTable, 'f', 'f.event_id = fcd.event_id AND f.view_id = 0 AND f.event_id != ' . $params['event_id'])
                        ->join($userTalble, 'u', 'u.user_id = f.user_id')
                        ->join($textTable, 'ft', 'ft.event_id = f.event_id')
                        ->leftJoin(Phpfox::getT('fevent_invite'), 'fi', 'fi.event_id = f.event_id AND fi.invited_user_id = ' . Phpfox::getUserId())
                        ->limit($limit)
                        ->order('f.start_time ASC, f.end_time ASC')
                        ->execute('getSlaveRows');
                    break;
                }
            case 'reminder':
                {
                    ...
                    $events = db()->select('f.*, fi.rsvp_id, fi.user_id AS inviter_id, fi.invited_user_id AS invitee_id, ft.description_parsed, ' . Phpfox::getUserField())
                        ->from($eventTable, 'f')
                        ->join($inviteTable, 'fi', 'fi.event_id = f.event_id AND fi.rsvp_id IN (' . implode(',', $rsvpReminder) . ')')
                        ->join($userTalble, 'u', 'u.user_id = fi.invited_user_id')
                        ->join($textTable, 'ft', 'ft.event_id = f.event_id')
                        ->where('f.start_time > ' . PHPFOX_TIME . ' AND f.view_id = 0 AND fi.invited_user_id = ' . Phpfox::getUserId())
                        ->order('fi.rsvp_id ASC, f.start_time ASC')
                        ->limit($limit)
                        ->execute('getSlaveRows');
                    break;
                }
        ...
        $events = $this->checkPrivacy($events, $isPage, $isProfile);
        ...
        $storedEvents[$type] = [
            'total' => (!empty($events) ? count($events) : 0),
            'events' => $events
        ];
    ...
    public function getMoreInfoForEventItem(&$event, $getSimpleCount = false)
    {
        ...
        db()->select('COUNT(*) AS total_attending, 0 AS total_friend')
            ->from($mainTable)
            ->where('event_id = ' . (int)$event['event_id'] . ' AND rsvp_id IN (1,2)')
            ->union();
        db()->select('0 AS total_attending, COUNT(*) AS total_friend')
            ->from($mainTable, 'fi')
            ->join($friendTable, 'f', 'fi.invited_user_id = f.friend_user_id AND f.user_id = ' . Phpfox::getUserId())
            ->where('fi.event_id = ' . (int)$event['event_id'] . ' AND fi.rsvp_id IN (1,2)')
            ->union()
            ->unionFrom('t', true);
        ...
        if ($getSimpleCount) {
            $event['attending_statistic'] = [
                'total_attending' => $totalCount,
                'total_friend_attending' => (int)$count['total_friend'],
                'total_other_people_attending' => (int)$totalCount - (int)$count['total_friend'],
            ];
            return true;
        }
        ...
        if ($totalCount > 0) {
            ...
            db()->select('fi.invited_user_id AS user_id')
                ->from($mainTable, 'fi')
                ->join($friendTable, 'f', 'f.friend_user_id = fi.invited_user_id')
                ->where('fi.event_id = ' . $event['event_id'] . ' AND fi.rsvp_id IN (1,2) AND f.is_page = 0 AND f.user_id = ' . Phpfox::getUserId())
                ->limit($limit)
                ->union()
                ->unionFrom('fevent_invite');

            $data = db()->select('fevent_invite.*, ' . Phpfox::getUserField())
                ->join(Phpfox::getT('user'), 'u', 'u.user_id = fevent_invite.user_id')
                ->limit($limit)
                ->execute('getSlaveRows');
        }
        ...
        $event['attending_statistic'] = [
            ...
            'total_friend_attending' => (int)$count['total_friend'],
            'total_other_people_attending' => (int)$totalCount - (int)$count['total_friend'],
            'other_people' => !empty($count['total_friend']) && ($count['total_friend'] > $limit) ? ((int)$count['total_friend'] - $limit) : 0
        ];
    }
    ...
    public function isInvitedByOwner($eventId, $iOwnerUserId = 0, $iInvitedUserId = 0)
    {
        ...
        if (!empty($iOwnerUserId) && !empty($iInvitedUserId)) {
            $count = db()->select('COUNT(*)')
                ...
                ->where('event_id = ' . (int)$eventId . ' AND user_id = ' . (int)$iOwnerUserId . ' AND invited_user_id = ' . (int)$iInvitedUserId)
                ->execute('getSlaveField');
        } else {
            $count = db()->select('e.event_id')
                ->from(Phpfox::getT('fevent'), 'e')
                ->leftJoin(Phpfox::getT('fevent_invite'), 'fi', 'fi.event_id = e.event_id AND ((fi.user_id != e.user_id AND fi.user_id != fi.invited_user_id) OR (fi.user_id = e.user_id)) AND fi.invited_user_id = ' . Phpfox::getUserId())
                ->where('e.event_id = ' . $eventId . ' AND (fi.user_id IS NOT NULL)')
                ->execute('getSlaveField');
        }
    ...
    public function getEvent($sEvent, $bUseId = false, $bNoCache = false)
    {
        ...
        $aEvent = $this->database()->select('e.*, e.country_iso as event_country_iso, ' . (Phpfox::getParam('core.allow_html') ? 'et.description_parsed' : 'et.description') . ' AS description, fi.user_id AS inviter_id, fi.invited_user_id AS invitee_id, ' . Phpfox::getUserField())
            ->from($this->_sTable, 'e')
            ->join(Phpfox::getT('user'), 'u', 'u.user_id = e.user_id')
            ->join(Phpfox::getT('fevent_text'), 'et', 'et.event_id = e.event_id')
    ...
    public function getAllNumbersOfAttendee(&$aEvent)
    {
        $aEvent = array_merge($aEvent, array(
            'iAttendingCnt' => $this->getNumbersOfAttendee($aEvent['event_id'], 1),
            'iMaybeCnt' => $this->getNumbersOfAttendee($aEvent['event_id'], 2),
            'iAwaitingCnt' => $this->getNumbersOfAttendee($aEvent['event_id'], 3)
        ));
    }
    ...
    public function getInvites($iEvent, $iRsvp, $iPage = 0, $iPageSize = 8)
    {
        ...
        $iCnt = $this->database()->select('COUNT(*)')
            ...
            ->where('event_id = ' . (int)$iEvent . ($iRsvp == 4 ? ' AND rsvp_id IN (1,2)' : ' AND rsvp_id = ' . $iRsvp))
            ->execute('getSlaveField');
        ...
        if ($iCnt) {
            ...
            $aInvites = $this->database()->select('ei.*, ' . Phpfox::getUserField())
                ->from(Phpfox::getT('fevent_invite'), 'ei')
                ->join(Phpfox::getT('user'), 'u', 'u.user_id = ei.invited_user_id')
                ->where('ei.event_id = ' . (int)$iEvent . ($iRsvp == 4 ? ' AND ei.rsvp_id IN (1,2)' : ' AND ei.rsvp_id = ' . $iRsvp))
    ...
    {
        $aRows = $this->database()->select('e.*, ei.rsvp_id, ei.user_id AS inviter_id, ei.invited_user_id AS invitee_id, ' . Phpfox::getUserField())
            ->from(Phpfox::getT('fevent_invite'), 'ei')
            ->join(Phpfox::getT('fevent'), 'e', 'e.event_id = ei.event_id AND e.view_id = 0')
            ->join(Phpfox::getT('user'), 'u', 'u.user_id = e.user_id')
            ->join(Phpfox::getT('fevent_text'), 'ft', 'ft.event_id = ei.event_id')
            ->where('ei.rsvp_id = 0 AND ei.invited_user_id = ' . Phpfox::getUserId() . ' AND ei.user_id != ' . Phpfox::getUserId())
            ->limit($iLimit)
            ->execute('getRows');
    ...
    public function getRandomSponsored($limit = 1)
    {
        ...
        if (!($aEvents = $this->cache()->get($sCacheId))) {
            $aEvents = $this->database()->select('e.*, s.*, s.country_iso AS sponsor_country_iso, s.gender AS sponsor_gender, ' . Phpfox::getUserField())
                ->from($this->_sTable, 'e')
                ->join(Phpfox::getT('user'), 'u', 'u.user_id = e.user_id')
                ->join(Phpfox::getT('better_ads_sponsor'), 's', 's.item_id = e.event_id AND s.is_custom = 3 AND s.is_active = 1')
        ...
        foreach ($aEvents as $iKey => $aEvent) {
            ...
            $aEvents[$iKey]['country_iso'] = $aEvent['sponsor_country_iso'];
            $aEvents[$iKey]['gender'] = $aEvent['sponsor_gender'];
        }
        ...
        return array_slice($aEvents, 0, $limit);
    }
    ...
    public function getPast($bIsPage = false, $bIsProfile = false, $iLimit = 3)
    {
        ...
        $aRows = $this->database()->select('v.*, ft.description_parsed, ei.rsvp_id, ei.user_id AS inviter_id, ei.invited_user_id AS invitee_id, ' . Phpfox::getUserField())
            ->from(Phpfox::getT('fevent'), 'v')
            ->join(Phpfox::getT('user'), 'u', 'u.user_id = v.user_id')
    ...
    public function getJsEvents($bIsPage = false, $bIsProfile = false)
    {
        $aRows = $this->database()->select('*')
            ->from($this->_sTable, 'm')
            ->where('m.view_id = 0 AND m.item_id = 0')
            ->execute('getRows');
    ...
    public function getBirthdays($iUser, $year)
    {
        ...
        {
            $aBirthdays = $this->database()->select(Phpfox::getUserField() . ', uf.dob_setting, fb.birthday_user_receiver, uf.first_name')
                ->from(Phpfox::getT('friend'), 'f')
                ->join(Phpfox::getT('user'), ' u', 'u.user_id = f.friend_user_id')
                ->join(Phpfox::getT('user_field'), 'uf', 'uf.user_id = u.user_id')
                ->leftJoin(Phpfox::getT('friend_birthday'), 'fb', 'fb.birthday_user_receiver = u.user_id AND fb.time_stamp > ' . (PHPFOX_TIME - 2629743))/* Fixes (SHB-989762) */
    ...
    public function getJsEventsForCalendar($from, $to, $conds = '')
    {
        ...
        $aRows = $this->database()->select('fe.*')
            ->from($this->_sTable, 'fe')
            ->leftJoin(Phpfox::getT('fevent_invite'), 'fei', 'fe.event_id = fei.event_id')
            ->where('fe.view_id = 0 AND fe.item_id = 0'
                . ' AND ((start_time > ' . $from . ' AND start_time < ' . $to . ')'
                . ' OR (end_time > ' . $from . ' AND end_time < ' . $to . ')'
                . ' OR (start_time < ' . $from . ' AND end_time > ' . $to . '))'
                . $conds
            )->group('fe.event_id')
            ->execute('getRows');
    ...
    public function getFeatured($bIsPage = false, $bIsProfile = false, $limit = 7, $getNew = false)
    {
        ...
        $aRows = $this->database()->select('v.*, ei.rsvp_id, ft.description_parsed, ' . Phpfox::getUserField())
            ->from(Phpfox::getT('fevent'), 'v')
            ->join(Phpfox::getT('user'), 'u', 'u.user_id = v.user_id')
            ->join(Phpfox::getT('fevent_text'), 'ft', 'ft.event_id = v.event_id')
            ->leftJoin(Phpfox::getT('fevent_invite'), 'ei', 'ei.event_id = v.event_id AND ei.invited_user_id = ' . Phpfox::getUserId())
    ...
    public function execute($aCallback = false, $searchParams = [])
    {
        ...
        $sAddress = !empty($searchParams['address']) ? urldecode($searchParams['address']) : '';
        ...
        $sCity = !empty($searchParams['city']) ? urldecode($searchParams['city']) : '';
        ...
        $rangeValueFrom = !empty($searchParams['rangevaluefrom']) ? urldecode($searchParams['rangevaluefrom']) : '';
        if (strlen(trim($rangeValueFrom)) > 0) {
            ...
            if ($kq == null || strlen(trim($kq[0])) < strlen(trim($rangeValueFromParsed))) {
                ...
                $glat = floatval(base64_decode($searchParams['glat']));
                $glong = floatval(base64_decode($searchParams['glong']));
        ...
        if (!empty($searchParams['country_child_id']) && (int)$searchParams['country_child_id'] > 0) {
            $this->_aConditions[] = " AND (m.country_child_id = " . (int)$searchParams['country_child_id'] . ")";
        }
    ...
    public function getTopEvent($sType = 'viewed', $iLimit = 4, $bIsPage = false, $bIsProfile = false, $bNoCount = false, $pageID = -1)
    {
        ...
        $aRows = array();
        if ($bIsPage) {
            $aRows = $this->database()->select(($bNoCount ? '' : 'SQL_CALC_FOUND_ROWS ') . 'v.*, ft.description_parsed, ei.rsvp_id, ei.user_id AS inviter_id, ei.invited_user_id AS invitee_id, ' . Phpfox::getUserField())
                ->from(Phpfox::getT('fevent'), 'v')
                ->join(Phpfox::getT('user'), 'u', 'u.user_id = v.user_id')
                ->join(Phpfox::getT('fevent_text'), 'ft', 'ft.event_id = v.event_id')
                ->leftJoin(Phpfox::getT('fevent_invite'), 'ei', 'ei.event_id = v.event_id AND ei.invited_user_id = ' . Phpfox::getUserId())
                ->where(' 1=1 AND v.view_id = 0 AND v.privacy IN(0) AND v.module_id = \'pages\' AND v.item_id = ' . (int)$pageID . $cond)
                ->order($sOrder)
                ->limit($iLimit)
                ->execute('getSlaveRows');
            if (!$bNoCount) {
            ...
            $aRows = $this->database()->select(($bNoCount ? '' : 'SQL_CALC_FOUND_ROWS ') . 'v.*, ft.description_parsed, ei.rsvp_id, ei.user_id AS inviter_id, ei.invited_user_id AS invitee_id, ' . Phpfox::getUserField())
                ...
                ->join(Phpfox::getT('fevent_text'), 'ft', 'ft.event_id = v.event_id')
                ->leftJoin(Phpfox::getT('fevent_invite'), 'ei', 'ei.event_id = v.event_id AND ei.invited_user_id = ' . Phpfox::getUserId())
                ->where(' 1=1 AND v.view_id = 0 AND ( (v.item_id = 0) ' . $wherePages . ') ' . $andWherePrivacy . $cond)
                ->order($sOrder)
                ->limit($iLimit)
                ->execute('getSlaveRows');
    ...
    public function getOnHomepageByType($sType = 'upcoming', $iLimit = 4, $bIsPage = false, $bIsProfile = false, $bNoCount = false, $pageID = -1)
    {
        ...
        if ($bIsPage) {
            ...
            if ((int)$iTotal > 0) {
                $aRows = $this->database()->select('v.*, pag.page_id, pag.title as page_title, vt.description_parsed, fi.rsvp_id, fi.user_id AS inviter_id, fi.invited_user_id AS invitee_id, ' . Phpfox::getUserField())
                    ->from(Phpfox::getT('fevent'), 'v')
                    ->join(Phpfox::getT('user'), 'u', 'u.user_id = v.user_id')
                    ->leftJoin(Phpfox::getT('pages'), 'pag', 'pag.page_id = v.item_id')
                    ->leftJoin(Phpfox::getT('fevent_text'), 'vt', 'vt.event_id = v.event_id')
                    ->leftJoin(Phpfox::getT('fevent_invite'), 'fi', 'fi.event_id = v.event_id AND fi.invited_user_id = ' . Phpfox::getUserId())
    ...
    public function cronSendNotification() {
        ...
        $notified_events = $this->database()
            ->select('*')
            ->from($this->_sTable)
            ->where($conds)
            ->executeRows();
        foreach ($notified_events as $event) {
            $invites = $this->database()->select('invite_id, invited_user_id, user_id')
                ->from(Phpfox::getT('fevent_invite'))
                ->where('(rsvp_id = 1 OR rsvp_id = 2) AND event_id = ' . (int)$event['event_id'])
                ->execute('getSlaveRows');
            foreach ($invites as $invite) {
                Phpfox::getService('notification.process')->add('fevent_notify', $event['event_id'], $invite['user_id'], $event['user_id']);
            }
            $this->database()
                ->update(':fevent', array('is_notified' => 1), 'event_id = ' . $event['event_id']);
        }
    }
    ...
    public function cronSendSubscribeMail($oldRunTimestamp, $newRunTimestamp)
    {
        ...
        foreach ($aSubscribeEmail as $keyaSubscribeEmail => $valueaSubscribeEmail) {
            ...
            if (count($aFilterEvent) > 0) {
                ...
                $message = _p('dear_sir_or_madam_here_are_businesses_you_may_interested_in_site_name_data_event_to_unsubscribe_follow_the_link_below_regards_site_name', array(
                    'within_day' => Phpfox::getParam('fevent.subscribe_within_day'),
                    'site_name' => Phpfox::getParam('core.site_title'),
    ...
    public function getUploadParams($aParams)
    {
        ...
        return [
            ...
            'js_events' => $aEvents,
            'upload_now' => "true",
            'submit_button' => '#js_fevent_done_upload',
            ...
            'thumbnail_sizes' => Phpfox::getParam('fevent.thumbnail_sizes'),
            'no_square' => true
        ];
    }
    ...
    public function getUploadDefaultParams($aParams = null)
    {
        ...
        return [
            ...
            'type_description' => _p('this_is_a_default_photo_which_you_will_see_in_listing_and_detail_page_you_can_upload_more_photos_in_the_next_step')
                . '<br>' . _p('fevent.allowed_file_type_jpg_gif_or_png') . ' ' . _p('the_file_size_limit_is_file_size_kb_strong', ['file_size' => $iMaxFileSize * 1048576 / 1024]) . ' ' . _p('recommended_dimension_for_the_best_view_x_px'),
