...
(function($) {
	...
	Calendar.prototype.setLanguage = function(lang) {
		console.log(lang);
		console.log(window.calendar_languages);
	...
	Calendar.prototype._month = function(month) {
		...
		t.month_name_short = this.locale['ms' + month];
	...
	Calendar.prototype._calculate_time_label_month_day = function(events, start, end) {
		var $self = this;
		...
		$.each(events, function(k, e){
			...
			if (e.start > start.getTime() && e.end < end.getTime()) {
				if ($self.options.view == 'month') {
					e.time_label = e.start_hour + ' - ' + e.end_hour;
				} else {
					e.time_label = s.getDate() + ' ' + $self.locale['ms' + s.getMonth()] + ' ' + e.start_hour + ' - ' + f.getDate() + ' ' + $self.locale['ms' + f.getMonth()] + ' ' + e.end_hour;
				}
			} else if (e.end < end.getTime()) {
				e.time_label = 'End: ' + e.end_hour;
			} else if (e.start < start.getTime()) {
				e.time_label = 'From ' + s.getDate() + ' ' + $self.locale['ms' + s.getMonth()] + ' ' + e.start_hour;
			} else {
				e.time_label = 'Start: ' + e.start_hour;
			}
		});
	}
	...
	Calendar.prototype._update_month_year = function() {
		...
		self.context.find('.cal-month-day, .cal-year-box .span3')
			...
			.on('click', function(event) {
				...
				if($(this).children('[data-cal-date]').text() == self.activecell) {
                    self.context.find('.cal-month-day').removeClass('open-event-list');
					return;
				}
				showEventsList(event, downbox, slider, self);
                if($(this).hasClass('cal-month-day')) {
                    self.context.find('.cal-month-day').removeClass('open-event-list');
                    $(this).addClass('open-event-list');
				}
			})
		;
	...
	Calendar.prototype.getEventsBetween = function(start, end) {
		...
		this._calculate_time_label_month_day(events, start, end);
		return events;
	};
