...
function inputToMapAdvEvent()
{
	if (typeof(oGeoCoder) == 'undefined') {
        initializeAdvEvent();
	}
	...
	sQueryAddress += ' ' + ($('#country_iso option:selected').text() == 'Select:' ? '' : $('#country_iso option:selected').text());
	oGeoCoder.geocode({
...
function initializeAdvEvent()
{
	...
	oMap = new google.maps.Map(mapHolder, myOptions);
...
function fevent_loadScript(sKey)
{
    var prefix_api = 'https://';
	var script = document.createElement('script');
	script.type= 'text/javascript';
	script.src = prefix_api + '//maps.google.com/maps/api/js?sensor=false&callback=initializeAdvEvent&key=' + sKey ;
