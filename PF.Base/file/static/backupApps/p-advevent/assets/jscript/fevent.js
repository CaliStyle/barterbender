var fevent = {
	...
	,bindGuestListButtons: function(iEventId){
		$('.fevent_detail_guest_list').off('click').click(function(){
			var sTab = $(this).data('tab');
			var iEventId = $('.p-fevent-detail-member').data('event-id');
			tb_show(oTranslations['guest_list'], $.ajaxBox('fevent.showGuestList', 'tab=' + sTab + '&event_id=' + iEventId));
		});
	}
        ...
        $Core.ajax('fevent.loadAjaxDetailMapView',
        {
            ...
            success: function(sOutput)
            {
                ...
                if(oOutput.status == 'SUCCESS')
                {
					...
					var $map_container = $('#fevent_detail_map');
					fevent.showMapsWithData($map_container, datas, contents,fevent_detail_map);
                }
            }
        });
	...
	,showMapView: function(id){
		var $block = $('.p-fevent-listing-container[data-block-id=' + id + ']')
		var typeEventMap = $block.data('source');
        $Core.ajax('fevent.loadAjaxMapView',
        {
            ...
            success: function(sOutput)
            {
                ...
                if(oOutput.status == 'SUCCESS')
                {
					...
					var $map_container = $('.p-fevent-listing-container[data-block-id='+ id +']').find('.fevent-map');
					fevent.showMapsWithData($map_container, datas, contents, null);
                }
            }
        });
	}
	, showMapsWithData: function($map_container, datas, contents, objectMap){
		...
		if (datas.length > 0) {
			...
			function showMapsWithData_initialize() {
				...
				objectMap = new google.maps.Map($map_container[0], mapOptions);
			...
			$($map_container).html('<p class="help-block" style="margin-top: 0">' + oTranslations['fevent.no_events_found_on_map'] + '</p>');
		}
	}
		...
		if($.isArray(info)){
			...
			$.each(info, function(key, aEvent){
				sHtml += '<div class="business-item-map" style="width: 300px; height: 64px; padding: 8px 5px; border-bottom: 1px solid #ebebeb; font-size: 12px; overflow: hidden; box-sizing: content-box; -webkit-box-sizing: content-box; -moz-box-sizing: content-box;">';
					...
					sHtml += '<div style="display: inline-block;"><a href="//maps.google.com/maps?daddr='+aEvent['location']+'" target="_blank"><img src="' + fevent.sCorePath + 'PF.Site/Apps/p-advevent/assets/image/icon-getdirection.png" /> '+oTranslations['fevent.v_getdirections']+'</a></div>';
	 	...
	 	var autocomplete = new google.maps.places.Autocomplete(input);
	  	...
	  	$('.category_checkbox').on('change', function() {
            ...
            if ($('.category_checkbox:checked').length) {
				...
				$('.subscribe-categories__text').text(oTranslations['fevent.select_categories']);
			}
        });
	}
...
$Behavior.initFeventDetailSlideshow = function(){
	var owl = $('.p-fevent-detail-slider.owl-carousel');
