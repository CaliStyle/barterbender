...
<form method="post" action="{url link='current'}" enctype="multipart/form-data" {if $bIsEdit && $aForms.event_type == 'repeat'}onsubmit="return custom_js_event_form(this);"{/if} id="js_event_form" class="p-fevent-container-add-page">
    ...
    <div id="js_event_block_detail" class="js_event_block page_section_menu_holder "
        ...
        <div class="form-group">
            ...
            <input type="text" class="form-control" name="val[location]" value="{value type='input' id='location'}"
                   id="location_venue" size="40" maxlength="200" placeholder="{_p var='location_venue_lc'}" {if !$bIsEdit}required{/if}>
        </div>
        {if !$bIsEdit}
            <div class="help-block mt-1 mb-3">
                <a href="#" class="fw-bold" onclick="$(this).parent().hide(); $('#js_event_add_country').show(); return false;">
                    <i class="ico ico-plus"></i> {_p var='add_more_location_info'}
                </a>
            </div>
        {/if}
        ...
        <div id="js_event_add_country" {if !$bIsEdit} style="display:none;"{/if} class="mb-2">
            ...
            <div class="form-group" style="display: none;">
                ...
                <input type="text" class="form-control" name="val[range_value]"
                       value="{value type='input' id='range_value'}" id="range_value" size="10" maxlength="20"/>
                <select name="val[range_type]" class="form-control">
                    <option value="0"
                            {if isset($aEvent.range_type) && $aEvent.range_type==0}selected{/if}>{_p var='fevent.miles'}</option>
                    <option value="1"
                            {if isset($aEvent.range_type) && $aEvent.range_type==1}selected{/if}>{_p var='fevent.km'}</option>
                </select>
            </div>
        </div>
