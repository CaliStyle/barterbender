...
var yncvideovp = {
    ...
    init: function () {
        ...
        var $playlistList = $popup.find('.ynuv_quick_list_playlist');
        if ($playlistList.length) {
            $playlistList.on('DOMSubtreeModified', yncvideovp.updatePlaylistListScroll);
        }
    ...
    addCommentFormObserver($commentForm) {
    ...
    updatePlaylistListScroll: function () {
        var $playlistList = $(this), self = this;
        clearTimeout(yncvideovpUpdatePlaylistListTimeOut);
        yncvideovpUpdatePlaylistListTimeOut = setTimeout(function () {
            if (!$playlistList.find('div.mCustomScrollBox').length) {
                $playlistList.find('div.checkbox').each(function () {
                    $(this).addClass('videovp-checkbox-custom');
                    $(this).find('input').after('<i class="ico ico-square-o mr-1"></i>');
                });
                $playlistList.data('mCS', null).mCustomScrollbar({
                    theme: "minimal-dark",
                })
            }
        }, 50);
