...
{if count($aGroups)}
    <div id="js_menu_drop_down" style="display:none;">
        <div class="link_menu dropContent" style="display:block;">
            <ul>
                <li><a href="#"
                       onclick="return $Core.ultimatevideo.actionCustomField(this, 'edit','{$sUrl}');">{_p('Edit')}</a>
                </li>
                <li><a href="#active"
                       onclick="return $Core.ultimatevideo.actionCustomField(this, 'active','{$sUrl}');">{_p('set_to_inactive')}</a>
                </li>
                <li><a href="#"
                       onclick="return $Core.ultimatevideo.actionCustomField(this, 'delete','{$sUrl}');">{_p('Delete')}</a>
                </li>
            </ul>
        </div>
    </div>
                ...
                <div class="sortable dont-unbind-children">
                    <ul>
                        ...
                        <li class="{if $mGroup !== 'PHPFOX_EMPTY_GROUP'}group{/if}{if $phpfox.iteration.groups == 1} first{/if}"
                            ...
                            <div style="display:none;"><input type="hidden" name="group[{$aGroup.group_id}]"
                                                              value="{$aGroup.ordering}"/></div>
                            <a href="#?id={$aGroup.group_id}&amp;type=group" class="js_drop_down"
                               id="js_group_{$aGroup.group_id}">{img theme='misc/draggable.png' alt='' class='v_middle'}{if !$aGroup.is_active}
                                <del>{/if}{{phrase var=$aGroup.phrase_var_name}{if !empty($aGroup.user_group_name)} ({$aGroup.user_group_name|clean}){/if}</a>{if !$aGroup.is_active}</del>{/if}{{/if}</a>
...
<div class="panel-body">
    <div class="extra_info">
        ...
        <div class="action dont-unbind-children" style="margin-top:15px;">
            <div>
                <a class="btn btn-primary" href="#" onclick="window.location.href=$('.toolbar-top .btn-group a:eq(4)').attr('href'); return false;">
                {_p('add_a_new_custom_group')}
                </a>
            </div>
        </div>
    </div>
