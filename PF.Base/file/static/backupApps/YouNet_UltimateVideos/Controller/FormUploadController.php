...
class FormUploadController extends Phpfox_Component
{
    public function process()
    {
        Phpfox::isUser(true);
        user('ynuv_can_upload_video', '1', null, true);
        ...
        $file_size = user('ynuv_file_size_limit_in_megabytes', 10);

        $path = PHPFOX_DIR_FILE . 'ynultimatevideo' . PHPFOX_DS;
        if (!is_dir($path)) {
            @mkdir($path, 0777, 1);
        }
        $sId = md5(uniqid() . Phpfox::getUserId());
        $date = date('Y/m/');
        $name = $date . $sId . '.' . Phpfox_File::instance()->getFileExt($_FILES['ajax_upload']['name']);
        $post = [];
        ...
        if($methodUpload == 1 && ($videoKey = Phpfox::getParam('ultimatevideo.ynuv_video_key')) && ($awsKey = Phpfox::getParam('ultimatevideo.ynuv_video_s3_key'))) {
            $tempPath = $path;
            foreach(['Y', 'm'] as $number) {
                $tempPath .= date($number) . PHPFOX_DS;
                if(!is_dir($tempPath)) {
                    @mkdir($tempPath, 0777);
                    @chmod($tempPath, 0777);
                }
            }
            if (!@move_uploaded_file($_FILES['ajax_upload']['tmp_name'], $path . $name)) {
                http_response_code(400);
            ...
            if(defined('PHPFOX_DEBUG') && PHPFOX_DEBUG) {
                Phpfox::getLog('ultimatevideo_s3.log')->info('s3_put_result ' . $result);
            }
            try {
                $zencoder = new \Services_Zencoder(Phpfox::getParam('ultimatevideo.ynuv_video_key'));
                $params = [
                    "input" => 's3://' . $bucket . '/' . $name,
                    'notifications' => [
                        'url' => url('/ultimatevideo/callback/')
                    ],
                    "outputs" => [
                        [
                            "label" => "mp4 high",
                ...
                storage()->set('ynuv_video_' . $encoding_job->id, [
        ...
        elseif ($methodUpload == 0 && Phpfox::getParam('ultimatevideo.ynuv_ffmpeg_path')) {
            $videoFilePath = PhpFox::getLib('file')->upload('ajax_upload', $path, $_FILES['ajax_upload']['name'], true, 0644, true, false);
            return [
