        ...
        if (($iEditId = $this->request()->getInt('id'))) {
            ...
            $this->template()->setHeader('cache', array(
                '<script type="text/javascript">
                    $Behavior.ynadvancedblogEditCategory = function(){
                        var aCategories = explode(\',\', \'' . $sCategories . '\');
            ...
            if (user('yn_advblog_max_blogs') && user('yn_advblog_max_blogs') <= Phpfox::getService('ynblog.blog')->getMyTotal()) {
                Phpfox_Error::display(_p('You have exceeded limited writable blogs'));
            }
        }
            ...
            if ($oValid->isValid($aVals)) {
                ...
                if (isset($aVals['publish']) || isset($aVals['draft'])) {
                    if (empty($aVals['temp_file'])) {
                        Phpfox_Error::set(_p('please_select_an_photo'));
                    }
