...
class Process extends Phpfox_Service
{
    ...
    public function delete($iId)
    {
        ...
        {
            return Phpfox_Error::set(_p('Unable to find the custom field you want to delete'));
        }
    ...
    public function deleteOption($iId)
    {
        ...
        {
            return Phpfox_Error::set(Phpfox::getPhrase('directory.unable_to_find_the_custom_option_you_plan_on_deleting'));
        }
    ...
    public function addValue($aValues, $iReviewId)
    {
        $aInsert = array('review_id' => $iReviewId);
    ...
    public function updateValue($aValues, $iReviewId)
    {
        $aUpdate = array();

        foreach($aValues as $iFieldId => $aValue)
        {
            // if field doesn't exit in custom value, field will be added.
            $aFields = $this->database()->select('cv.value_id')
                ->from($this->_sTableValue , 'cv')
                ->where('cv.field_id = '. $iFieldId .' AND cv.review_id = '.$iReviewId)
                ->execute('getSlaveRows');

            if(count($aFields) <= 0) {
                $this->addValue( array( $iFieldId => $aValue), $iReviewId);
                continue;
            }

            $aUpdate['field_id'] = $iFieldId;

            if(is_array($aValue))
            {
               $this->database()->delete($this->_sTableValue,'field_id = '. $iFieldId .' AND review_id = '.$iReviewId);
            ...
            {
                $aUpdate['option_id'] = null;
                $aUpdate['value'] = Phpfox::getLib('parse.input')->clean($aValue);
                $this->database()->update($this->_sTableValue, $aUpdate,' review_id = ' . $iReviewId . ' AND field_id = '.$iFieldId);
            }
        }
    ...
    public function deleteValue($iReviewId)
    {
        $this->database()->delete(Phpfox::getT('ynmember_custom_value'), 'review_id = ' . $iReviewId);
    }
    ...
    private function _addOptions($iFieldId, &$aVals)
    {
        ...
        if (isset($aVals['current']))
        {
            ...
            $aExisting[] = str_replace('core.ynmember_cf_option_' . $aVals['field_id'] . '_','',$sVarName);
        }
        ...
        foreach ($aVals['option'] as $iKey => $aOptions)
        {
            ...
            foreach ($aOptions as $sLang => $aOption)
            {
                ...
                $sPhraseVar = 'ynreview_cf_option_' . $iFieldId . '_' . $iSeqNumber;
