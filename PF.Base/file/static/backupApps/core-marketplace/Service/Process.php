...
class Process extends Phpfox_Service
{
    ...
    public function add($aVals)
    {
        ...
        if (!Phpfox::getUserParam('marketplace.listing_approve')) {
            (Phpfox::isModule('feed') && Phpfox::getParam('marketplace.marketplace_allow_create_feed_when_add_new_item', 1) ? Phpfox::getService('feed.process')->add('marketplace', $iId, $aVals['privacy'],
    ...
    public function approve($iId)
    {
        ...
        $aListing = $this->database()->select(Phpfox::getUserField() . ', v.*')
            ->from($this->_sTable, 'v')
        ...
        (Phpfox::isModule('feed') && $bAddFeed && Phpfox::getParam('marketplace.marketplace_allow_create_feed_when_add_new_item', 1) ? Phpfox::getService('feed.process')->add('marketplace', $iId,
