...
<div class="market-app detail-extra-info">
    ...
    {if Phpfox::isUser() && $aListing.user_id != Phpfox::getUserId()}
       <div class="item-action-contact">
            <div class="item-action-list">
                ...
                {/if}
                {if (($aListing.is_sell || $aListing.allow_point_payment) && $aListing.view_id != '2' && $aListing.price != '0.00')}
                ...
                {if $aListing.canContactSeller}
                    <button class="btn btn-default" onclick="$Core.marketplace.contactSeller({l}id: {$aListing.user_id}, listing_id: {$aListing.listing_id}, module_id: 'marketplace'{r}); return false;">
    ...
    {if !empty($aListing.location)}
        <div class="item-info-location">
            ...
            <span>
                <a href="//maps.google.com/?q={$aListing.location}" target="_blank">{$aListing.location}</a>
            </span>
        </div>
    {/if}
