...
class Get extends \Phpfox_Service
{
    ...
    public function getForEdit($iId)
    {
        ...
        $aAd = db()->select('*')->from($this->_sTable)->where('ads_id = ' . (int)$iId)->execute('getSlaveRow');

        if (!isset($aAd['ads_id'])) {
            return Phpfox_Error::set(_p('better_ads_unable_to_find_this_ad'));
        }

        $aTemp = db()->select('cc.child_id')->from(':country_child', 'cc')->join(':better_ads_country', 'a',
            'a.child_id = cc.child_id')->where('a.ads_id = ' . $aAd['ads_id'])->execute('getSlaveRows');
    ...
    public function getSponsor($iId, $iUser = null)
    {
        $aSponsor = db()->select('s.*, ai.invoice_id, ai.ads_id, ai.is_sponsor, ai.currency_id, ai.price, ai.status, ai.time_stamp, ai.time_stamp_paid')
            ->from(':better_ads_sponsor', 's')
            ->leftJoin(':better_ads_invoice', 'ai', 'ai.ads_id = s.sponsor_id')
    ...
    public function getInvoices($aConds, $sSort = 'time_stamp DESC', $iPage = '', $iLimit = '')
    {
        $iCnt = db()->select('COUNT(*)')
            ...
            ->leftJoin(':user', 'u', 'u.user_id = ai.user_id')
            ->where($aConds)
            ->execute('getSlaveField');

        $aInvoices = db()->select('u.*, ai.*, a.name as ad_name')
            ->from(':better_ads_invoice', 'ai')
            ->leftJoin(':better_ads', 'a', 'a.ads_id = ai.ads_id')
            ->leftJoin(':user', 'u', 'u.user_id = ai.user_id')
            ->where($aConds)
            ->order($sSort)
            ->limit($iPage, $iLimit, $iCnt)
    ...
    public function getTypeOfApproved($iStartDate, $iEndDate)
    {
        if ($iStartDate > PHPFOX_TIME) {
            return 'upcoming';
        } elseif ($iEndDate == 0 || $iEndDate > PHPFOX_TIME) {
            return 'running';
        } else {
            return 'ended';
        }
