...
class Process extends \Phpfox_Service
{
    ...
    public function addSponsor($aVals, $bValidate = true)
    {
        ...
        if($bCanSponsorAll || ($bWithoutPaying && $iAutoPublish))
        {
            if($sModule == 'feed')
            {
                $this->cache()->remove('better_ads_feed_sponsors');
            }
        }
    ...
    public function updateActivity($aVals)
    {
        ...
        foreach ($aVals as $iId => $aVal) {
            db()->update($this->_sTable, array('is_active' => (isset($aVal['is_active']) ? '1' : '0')),
                'ads_id = ' . (int)$iId);
        }

        Phpfox::getLib('cache')->removeGroup('betterads');
    ...
    public function addSponsorViewsCount($iSponsorId, $sModule, $sFunction = 'sponsor')
    {
        ...
        db()->update(':better_ads_sponsor', array('total_view' => $aItem['total_view'] + 1),
    ...
    */
    ...
    public function addCustom($aVals)
    {
        ...
        $fPrice = ($aVals['default_cost'] * $aVals['total_view']);
    ...
    public function approve($iId)
    {
        ...
        $this->cache()->removeGroup('betterads');
    ...
    public function approveSponsor($iId)
    {
        ...
        if($aAd) {
            ...
            if (!empty($sModule) && Phpfox::hasCallback($sModule, 'enableSponsor')) {
    ...
    public function denySponsor($iId)
    {
        ...
        db()->update(':better_ads_sponsor', array(
            'is_custom' => '4',
            'is_active' => '1'
        ), 'sponsor_id = ' . (int)$iId
        );
    ...
    public function sendApproveEmail($iId, $bSponsor = false)
    {
        ...
        if ($bSponsor === true) {
            $sSubject = array('better_ads_sponsor_ad_approved');
            $sMessage = array('better_ads_your_sponsor_ad_on_site_name_has_been_approved', array(
            ...
            $sMessage = array('better_ads_your_ad_on_site_name_has_been_approved', array(
                    'site_name' => Phpfox::getParam('core.site_title'),
                    'link' => Phpfox_Url::instance()->makeUrl('ad.add', array('id' => $aAd['ads_id']))
                )
            );
        }
    ...
    public function sendDenyEmail($iId, $bSponsor = false)
    {
        ...
        if ($bSponsor === true) {
            ...
            $sMessage = array('better_ads_your_sponsor_ad_on_site_name_has_been_denied', array(
                'site_name' => Phpfox::getParam('core.site_title'),
                'link' => Phpfox_Url::instance()->makeUrl('ads.sponsor', array('view' => $aAd['sponsor_id']))
            ));
        } else {
            $sSubject = 'ad_denied';
            $sMessage = array('better_ads_your_sponsor_ad_on_site_name_has_been_denied', array(
                    'site_name' => Phpfox::getParam('core.site_title'),
                    'link' => Phpfox_Url::instance()->makeUrl('ads.add', array('id' => $aAd['ads_id']))
                )
            );
        }
    ...
    public function updateAdPlacementActivity($iId, $iType)
    {
        Phpfox::isUser(true);
        Phpfox::getUserParam('admincp.has_admin_access', true);
        db()->update(':better_ads_plan', array('is_active' => (int)($iType == '1' ? 1 : 0)),
            'plan_id = ' . (int)$iId);
        Phpfox::getLib('cache')->remove();
    }
