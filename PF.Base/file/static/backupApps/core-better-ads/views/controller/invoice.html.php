    ...
    <form action="{url link='current'}" method="get" class="ml--1 mr--1">
        ...
        <div class="form-group px-1 d-inline-block shink-1">
            ...
            <select name="search[status]" id="status" class="form-control">
                <option value="0" {value type="select" default="0"}>{_p var='all_status'}</option>
            ...
            {foreach from=$aInvoices item=aInvoice}
                <tr>
                    ...
                    <td class="px-3 align-middle">
                        {if $aInvoice.status === null && $aInvoice.ad_name}
                        <a class="btn btn-primary btn-sm" href="{if $aInvoice.is_sponsor != 1}{url link='ad.add.completed' id=$aInvoice.ads_id}{else}{url link='ads.sponsor' pay=$aInvoice.ads_id}{/if}">{_p var='better_ads_pay_now'}</a>
