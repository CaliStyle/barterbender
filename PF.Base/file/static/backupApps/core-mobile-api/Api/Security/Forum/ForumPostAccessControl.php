...
class ForumPostAccessControl extends AccessControl
{
    ...
    public function __construct(SettingInterface $setting, UserInterface $context)
    {
        ...
        $this->supports = $this->mergePermissions([self::ADD, self::DELETE, self::EDIT, self::VIEW, self::APPROVE, self::THANK]);
    }
    ...
    public function isGranted($permission, ResourceBase $resource = null)
    {
        ...
        switch ($permission) {
            ...
            case self::THANK:
                /** @var $resource ForumPostResource * */
                $granted = $resource && !$isOwner && \Phpfox::getParam('forum.enable_thanks_on_posts') && $this->isGrantedSetting('forum.can_thank_on_forum_posts') && !$this->isBlocked($resource->getAuthor());
                break;
        }
