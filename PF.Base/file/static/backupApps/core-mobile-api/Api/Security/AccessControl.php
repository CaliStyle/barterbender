...
class AccessControl
{
    ...
    public function isGranted($permission, ResourceBase $resource = null)
    {
        ...
        if ($resource instanceof ResourceBase
            && property_exists($resource, "module_id") && !empty($resource->module_id)
            && property_exists($resource, "item_id") && !empty($resource->item_id)) {
            $this->setAppContext(AppContextFactory::create($resource->module_id, $resource->item_id));
        }
        ...
        if ($permission == self::IS_AUTHENTICATED) {
            if ($this->getUserContext()->getId() == 0) {
                return false;
            }
        }

        if ($permission == self::LIKE) {
            return \Phpfox::isModule('like') && (!$resource || !$resource->getIsPending());
        }

        if ($permission == self::SHARE) {
            return \Phpfox::isModule('share') && $this->isGrantedSetting('share.can_share_items') && (!$resource || (!$resource->getIsPending() && $resource->getPrivacy() == PrivacyType::EVERYONE));
        }
    ...
    public function isInFriendList($module, $itemId, UserInterface $user)
    {
        ...
        if ($user->getId()) {
            $iCheck = (int)db()->select('COUNT(privacy_id)')
                ->from(':privacy', 'p')
                ->join(':friend_list_data', 'fld', 'fld.list_id = p.friend_list_id AND fld.friend_user_id = ' . $user->getId())
                ->where([
                    'p.module_id' => $module,
                    'p.item_id'   => $itemId
    ...
    {
        return \Phpfox::getService('user.block')->isBlocked(null, $owner->getId());
    }

    public function isFriendWith(UserInterface $viewer, UserInterface $owner)
    {
        return \Phpfox::getService('friend')->isFriend($viewer->getId(), $owner->getId());
    }

    public function isFriendOfFriend(UserInterface $viewer, UserInterface $owner)
    {
        return \Phpfox::getService('friend')->isFriendOfFriend($owner->getId());
    }
