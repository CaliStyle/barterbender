    ...
    public function isGranted($permission, ResourceBase $resource = null)
    {
        ...
        switch ($permission) {
            case self::VIEW:
                ...
                if ($granted && !empty($this->parameters['view'])) {
                    if ($this->parameters['view'] == 'spam' || $this->parameters['view'] == 'pending') {
                        $granted = $this->isGrantedSetting('blog.can_approve_blogs');
                    } else if ($this->parameters['view'] == 'my') {
            ...
            case self::PURCHASE_SPONSOR:
                $granted = $this->isGrantedSetting(['blog.can_purchase_sponsor']);
                break;
            case self::PUBLISH:
