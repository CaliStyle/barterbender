...
class VideoForm extends GeneralForm
{
    ...
    {
        $methodUpload = $this->setting->getAppSetting('v.pf_video_method_upload');
        $allowUpload = $this->setting->getAppSetting('v.pf_video_support_upload_video') && (($methodUpload == 1 && $this->setting->getAppSetting('v.pf_video_key')) || ($methodUpload == 0 && $this->setting->getAppSetting('v.pf_video_ffmpeg_path')));
        $sectionName = 'basic';
        ...
        if ($this->isEditing()) {
            $this->addField('file', FileType::class, [
                ...
                'preview_url'         => $this->getPreviewImage(),
                'max_upload_filesize' => $this->getSizeLimit($this->setting->getUserSetting('v.pf_video_max_file_size_photo_upload'))
            ], null, $sectionName);
        }
        ...
        if (empty($this->data['item_id'])) {
            $this
                ...
                ], $sectionName, $this->privacy->getValue('v.default_privacy_setting'));
        }
        $this
            ->addModuleFields([
                'module_value' => 'video'
