    ...
    function buildForm($options = null, $data = [])
    {
        ...
        $this
            ...
            ->addField('file', FileType::class, [
                ...
                'max_upload_filesize' => $this->getSizeLimit($this->setting->getUserSetting('blog.blog_photo_max_upload_size'))
            ], null, $sectionName);
        ...
        if (empty($this->data['item_id'])) {
            $this->addPrivacyField([
                'description' => 'control_who_can_see_this_blog'
            ], $sectionName, $this->privacy->getValue('blog.default_privacy_setting'));
        }
        $this
            ->addModuleFields([
                'module_value' => 'blog'
        ...
        if (!$this->isEdit()) {
            ...
            if ($this->data['post_status'] == 2) {
                $this
                    ->addField('draft', CheckboxType::class, [
                        'label' => 'save_as_draft',
                        'value' => 1
                    ]);
            }
