...
class UserResource extends ResourceBase implements UserInterface
{
        ...
        if (empty($this->summary)) {
            ...
            if (Phpfox::getService('user.privacy')->hasAccess($this->getId(), 'profile.view_location')) {
                ...
                if ($hasCityLocation || $hasCountry || $hasChild) {
                    $summary .= $this->getTranslator()->translate('lives_in') . ($hasCityLocation ? " " . $this->city_location : "");
            ...
            $this->summary = rtrim($summary);
        }


        return trim(html_entity_decode($this->summary, ENT_QUOTES), ". ,");
    }
    ...
    {
        return $this->profile_page !== null ? $this->profile_page['id'] : $this->id;
    }
     ...
     */
    function compareWith(UserInterface $user)
    {
    ...
    public function _getFriendship()
    {
        ...
        $iFriendRequestAwait = $isModule ? Phpfox::getService('friend.request')->isRequested($this->getId(), $userId) : false;
        $iFriendRequestSent = $iFriendRequestAwait ? false : ($isModule ? Phpfox::getService('friend.request')->isRequested($userId, $this->getId()) : false);
        if (!$userId) {
            $status = self::FRIENDSHIP_IS_UNKNOWN;
        } else if (!$isModule) {
            $status = self::FRIENDSHIP_IS_UNKNOWN;
        } else if (($isModule && Phpfox::getService('friend')->isFriend($userId, $this->getId())) || !empty($this->rawData['is_friend'])) {
        ...
        } else if ($this->accessControl->isGranted(UserAccessControl::ADD_FRIEND) && $userId != $this->getId()) {
        ...
        return self::createSettingForResource([
            ...
            'forms' => [
                ...
                'cancelAccount' => [
                    ...
                    'succeedAction' => '@auth/forceLogout',
    ...
    public function getFriendId()
    {
        if ($this->friend_id == null) {
            $this->friend_id = db()->select('friend_id')->from(':friend')->where([
