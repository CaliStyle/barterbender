...
class PollApi extends AbstractResourceApi implements ActivityFeedInterface, MobileAppSettingInterface
{
        ...
        if ($params['when']) {
            ...
            switch ($params['when']) {
                case 'today':
                    ...
                    $this->search()->setCondition(' AND (poll.time_stamp >= \'' . Phpfox::getLib('date')->convertToGmt($iTimeDisplay) . '\' AND poll.time_stamp < \'' . Phpfox::getLib('date')
                            ->convertToGmt($iEndDay) . '\')');
                    break;
                case 'this-week':
                    $this->search()->setCondition(' AND poll.time_stamp >= ' . (int)Phpfox::getLib('date')->convertToGmt(Phpfox::getLib('date')->getWeekStart()));
                    $this->search()->setCondition(' AND poll.time_stamp <= ' . (int)Phpfox::getLib('date')->convertToGmt(Phpfox::getLib('date')->getWeekEnd()));
                    break;
                case 'this-month':
                    $this->search()->setCondition(' AND poll.time_stamp >= \'' . Phpfox::getLib('date')->convertToGmt(Phpfox::getLib('date')->getThisMonth()) . '\'');
                    $iLastDayMonth = Phpfox::getLib('date')->mktime(0, 0, 0, date('n'), Phpfox::getLib('date')->lastDayOfMonth(date('n')), date('Y'));
                    $this->search()->setCondition(' AND poll.time_stamp <= \'' . Phpfox::getLib('date')->convertToGmt($iLastDayMonth) . '\'');
                    break;
                default:
                    break;
            }
    ...
    function findOne($params)
    {
        ...
        if (!isset($item['is_friend'])) {
            $item['is_friend'] = 0;
        }
        if (Phpfox::isModule('privacy')
            && !Phpfox::getService('privacy')->check('poll', $item['poll_id'], $item['user_id'], $item['privacy'],
                $item['is_friend'], true)) {
            return $this->permissionError();
        }
    ...
    private function convertSubmitForm(&$values, $edit = false)
    {
        ...
        $maxAns = $this->setting->getUserSetting('poll.maximum_answers_count');
        $totalAns = count($values['answers']);
        if ($maxAns < count($values['answers'])) {
            return $this->permissionError($this->getLocalization()->translate('maximum_answers_you_can_add_is', ['max' => $maxAns]));
        }
        if ($totalAns < 2) {
            return $this->permissionError($this->getLocalization()->translate('minimum_answers_you_must_add_is_2'));
        }
    ...
    function getFeedDisplay($feed, $item)
    {
        ...
        if (empty($item) && !$item = $this->loadResourceById(isset($feed['poll_id']) ? $feed['poll_id'] : $feed['item_id'])) {
            return null;
        }
        $resource = $this->populateResource(PollResource::class, $item);
    ...
    function sponsor($params)
    {
        ...
        $item = $this->loadResourceById($id, true);
        $this->denyAccessUnlessGranted(PollAccessControl::SPONSOR, $item);
