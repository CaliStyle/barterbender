...
{
    const ERROR_PHOTO_NOT_FOUND = "Album not found";
            ...
            switch ($view) {
                ...
                default:
                    if (!empty($parentModule)) {
                        ...
                        $condition = 'AND pa.view_id = 0 AND pa.total_photo > 0 AND pa.timeline_id = 0';
        ...
        if ($params['when']) {
            ...
            switch ($params['when']) {
                case 'today':
                    ...
                    $this->search()->setCondition(' AND (pa.time_stamp >= \'' . Phpfox::getLib('date')->convertToGmt($iTimeDisplay) . '\' AND pa.time_stamp < \'' . Phpfox::getLib('date')->convertToGmt($iEndDay) . '\')');
                    break;
                case 'this-week':
                    $this->search()->setCondition(' AND pa.time_stamp >= ' . (int)Phpfox::getLib('date')->convertToGmt(Phpfox::getLib('date')->getWeekStart()));
                    $this->search()->setCondition(' AND pa.time_stamp <= ' . (int)Phpfox::getLib('date')->convertToGmt(Phpfox::getLib('date')->getWeekEnd()));
                    break;
                case 'this-month':
                    $this->search()->setCondition(' AND pa.time_stamp >= \'' . Phpfox::getLib('date')->convertToGmt(Phpfox::getLib('date')->getThisMonth()) . '\'');
                    $iLastDayMonth = Phpfox::getLib('date')->mktime(0, 0, 0, date('n'), Phpfox::getLib('date')->lastDayOfMonth(date('n')), date('Y'));
                    $this->search()->setCondition(' AND pa.time_stamp <= \'' . Phpfox::getLib('date')->convertToGmt($iLastDayMonth) . '\'');
                    break;
                default:
                    break;
            }
    ...
    function sponsor($params)
    {
        ...
        $item = $this->loadResourceById($id, true);
        $this->denyAccessUnlessGranted(PhotoAlbumAccessControl::SPONSOR, $item);
