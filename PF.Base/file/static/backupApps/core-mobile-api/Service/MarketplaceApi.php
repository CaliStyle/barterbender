...
{
    const ERROR_MARKETPLACE_NOT_FOUND = "Listing not found";
    ...
    function findAll($params = [])
    {
        ...
        if ($params['bounds']) {
            ...
            if (isset($bounds['south'], $bounds['north'])) {
                $this->search()->setCondition(' AND l.location_lat >= ' . $bounds['south'] . ' AND l.location_lat != 0');
                $this->search()->setCondition(' AND l.location_lat <= ' . $bounds['north']);
            }
            if (isset($bounds['west'], $bounds['east'])) {
                if ($bounds['west'] > $bounds['east']) {
                    $this->search()->setCondition(' AND l.location_lng <= ' . $bounds['west'] . ' AND l.location_lng != 0');
                    $this->search()->setCondition(' AND l.location_lng >= ' . $bounds['east']);
                } else {
                    $this->search()->setCondition(' AND l.location_lng >= ' . $bounds['west'] . ' AND l.location_lng != 0');
                    $this->search()->setCondition(' AND l.location_lng <= ' . $bounds['east']);
                }
