...
class FeedAttachmentHelper
{
     ...
     * @param null|FeedResource $feedResource
     *
     * @return array
     */
    public function forListing($feedItem, $feedResource = null)
    {
        ...
        if (strpos($typeId, "_comment") !== false
            ...
            if ($comment
                ...
                && ($service = $router->getApiServiceByResourceName($comment->getItemType()))) {
            ...
            if (!empty($feedItem['parent_feed_id']) && !empty($feedItem['parent_module_id'])) {
                ...
                if (isset($parentFeed['privacy']) && $parentFeed['privacy']
                    && (isset($parentFeed['user_id']) && $parentFeed['user_id'] != Phpfox::getUserId())
                ) {
                    $parentFeed = [];
                }
                if (empty($parentFeed)) {
                ...
                if (in_array($typeId, $this->specialShare)) {
                    ...
                    $miniFeed = Phpfox::getService('mobile.feed_api')->processRow(array_merge($parentFeed, $parentFeedItem));
            ...
            if (isset($this->embedTypes[$typeId])) {
                ...
                && ($service = $router->getApiServiceByResourceName($resourceName))) {
