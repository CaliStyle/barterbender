...
class AttachmentApi extends AbstractResourceApi
{
    ...
    public function getAttachmentsBy($itemId, $itemType, $profileId = null)
    {
        ...
        list($iCnt, $attachments) = $this->attachmentService->get($conds, false);
    ...
    {
        $params = $this->resolver->setDefined(['custom_attachment', 'item_id', 'input'])
    ...
    private function processCreate($params)
    {
        ...
        foreach ($_FILES['file']['error'] as $iKey => $sError) {
            if ($sError == UPLOAD_ERR_OK) {
                ...
                if (empty($aValid)) {
                    \Phpfox_Error::set($this->getLocalization()->translate('attachment_does_not_support_any_extension'));
                } else {
                    $iMaxSize = null;
                ...
                return \Phpfox_Error::set('Invalid file upload');
            }
            if ($params['file_type'] == 'photo') {
            ...
            $success[] = [
                'id'  => ($iId ? $iId : $aAttachment['attachment_id']),
                'url' => $fileUrl
            ];
        }
