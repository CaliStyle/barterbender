...
class Ynccomment extends Phpfox_Service
{
    ...
    public function get(
        ...
        foreach ($aRows as $iKey => $aRow) {
            ...
            $aRows[$iKey]['time_stamp'] = Phpfox::getTime(Phpfox::getParam('comment.comment_time_stamp'),
                $aRow['time_stamp']);
            $aRows[$iKey]['posted_on'] = _p('user_link_at_item_time_stamp', array(
                    'item_time_stamp' => Phpfox::getTime(Phpfox::getParam('comment.comment_time_stamp'),
                        $aRow['time_stamp']),
                    'user' => $aRow
                )
            );
            $aRows[$iKey]['update_time'] = Phpfox::getTime(Phpfox::getParam('comment.comment_time_stamp'),
                $aRow['update_time']);
    ...
    private function _getChildren(
        ...
        if (count($aFeedComments)) {
            foreach ($aFeedComments as $iFeedCommentKey => $aFeedComment) {
                if ($iTimStamp != null || !setting('ynccomment_show_replies_on_comment')) {
