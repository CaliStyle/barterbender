...
class Process extends Phpfox_Service
{
    ...
    public function addCommentExtra($iCommentId, $aVals, $aOldVersion = array(), $bIsUpdate = false)
    {
        $bHasAttach = false;
        if (!$iCommentId) {
            return false;
        ...
        $aUpdateType = [];
        if ($bIsUpdate) {
            if(!empty($aOldVersion)) {
                preg_match($sReg, $aOldVersion['text'], $aMatchOld);
                $sOldLink = $aMatchOld[0];
            }
            $aOldExtra = Phpfox::getService('ynccomment')->getExtraByComment($iCommentId, true);
            $aRemovedPreview = Phpfox::getService('ynccomment')->getExtraByComment($iCommentId, true, 'preview');
            ...
            if (count($aOldExtra)) {
                ...
                if ($aOldExtra['extra_type'] != 'preview' && !empty($aVals['attach_changed'])) {
                    ...
                    db()->delete(':ynccomment_comment_extra', 'comment_id =' . (int)$iCommentId);
                }
            }
        }
        //Attach photo
        ...
        if ($bHasAttach) {
            db()->insert(':ynccomment_comment_extra', $aInsert);
            if ($bIsUpdate) {
                db()->delete(':ynccomment_comment_extra',
            ...
            } elseif (!empty($sOldLink) && !count($aMatch)) {
                if ($bIsUpdate) {
                    db()->delete(':ynccomment_comment_extra',
                        'extra_type = \'preview\' AND is_deleted = 0 AND comment_id =' . (int)$iCommentId);
                }
        ...
        if (!empty($aVals['text'])) {
            ...
            foreach ($aEmojis as $aEmoji) {
                if (strpos($aVals['text'], $aEmoji['code']) !== false) {
                    Phpfox::getService('ynccomment.tracking')->addTracking($iCommentId, $iUserId,
                        $aEmoji['emoticon_id'], 'emoticon');
                }
            }
        }
        if (!empty($aVals['sticker_id'])) {
            Phpfox::getService('ynccomment.tracking')->addTracking($iCommentId, $iUserId, $aVals['sticker_id'],
                'sticker');
        }
    }
    ...
    public function delete($iId)
    {
        ...
        $this->database()->delete(Phpfox::getT('ynccomment_previous_versions'), 'comment_id = ' . (int)$iId);
        if ($comment['parent_id']) {
            $this->database()->updateCounter('comment', 'child_total', 'comment_id', $comment['parent_id'], true);
        }
        $aExtra = Phpfox::getService('ynccomment')->getExtraByComment($iId);
        if (count($aExtra)) {
            if ($aExtra['extra_type'] == 'photo') {
                $aParams = Phpfox::getService('ynccomment')->getUploadParamsComment();
                $aParams['type'] = 'ynccomment_comment';
            ...
            //delete extra
            db()->delete(':ynccomment_comment_extra', 'comment_id =' . (int)$iId);
        }
        if (redis()->enabled()) {
    ...
    public function hideComment($iCommentId, $iUserId, $bUnHide = false)
    {
        ...
        if (!$bUnHide) {
            $iHideId = db()->insert(':ynccomment_hide', [
                'user_id' => (int)$iUserId,
                'comment_id' => (int)$iCommentId
            ]);
            return $iHideId;
        } else {
            return db()->delete(':ynccomment_hide',
    ...
    public function removeExtraComment($iCommentId, $sType)
    {
        ...
        if ($sType == 'preview') {
            db()->update(':ynccomment_comment_extra', ['is_deleted' => 1],
    ...
    public function updateText($iId, $aData)
    {
        ...
        if (Phpfox::getService('comment')->hasAccess($iId, 'edit_own_comment', 'edit_user_comment')) {
            ...
            $aVals = $this->database()->select('cmt.*, cmtt.*, ce.extra_id, ce.extra_type, ce.params')
                ...
                ->leftJoin(':ynccomment_comment_extra', 'ce', 'ce.comment_id = cmt.comment_id')
            ...
            //Add edit history
            $bHistory = Phpfox::getService('ynccomment.history')->addEditHistory($iId, $aVals, $aData, $aUpdateAction);
            ...
            $this->database()->update(Phpfox::getT('comment'),
                array('update_time' => $aData['update_time'], "update_user" => Phpfox::getUserBy("full_name")),
                "comment_id = " . (int)$iId);
    ...
    public function moderate($iId, $sAction, $bIsAdmin = false)
    {
        ...
        if ($sAction == 'approve') {
            ...
            if (($bIsAdmin && $aVals['comment_view_id'] == 2)) {
                ...
                Phpfox::getService('notification.process')->add('comment_user_approve', $iId, $aComment['user_id'],
