...
class Process extends Phpfox_Service
{
    ...
    public function addStickerSet($aVals, $bIsUpdate = false)
    {
        ...
        if (!$bIsUpdate) {
            ...
            $iId = $aVals['id'];
            db()->update($this->_sTable, ['title' => $this->preParse()->clean($aVals['title'], 255)],
                'set_id = ' . (int)$iId);
        }
        $this->cache()->removeGroup('ynccomment');
        return $iId;
    }
    ...
    public function updateStickersOrdering($aParams, $iSetId)
    {
        ...
        foreach ($aParams['values'] as $mKey => $mOrdering) {
            ...
            $iCnt++;
            db()->update(':ynccomment_stickers', array('ordering' => $iCnt),
    ...
    public function deleteStickerSet($iSetId)
    {
        ...
        $aSet = Phpfox::getService('ynccomment.stickers')->getForEdit($iSetId);
        if (!$aSet || $aSet['is_default'] || $aSet['view_only']) {
            return false;
        }
        //Remove set from user set
        db()->delete(':ynccomment_user_sticker_set', 'set_id = ' . (int)$iSetId);
        //Remove sticker
        $aSticker = Phpfox::getService('ynccomment.stickers')->getStickersBySet($iSetId);
        if ($aSticker) {
    ...
    public function deleteSticker($iStickerId, $aStickerSet = null, $bCheckThumbnail = true)
    {
        ...
        db()->delete(':ynccomment_comment_track', 'track_type = \'sticker\' AND item_id = ' . (int)$iStickerId);
        if ($aStickerSet == null) {
            $aStickerSet = db()->select('ss.*')
                ->from($this->_sTable, 'ss')
                ->join(':ynccomment_stickers', 's', 's.set_id = ss.set_id')
        ...
        db()->update(':ynccomment_stickers',['is_deleted' => 1], 'sticker_id = ' . $aSticker['sticker_id']);

        db()->updateCounter('ynccomment_sticker_set', 'total_sticker', 'set_id', $aStickerSet['set_id'], true);
        return true;
    }
    ...
    public function setDefaultSet($iSetId, $bUnMark = false)
    {
        ...
        $iTotalDefault = Phpfox::getService('ynccomment.stickers')->countDefaultSet();
        if (!$bUnMark) {
            if ($iTotalDefault >= 2) {
                return false;
            }
    ...
    public function updateMyStickerSet($iSetId, $iUserId, $bIsAdd)
    {
        ...
        if ($bIsAdd) {
            ...
            return db()->delete(':ynccomment_user_sticker_set',
