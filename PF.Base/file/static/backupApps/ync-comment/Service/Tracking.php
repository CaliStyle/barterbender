...
class Tracking extends Phpfox_Service
{
    ...
    {
        $this->_sTable = Phpfox::getT('ynccomment_comment_track');
    }
    ...
    {
        if ($sType == 'emoticon') {
            db()->select('ce.*, ')->join(':ynccomment_emoticon', 'ce', 'ce.emoticon_id = ct.item_id');
        } elseif ($sType == 'sticker') {
            db()->select('s.*, ')
                ->join(':ynccomment_stickers', 's', 's.sticker_id = ct.item_id AND s.is_deleted = 0')
                ->join(':ynccomment_sticker_set', 'ss', 'ss.set_id = s.set_id AND ss.is_active = 1');
        }
