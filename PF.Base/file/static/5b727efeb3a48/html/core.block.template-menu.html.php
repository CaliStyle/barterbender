...
{else}
	...
	{if !Phpfox::isUser()}
		<li class="visible-xs ynrespassion-login-small">
			<div class="login-menu-btns-xs clearfix">
				<div class="{if Phpfox::getParam('user.allow_user_registration') && !Phpfox::getParam('user.invite_only_community')}div01{/if}">
					<a class="btn btn01 btn-primary text-uppercase {if !Phpfox::getParam('core.force_https_secure_pages')}popup{else}no_ajax{/if}" rel="hide_box_title" role="link" href="{url link='login'}">
						<i class="fa fa-sign-in"></i> {_p var='sign in'}
					</a>
				</div>
				{if Phpfox::getParam('user.allow_user_registration') && !Phpfox::getParam('user.invite_only_community')}
				<div class="div02">
					<a class="btn btn02 btn-info text-uppercase {if !Phpfox::getParam('core.force_https_secure_pages')}popup{else}no_ajax{/if}" rel="hide_box_title" role="link" href="{url link='user.register'}">
						{_p var='sign up'}
					</a>
				</div>
				{/if}
	...
	<ul class="nav navbar-nav visible-xs visible-sm site_menu">
		...
		<!-- <li class="">
		 ...
		 <div class="visible-xs item-user-menu-xs">
