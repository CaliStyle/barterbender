...
{if Phpfox::isUser()}
    <ul class="user-sticky-bar-items">
        ...
        {else}
        <li class="user-icon avatar circle" id="hd-user">
            <div class="item-user">
                <div class="item-avatar">{img user=$aGlobalUser suffix='_50_square'}</div>
                <div class="item-name">{$aGlobalUser|user}</div>
            </div>
            <div class="item-home"><a href="{url link=''}">{_p var='home'}</a></div>
        </li>
        <li class="" id="hd-request">
            <a role="button"
...
{else}
    <div class="guest-login-small {if Phpfox::isModule('captcha') && Phpfox::getParam('user.captcha_on_login')}has-captcha{/if}" data-component="guest-actions">
            ...
            <div id="header_menu_login_holder">
                ...
                {if Phpfox::isModule('captcha') && Phpfox::getParam('user.captcha_on_login')}
                    <div class="header_menu_login_button">
                        <a class="btn btn-sm btn-primary btn-captcha {if Phpfox::canOpenPopup('login')}popup{else}no_ajax{/if}"
                ...
                {else}
                    <form method="post" action="{url link='user.login'}">
                        <div class="header_menu_login_left">
                            ...
                            <div><input type="text" name="val[login]" value="" class="header_menu_login_input input-sm" tabindex="1" /></div>
                        </div>
                        <div class="header_menu_login_right">
                            <div class="header_menu_login_label">{_p var='password'}:</div>
                            <div><input type="password" name="val[password]" value="" class="header_menu_login_input input-sm" tabindex="2" autocomplete="off" /></div>
                            <div class="header_menu_login_sub">
                                <a href="{url link='user.password.request'}">{_p var='forgot_your_password'}</a>
                            </div>
                        </div>
                        <div class="header_menu_login_button">
                            <button class="btn btn-primary btn-sm" tabindex="3">{_p var='sign_in'}</button>
                            <a class="btn btn-sm btn-primary btn-responsive {if Phpfox::canOpenPopup('login')}popup{else}no_ajax{/if}"
                               rel="hide_box_title visitor_form" role="link" href="{url link='login'}">
                                {_p var='sign_in'}
                            </a>
                        </div>
