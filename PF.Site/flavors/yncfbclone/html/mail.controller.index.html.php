...
{foreach from=$aMails item=aMail name=mail}
    <article class="mail_holder{if !$bIsSentbox && !$bIsTrash && $aMail.viewer_is_new} mail_is_new{/if} " data-url="{url link='mail.thread' id=$aMail.thread_id}{if $bIsSentbox}view_sent/{/if}" id="js_message_{$aMail.thread_id}">
        <div class=" moderation_row">
                <label class="item-checkbox">
                    <input type="checkbox" class="js_global_item_moderate" name="item_moderate[]" value="{$aMail.thread_id}"/>
