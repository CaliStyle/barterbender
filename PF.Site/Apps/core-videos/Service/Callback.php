...
class Callback extends Phpfox_Service
{
    ...
    public function updateFeedItemInfo($data)
    {
        ...
        // Update video info
        db()->update(Phpfox::getT('video'), ['status_info' => $content, 'privacy' => $iPrivacy], 'video_id = ' . (int)$iItemId);
    }
    ...
    public function sendNotifyToTaggedUsers($params)
    {
        ...
        foreach ($aTagged as $iUserId) {
            ...
            Phpfox::getLib('mail')->to($iUserId)
                ->subject(['user_name_tagged_you_in_video_tittle', ['user_name' => $sTagger, 'title' => $title]])
                ->message(['user_name_tagged_you_in_video_tittle_link',
    ...
    public function getActivityFeed($aItem, $aCallback = null, $bIsChildItem = false)
    {
        ...
        $aRow = db()->select('v.location_latlng, v.location_name, v.user_id, v.video_id, v.title, v.status_info, v.time_stamp, v.total_comment, v.total_view as video_total_view, v.privacy, v.total_like, v.module_id, v.item_id, v.image_path, v.image_server_id, v.is_stream, v.server_id, v.destination, vt.text_parsed as text, ve.video_url, ve.embed_code, v.duration')
            ->from(Phpfox::getT('video'), 'v')
            ->leftJoin(Phpfox::getT('video_text'), 'vt', 'vt.video_id = v.video_id')
        ...
        // get tagged users
        $aReturn['total_friends_tagged'] = Phpfox::getService('feed')->getTaggedUsers($iItemId, $sTypeId, true);
    ...
    public function addComment($aVals, $iUserId = null, $sUserName = null)
    {
        ...
        $aSubject = (Phpfox::getUserId() == $aVideo['user_id'] ? ['full_name_commented_on_gender_video',
            [
                'full_name' => Phpfox::getUserBy('full_name'),
        ...
        $aMessage = (Phpfox::getUserId() == $aVideo['user_id'] ? ['full_name_commented_on_gender_video_message',
            [
                'full_name' => Phpfox::getUserBy('full_name'),
                'gender' => Phpfox::getService('user')->gender($aVideo['gender'], 1),
                'link' => $aVideo['link'],
        ...
        Phpfox::getService('comment.process')->notify([
                ...
                'owner_subject' => ['full_name_commented_on_your_video_title',
                    ['full_name' => Phpfox::getUserBy('full_name'), 'title' => $aVideo['title']]],
                'owner_message' => ['full_name_commented_on_your_video_message',
                    [
                        'full_name' => Phpfox::getUserBy('full_name'),
                        'link' => $aVideo['link'],
                        'title' => $aVideo['title']
        ...
        if ($aVideo['module_id'] == 'user') {
            ...
            Phpfox::getService('comment.process')->notify([
                    ...
                    'notify_id' => 'comment_v',
                    'mass_id' => 'video',
                    'mass_subject' => (Phpfox::getUserId() == $aVideo['item_id'] ? ['full_name_commented_on_gender_video',
                        [
                            'full_name' => Phpfox::getUserBy('full_name'),
                            'gender' => Phpfox::getService('user')->gender($aVideo['gender'], 1)
                        ]] : ['full_name_commented_on_video_full_name_s_video',
                    ...
                    'mass_message' => (Phpfox::getUserId() == $aVideo['item_id'] ? ['full_name_commented_on_gender_video_message',
                        [
                            'full_name' => Phpfox::getUserBy('full_name'),
                            'gender' => Phpfox::getService('user')->gender($aVideo['gender'], 1),
                            'link' => $aVideo['link'],
    ...
    public function getActivityFeedComment($aRow)
    {
        ...
        $aItem = $this->database()->select('v.video_id, v.title, v.time_stamp, v.total_comment, v.total_like, c.total_like, ct.text_parsed AS text, ' . Phpfox::getUserField())
            ->from(Phpfox::getT('comment'), 'c')
