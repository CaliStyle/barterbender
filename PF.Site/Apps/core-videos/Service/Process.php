...
class Process extends Phpfox_Service
{
    ...
    public function addVideo($aVals)
    {
        ...
        if ($sModule == 'pages' && Phpfox::isAppActive('Core_Pages')) {
        ...
        } elseif ($sModule == 'groups' && Phpfox::isAppActive('PHPfox_Groups')) {
        ...
        if (Phpfox::isModule('feed') && !defined('PHPFOX_SKIP_FEED_ENTRY') && ($iViewId == 0) && Phpfox::getParam('v.pf_video_allow_create_feed_when_add_new_item', 1)) {
            $iFeedId = Phpfox::getService('feed.process')->callback($aCallback)->add('v', $iId, $aVals['privacy'], 0,
                ($aCallback === null ? (isset($aVals['parent_user_id']) ? $aVals['parent_user_id'] : 0) : $aVals['callback_item_id']), $iUserId);
        }

        // notification to tagged and mentioned friends
        $this->notifyTaggedInFeed($aVals['status_info'], $iId, $iUserId, $iFeedId, $aVals['tagged_friends'], $aVals['privacy'], (isset($aVals['parent_user_id']) ? (int)$aVals['parent_user_id'] : 0), $sModule);
    ...
    {
        $aVideo = db()->select('v.video_id, v.module_id, v.item_id, v.user_id, v.destination, v.image_path, v.server_id, v.image_server_id, v.is_sponsor, v.is_featured, v.view_id')
            ->from($this->_sTable, 'v')
    ...
    {
        Phpfox::isUser(true);
        Phpfox::getUserParam('pf_video_feature', 0, null, true);
    ...
    {
        Phpfox::isUser(true);
        Phpfox::getUserParam('pf_video_approve', '0', null, true);
