...
use Apps\P_AdvEventAPI\Api\Form\EventInviteForm;
...
class EventInviteApi extends AbstractResourceApi
{
    ...
    private function processCreate($aVals)
    {
        ...
        if (isset($aVals['emails'])) {
            ...
            foreach ($aEmails as $sEmail) {
                ...
                $sLink = Phpfox_Url::instance()->permalink('fevent', $iId, $aEvent['title']);

                $sMessage = $this->getLocalization()->translate('full_name_invited_you_to_the_title', array(
                ...
                if (!empty($aVals['personal_message'])) {
                    $sMessage .= "\n\n" . $this->getLocalization()->translate('mobile_full_name_added_the_following_personal_message', array(
                                'full_name' => Phpfox::getUserBy('full_name')
                            )
                        ) . "\n";
                ...
                $bSent = $oMail->to($sEmail)
                    ->subject(array(
                        'full_name_invited_you_to_the_event_title',
                        array(
                            'full_name' => Phpfox::getUserBy('full_name'),
                            'title' => $oParseInput->clean($aEvent['title'], 255)
                        )
                ...
                if ($bSent) {
                    ...
                    $this->database()->insert(':fevent_invite', array(
                            'event_id' => $iId,
                            'type_id' => 1,
        ...
        if (isset($aVals['user_ids']) && is_array($aVals['user_ids'])) {
            ...
            foreach ($aUsers as $aUser) {
                ...
                $sLink = Phpfox_Url::instance()->permalink('fevent', $aEvent['event_id'], $aEvent['title']);
                ...
                $bSent = Phpfox::getLib('mail')->to($aUser['user_id'])
                    ->subject(array(
                        'fevent.full_name_invited_you_to_the_event_title',
                        array(
                            'full_name' => Phpfox::getUserBy('full_name'),
                            'title' => $oParseInput->clean($aEvent['title'], 255)
                        )
                ...
                (Phpfox::isModule('request') ? Phpfox::getService('request.process')->add('fevent_invite', $iId,
                    $aUser['user_id']) : null);
            }
        }
    ...
    public function updateRsvp($params)
    {
        ...
        if (isset($params['rsvp']) && $params['rsvp'] == 0) {
            $this->processService->removeInvite($params['id'], $params['rsvp']);
            return $this->success([
                'rsvp' => $params['rsvp']
            ]);
        } else {
    ...
    public function createAccessControl()
    {
        ...
        if ($moduleId && $itemId) {
            $this->accessControl->setAppContext(AppContextFactory::create($moduleId, $itemId));
        }
    }
