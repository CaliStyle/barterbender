...
{if defined('PHPFOX_IS_USER_PROFILE') && Phpfox::isUser() && Phpfox::getUserId() == $aForms.user_id}
    <li>
...
{if isset($aParentModule) && defined('PHPFOX_IS_PAGES_VIEW') && $aForms.canSetCover}
    <li>
        <a href="#" class="photo_make_as_cover" onclick="$Core.Photo.setCoverPhoto({$aForms.photo_id}, {$aParentModule.item_id}, '{$aParentModule.module_id}'); return false;" >
            <i class="ico ico-photo"></i>{_p var='set_as_page_s_cover_photo'}
        </a>
    </li>
    <li role="separator" class="divider"></li>
{/if}
...
{elseif $aForms.canPurchaseSponsor}
    ...
    <li id="js_sponsor_purchase_{$aForms.photo_id}">
        <a href="#" onclick="$.ajaxCall('photo.sponsor','photo_id={$aForms.photo_id}&type=0'); return false;">
            <i class="ico ico-sponsor mr-1"></i>{_p var='unsponsor_this_photo'}
        </a>
    </li>
    {else}
