    ...
    function buildForm($options = null, $data = [])
    {
        ...
        $this
            ...
            ->addField('answers', PollAnswerType::class, [
                'max_answers'   => $this->setting->getUserSetting('poll.maximum_answers_count') >= 2 ? $this->setting->getUserSetting('poll.maximum_answers_count') : 2,
                'min_answers'   => 2,
                'returnKeyType' => 'next',
                'required'      => true
        ...
        if ($this->setting->getUserSetting('poll.poll_can_upload_image')) {
            $this
                ->addField('file', FileType::class, [
                    ...
                    'item_type'           => 'poll',
                    'file_type'           => 'photo',
                    'required'            => !!$this->setting->getAppSetting('poll.is_image_required'),
                    'preview_url'         => $this->getPreviewImage(),
                    'value'               => !empty($this->getPreviewImage()) ? ['status' => FileType::UNCHANGED, 'temp_file' => null] : '',
                    'max_upload_filesize' => $this->getSizeLimit($this->setting->getUserSetting('poll.poll_max_upload_size'))
                ], null, $sectionName);
        }
        $this
        ...
        if (empty($this->data['item_id']) && empty($this->itemId)) {
            ...
            ], $sectionName, $this->privacy->getValue('poll.default_privacy_setting'));
        }
        $this
            ->addModuleFields([
