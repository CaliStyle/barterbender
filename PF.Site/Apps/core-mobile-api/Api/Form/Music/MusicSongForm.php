    ...
    function buildForm($options = null, $data = [])
    {
        ...
        if ($this->editing) {
            ...
            $this->addField('files', MultiFileType::class, [
                'label'               => 'songs',
                'min_files'           => 1,
                'max_files'           => $this->setting->getUserSetting('music.max_songs_per_upload'),
                'item_type'           => 'music_song',
                'file_type'           => 'music',
                'required'            => true,
                'max_upload_filesize' => $this->getSizeLimit($this->setting->getUserSetting('music.music_max_file_size'), true)
        ...
        if (empty($this->data['item_id'])) {
            ...
            ], $sectionName, $this->privacy->getValue('music.default_privacy_setting_song'));
        }
