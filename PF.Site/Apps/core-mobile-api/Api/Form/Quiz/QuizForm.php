    ...
    function buildForm($options = null, $data = [])
    {
        ...
        $this
            ...
            ->addField('questions', QuizQuestionType::class, [
                'label'         => 'question',
                'min_questions' => $this->setting->getUserSetting('quiz.min_questions'),
                'max_questions' => $this->setting->getUserSetting('quiz.max_questions'),
                'max_answers'   => $this->setting->getUserSetting('quiz.max_answers'),
                'min_answers'   => $this->setting->getUserSetting('quiz.min_answers'),
                'required'      => true
        ...
        if ($this->setting->getUserSetting('quiz.can_upload_picture')) {
            $this->addField('file', FileType::class, [
                'label'               => 'banner',
                'multiple'            => false,
                'item_type'           => 'quiz',
                'file_type'           => 'photo',
                'required'            => $this->setting->getUserSetting('quiz.is_picture_upload_required'),
                'preview_url'         => $this->getPreviewImage(),
                'value'               => !empty($this->getPreviewImage()) ? ['status' => FileType::UNCHANGED, 'temp_file' => null] : '',
                'max_upload_filesize' => $this->getSizeLimit($this->setting->getUserSetting('quiz.quiz_max_upload_size'))
            ], null, $sectionName);
        }
        ...
        $this
            ...
            ], $sectionName, $this->privacy->getValue('quiz.default_privacy_setting'));
