...
class UserSearchForm extends SearchForm
{
    ...
    {
        if ($this->setting->getUserSetting('user.can_search_user_gender')) {
            $this->addField('gender', RadioType::class, [
                'label'   => 'browse_for',
                'options' => $this->genderOptions()
            ]);
        }
        if ($this->setting->getUserSetting('user.can_search_user_age')) {
        ...
        if ($this->setting->getUserSetting('user.can_search_by_zip')) {
    ...
    private function genderOptions()
    {
        ...
        $options = [
            [
                'value' => 0,
                'label' => $this->local->translate('all_members')
            ]
        ];
