...
class ForumPostApi extends AbstractResourceApi implements ActivityFeedInterface
{
    ...
    function findAll($params = [])
    {
        ...
        $isThreadDetail = !empty($params['thread']) ? true : false;
        ...
        if (!$isThreadDetail) {
            ...
            if (!empty($params['tag'])) {
                ...
                if ($parentModule) {
                    ...
                    $this->search()->setCondition("AND ft.group_id = " . (int)$parentModule['item_id'] . " AND tag.tag_url = '" . Phpfox_Database::instance()->escape(urldecode($this->request()->get('req5'))) . "'");
                } else {
                    $this->search()->setCondition("AND ft.group_id = 0 AND tag.tag_url = '" . Phpfox_Database::instance()->escape(urldecode($this->request()->get('req3'))) . "'");
                }
            }
            if ($params['q']) {
                $isSearch = true;
    ...
    private function processCreate($values)
    {
        ...
        if (isset($values['module_id'])
            && Phpfox::isModule($values['module_id'])
            && isset($values['item_id'])
            && Phpfox::hasCallback($values['module_id'], 'addForum')
        ) {
            $aCallback = Phpfox::callback($values['module_id'] . '.addForum', $values['module_id']);

            if ($aCallback === false) {
                return $this->permissionError($this->getLocalization()->translate('only_members_can_add_a_reply_to_threads'));
            }
        }
        $aThread = Phpfox::getService('forum.thread')->getActualThread($values['thread_id'], $aCallback);
        ...
        $values['forum_id'] = $aThread['forum_id'];
        if (!empty($values['attachment'])) {
            $values['attachment'] = implode(",", $values['attachment']);
        }
        return $this->postProcessService->add($values, $aCallback);
    }
    ...
    function delete($params)
    {
        ...
        if ($canDelete && $this->postProcessService->delete($params['id'])) {
            return $this->success([], [], $this->getLocalization()->translate('Post deleted successfully'));
        }
        return $this->permissionError();
    }
