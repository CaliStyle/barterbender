...
class ForumThreadApi extends AbstractResourceApi
{
    ...
    private function processCreate($values)
    {
        ...
        if (!empty($values['item_id']) && !empty($values['module_id'])) {
            $callback = Phpfox::callback($values['module_id'] . '.addForum', $values['item_id']);
            if (!$callback) {
                return $this->error($this->getLocalization()->translate('Cannot find the parent item.'));
            }
        }
    ...
    function loadResourceById($id, $returnResource = false)
    {
        ...
        if (!isset($thread['is_subscribed'])) {
            $thread['is_subscribed'] = !!$this->database()->select('subscribe_id')->from(':forum_subscribe')->where('thread_id = ' . (int)$thread['thread_id'] . ' AND user_id = ' . $this->getUser()->getId())->execute('getField');
        }
        if ($returnResource) {
    ...
    {
        $forumId = $this->resolver->resolveSingle($params, 'forum');
    ...
    protected function setHyperlinks(ForumThreadResource $resource, $includeLinks = false)
    {
        $resource->setSelf([
    ...
    function sponsor($params)
    {
        ...
        $item = $this->loadResourceById($id, true);
        $this->denyAccessUnlessGranted(ForumThreadAccessControl::SPONSOR, $item);
