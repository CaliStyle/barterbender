...
class AdConfigService extends Phpfox_Service
{
    ...
    public function getAdsConfigs($aCond, $iPage = 1, $iLimit = 10, &$iCnt)
    {
        $aCond[] = 'AND 1=1';
    ...
    private function convertAdConfig($data, $bForEdit = false)
    {
        if (count($data)) {
            foreach ($data as $key => $val) {
                ...
                $data[$key]['disallow_access'] = empty($val['disallow_access']) ? null : unserialize($val['disallow_access']);
            }
        }
        return $data;
    }
    ...
    public function getAllConfigsToSetting($aSettings)
    {
        ...
        if (count($aConfigs)) {
            ...
            foreach ($aConfigs as $key => $aConfig) {
                ...
                //Disallow user group
                if (in_array($userGroupId, $aConfig['disallow_access'])) {
                    continue;
                }
