...
class CommentApi extends AbstractResourceApi implements MobileAppSettingInterface
{
    ...
    public function delete($params)
    {
        ...
        if (empty($params['item_type']) || empty($params['item_id'])) {
            ...
            $comment = $this->loadResourceByType($params['item_type'], $params['item_id']);
        }
        if (!$comment) {
    ...
    public function getReply($params)
    {
        ...
        if ($this->bIsNewComment) {
            foreach ($reply as $key => $comment) {
                ...
                if ($reply[$key - 1]['is_hidden'] && $reply[$key]['is_hidden']) {
    ...
    {
        $params = $this->resolver->setDefined(['item_type', 'item_id', 'parent_id', 'last_id', 'limit'])
        ...
        if ($recursive) {
            foreach ($comments as $key => $comment) {
                if ($this->bIsNewComment) {
                    ...
                    if ($comments[$key - 1]['is_hidden'] && $comments[$key]['is_hidden']) {
    ...
    function update($params)
    {
        ...
        if ($comment) {
            ...
            if ($this->processUpdate($params['id'], $params['text'], $comment->getItemType())) {
                $text = preg_replace('/([^>\n]?)(\n)/', '&#10;', $params['text']);
    ...
    public function loadResourceByType($type, $itemId)
    {
        $comment = $this->database()
            ...
            ->leftJoin(Phpfox::getT('user'), 'u', 'u.user_id = c.user_id')
            ->leftJoin(Phpfox::getT('comment_text'), 'ct', 'ct.comment_id = c.comment_id')
            ->where('c.type_id = "' . (int)$type . '" AND c.item_id = ' . (int)$itemId)
            ->execute("getRow");
        if (empty($comment)) {
            return null;
        }
        return CommentResource::populate($comment);
    }

    /**
     * @return \Comment_Service_Process|object
     */
     ...
     * @throws \Exception
     */
    private function processUpdate($id, $text, $type)
    {
        ...
        return $this->processService()->updateText($id, $text);
    }
