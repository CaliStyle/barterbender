...
class BrowseHelper extends \Phpfox_Service
{
    ...
    public function execute(\Closure $callback = null)
    {
        ...
        $this->_oBrowse->query();
        $this->_aRows = $this->database()->select(Phpfox::getUserField() . "," . $this->_aParams['alias'] . '.*' . (isset($this->_aParams['select']) ? ',' . $this->_aParams['select'] : ''))
    ...
    {
        return PsrRequestHelper::instance();
    }
