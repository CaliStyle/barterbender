...
class BlogApi extends AbstractResourceApi implements ActivityFeedInterface, MobileAppSettingInterface
{
    ...
    function findAll($params = [])
    {
        ...
        switch ($view) {
            ...
            case 'my':
                ...
                break;
            case 'draft':
                $this->search()->setCondition("AND blog.user_id = " . (int)$this->getUser()->getId() . " AND blog.is_approved IN(" . ($user['user_id'] == $this->getUser()->getId() ? '0,1' : '1')
                    . ") AND blog.privacy IN(" . (Phpfox::getParam('core.section_privacy_item_browsing') ? '%PRIVACY%' : Phpfox::getService('core')->getForBrowse($user))
        ...
        if (!empty($params['category'])) {
            if ($aBlogCategory = $this->categoryService->getCategory($params['category'])) {
                $this->search()->setCondition('AND blog_category.category_id = ' . (int)$params['category'] . ' AND blog_category.user_id = ' . ($isProfile ? (int)$user['user_id'] : 0));
            }
        }
        ...
        if ($params['when']) {
            ...
            switch ($params['when']) {
                case 'today':
                    ...
                    $this->search()->setCondition(' AND (blog.time_stamp >= \'' . Phpfox::getLib('date')->convertToGmt($iTimeDisplay) . '\' AND blog.time_stamp < \'' . Phpfox::getLib('date')->convertToGmt($iEndDay) . '\')');
                    break;
                case 'this-week':
                    $this->search()->setCondition(' AND blog.time_stamp >= ' . (int)Phpfox::getLib('date')->convertToGmt(Phpfox::getLib('date')->getWeekStart()));
                    $this->search()->setCondition(' AND blog.time_stamp <= ' . (int)Phpfox::getLib('date')->convertToGmt(Phpfox::getLib('date')->getWeekEnd()));
                    break;
                case 'this-month':
                    $this->search()->setCondition(' AND blog.time_stamp >= \'' . Phpfox::getLib('date')->convertToGmt(Phpfox::getLib('date')->getThisMonth()) . '\'');
                    $iLastDayMonth = Phpfox::getLib('date')->mktime(0, 0, 0, date('n'), Phpfox::getLib('date')->lastDayOfMonth(date('n')), date('Y'));
                    $this->search()->setCondition(' AND blog.time_stamp <= \'' . Phpfox::getLib('date')->convertToGmt($iLastDayMonth) . '\'');
                    break;
                default:
                    break;
            }
    ...
    private function processUpdate($id, $values, $item)
    {
        ...
        $userId = $item['user_id'];
        $this->processService->update($id, $userId, $values, $item);
        return true;
    }
    ...
    function sponsor($params)
    {
        ...
        $item = $this->loadResourceById($id, true);
        $this->denyAccessUnlessGranted(BlogAccessControl::SPONSOR, $item);
