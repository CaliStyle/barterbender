...
class MusicPlaylistApi extends AbstractResourceApi implements ActivityFeedInterface
{
    ...
    function findAll($params = [])
    {
        ...
        switch ($params['when']) {
            case 'today':
                ...
                $this->search()->setCondition(' AND (mp.time_stamp >= ' . Phpfox::getLib('date')->convertToGmt($startTime) . ' AND mp.time_stamp <= ' . Phpfox::getLib('date')->convertToGmt($endTime) . ')');
                break;
            case 'this-week':
                $this->search()->setCondition(' AND mp.time_stamp >= ' . (int)Phpfox::getLib('date')->convertToGmt(Phpfox::getLib('date')->getWeekStart()));
                $this->search()->setCondition(' AND mp.time_stamp <= ' . (int)Phpfox::getLib('date')->convertToGmt(Phpfox::getLib('date')->getWeekEnd()));
                break;
            case 'this-month':
                $this->search()->setCondition(' AND mp.time_stamp >= ' . Phpfox::getLib('date')->convertToGmt(Phpfox::getLib('date')->getThisMonth()));
                $lastDayMonth = Phpfox::getLib('date')->mktime(23, 59, 59, Phpfox::getTime('n'), Phpfox::getLib('date')->lastDayOfMonth(Phpfox::getTime('n')), date('Y'));
                $this->search()->setCondition(' AND mp.time_stamp <= ' . Phpfox::getLib('date')->convertToGmt($lastDayMonth));
                break;
            default:
                break;
    ...
    function findOne($params)
    {
        ...
        if (Phpfox::isModule('privacy') && !Phpfox::getService('privacy')->check('music_playlist', $item['playlist_id'], $item['user_id'], $item['privacy'],
                $item['is_friend'], true)
        ) {
            return $this->permissionError();
        }
    ...
    public function removeSong($params)
    {
        ...
        if ($this->processService->removeSong($songId, $id)) {
            ...
            ], [], $this->getLocalization()->translate('remove_song_from_playlist_successfully'));
        }
    }
