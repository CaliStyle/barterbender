...
class FriendApi extends AbstractResourceApi implements MobileAppSettingInterface, ActivityFeedInterface
{
    ...
    function findAll($params = [])
    {
        ...
        $params = $this->resolver
            ->setDefined(['limit', 'q', 'page', 'user_id', 'list_id', 'format', 'is_add_list', 'view'])
            ->setDefault([
                'page'        => 1,
                'limit'       => 50,
                'is_add_list' => false,
    ...
    private function getFriendShip($userId, $values)
    {
        ...
        if ($currentUser && Phpfox::isModule('friend') && $currentUser != $userId) {
            $values['is_friend'] = ($this->getFriendService()->isFriend($currentUser, $userId) ? true : false);
            $values['is_friend_of_friend'] = ($this->getFriendService()->isFriendOfFriend($userId) ? true : false);
            if (!$values['is_friend']) {
                $iRequestId = Phpfox::getService('friend.request')->isRequested($currentUser, $userId, true);
                if ($iRequestId) {
                ...
                if (!$values['is_friend_request']) {
                    $iRequestId = Phpfox::getService('friend.request')->isRequested($values['user_id'], $currentUser, true);
    ...
    public function searchFriend($params)
    {
        ...
        if (count($friends)) {
            ...
            foreach ($friends as $key => $friend) {
                $data = UserResource::populate($friend)->toArray(['id', 'full_name', 'avatar', 'is_featured']);
                $data['disable'] = false;
