...
class FeedApi extends AbstractResourceApi implements MobileAppSettingInterface
{
     ...
     * @return FeedResource
     */
    public function processRow($feed)
    {
        /** @var FeedResource $resource */
        $resource = $this->populateResource(FeedResource::class, $feed);
        $resource->setEmbedObject($this->feedAttachmentHelper->forListing($feed, $resource));
    ...
    function findAll($params = [])
    {
        $params = $this->resolver
            ->setDefined(['user_id', 'limit', 'page', 'item_type', 'item_id', 'hashtag', 'last_feed_id'])
            ->setDefault([
        ...
        if ($params['user_id']) {
            ...
            if (Phpfox::getService('user.block')->isBlocked(null, $params['user_id'])
                ...
                if ($params['page'] == 1) {
                    return $this->permissionError($this->getLocalization()->translate('sorry_information_of_this_user_isn_t_available_for_you'));
                }
                return $this->success([]);
            }
    ...
    function findOne($params)
    {
        ...
        if (!empty($feed['type_id']) && !empty($feed['item_id'])) {
            if (Phpfox::hasCallback($feed['type_id'], 'canViewItem')) {
                ...
                $item = NameResource::instance()->getPermissionByResourceName(str_replace('_', '-', $typeId), $feed['item_id']);
                if ($item !== null && !$item) {
                    return $this->permissionError();
                } else {
                    $error = false;
                    ...
                    if ($feed['user_id'] != $userId) {
                        switch ($feed['privacy']) {
                            ...
                            case 2:
                                ...
                                if (isset($userObject['is_friend']) && $userObject['is_friend']) {
                                    break;
        ...
        $feed['is_detail'] = true;
        $item = $this->processRow($feed)->toArray();
    ...
    function create($params)
    {
        ...
        if ($form->isValid() && ($values = $form->getValues())) {
            if (!empty($values['post_as_parent']) && version_compare(Phpfox::getVersion(), "4.7", ">=")) {
                $_REQUEST['custom_pages_post_as_page'] = $values['parent_item_id'];
            }
    ...
    private function createPhotoPost($photoValues, $params = [])
    {
        ...
        return $this->parametersBag->get('feed_id');
    }
    ...
    private function createVideoPost($videoValues, $params = [])
    {
        ...
        if (!empty($params['location'])) {
            ...
            $videoValues['location_latlng'] = $params['location']['latlng'];
        }
    ...
    private function createLinkPost($linkValues, $params = [])
    {
        ...
        $id = Phpfox::getService('link.process')->add($params, false, $aCallback);
    ...
    public function getPostingCallback($type, $item, $posterId = null)
    {
        ...
        if ($type == "pages") {
            ...
            if (!isset($aPage['page_id'])) {
                return $this->error("unable_to_find_the_page_you_are_trying_to_comment_on");
            }
            ...
            if (!isset($aEvent['event_id'])) {
                return $this->error('unable_to_find_the_event_you_are_trying_to_comment_on');
            }
        ...
        if ($form->isValid() && ($values = $form->getValues())) {
            switch ($values['post_type']) {
                ...
                case FeedPostForm::PHOTO_POST:
                    ...
                    break;
                default:
                    if ($values == FeedPostForm::STATUS_POST && Phpfox::getLib('parse.format')->isEmpty($values['user_status'])) {
                        return $this->error($this->getLocalization()->translate('add_some_text_to_share'));
                    }
    ...
    function delete($params)
    {
        ...
        if ($result) {
            ...
            return $this->error("Delete feed failed");
        }
    ...
    public function shareFeed($post)
    {
        ...
        if ($form->isValid() && $post = $form->getValues()) {
            if ($post['post_type'] == 'friend') {
                ...
                if (!empty($feed) && !empty($feed['parent_module_id']) && $feed['parent_feed_id']) {
    ...
    public function getStatusForEdit($params)
    {
        ...
        if ((in_array($feed['type_id'], ['user_status', 'link', 'photo', 'v'])
            ...
            $item = [
                ...
                'item' => [
                    'status_text' => html_entity_decode($feed['feed_status'], ENT_QUOTES),
                    'id' => (int)$feed['feed_id'],
                    ...
                    'privacy_options' => (new PrivacyType())->getDefaultPrivacy(),
    ...
    public function getFeedIdByItem($itemId, $typeId, $module = null)
    {
        ...
        $feedId = $this->database()->select('feed_id')
    ...
    {
        $items = $this->getFeedService()->getTaggedUsers($item_id, $item_type, false, 1, 100);
        $results = [];
        if (count($items)) {
            $results = array_map(function ($user) {
                return UserResource::populate($user)->toArray(['id', 'resource_name', 'full_name', 'avatar']);
            }, $items);
        }
    ...
    public function getTaggedFriends($params)
    {
        ...
        $items = $this->getFeedService()->getTaggedUsers($params['item_id'], $params['item_type'], false, $params['page'], $params['limit']);
        $results = [];
        if (count($items)) {
            $results = array_map(function ($user) {
                $resource = UserResource::populate($user);
                return $resource->setExtra((new UserApi())->getAccessControl()->getPermissions($resource))->toArray(['id', 'resource_name', 'full_name', 'avatar', 'is_owner', 'friendship', 'statistic', 'friend_id', 'is_featured', 'extra']);
            }, $items);
        }
    ...
    public function hideAllFromObject($params)
    {
        ...
        $userId = $this->getUser()->getId();
        if (!$userId || $id == $userId) {
            return $this->permissionError($this->getLocalization()->translate('please_sign_in_to_continue_this_action'));
        }
    ...
    public function unHideOneFeed($params)
    {
        ...
        if (Phpfox::getService('feed.hide')->delete($userId, $id, 'feed')) {
            return $this->success([]);
        }
        return $this->error();
    }
