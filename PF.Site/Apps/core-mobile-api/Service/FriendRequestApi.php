...
class FriendRequestApi extends AbstractResourceApi
{
    ...
    {
        $friendId = $this->resolver
            ->setRequired(['friend_user_id'])
            ->resolveSingle($params, 'friend_user_id', 'int', ['min' => 1]);

        $this->denyAccessUnlessGranted(UserAccessControl::ADD_FRIEND);
        ...
        if ($success && $this->isPassed()) {
            return $this->success([
                'friendship' => $this->friendDirection == self::ONE_WAY ? 1 : 3
            ], [], $this->getLocalization()->translate($this->friendDirection == self::ONE_WAY ? 'add_friend_successfully' : 'the_request_has_been_sent_successfully'));
        }
    ...
    {
        $request = $this->getRequestService()->getRequest($id);
        return $request;
    }

    private function loadMyRequestByFriendId($friendId)
    {
    ...
    private function loadFriendRequestToMe($friendId)
    {
        return $this->database()->select('*')
            ->from(':friend_request')
    ...
    private function processCreateFriendRequest($friendId)
    {
        $userId = $this->getUser()->getId();
     ...
     */
    private function getFriendProcessService()
    {
