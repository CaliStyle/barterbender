...
class DeviceService extends Phpfox_Service
{
    ...
    public function executePushNotification($queue = null)
    {
        ...
        if (!empty($tokens)) {
            ...
            if (Phpfox::getService(PushNotificationInterface::class)->pushNotification($queue['receiver_id'], array_merge($result, ['tokens' => $tokens]))) {
                //Sent Success
                return true;
            } else {
                //Sent Fail
    ...
    private function getNotificationDetail($data)
    {
        ...
        switch ($data['notification_type']) {
            case 'friend':
                $result = [
                    'title'         => $this->getLocalization()->translate('new_friend_request', [], $user['language_id']),
                    'message'       => $this->getLocalization()->translate('full_name_added_you_as_a_friend', ['full_name' => $user['full_name']], $user['language_id']),
            ...
            case 'mail':
                ...
                $result = [
                    'title'         => $this->getLocalization()->translate('full_name_sent_you_a_message', ['full_name' => $user['full_name']], $user['language_id']),
                    'message'       => $message,
                    'resource_link' => ""
                ];
                break;
            default:
                $notification = $this->database()->select('n.*, n.user_id as item_user_id, ' . Phpfox::getUserField())
                    ...
                    ->execute('getRow');
                Phpfox::getService('user.auth')->setUserId($notification['item_user_id']);
                ...
                if (!empty($callback)) {
                    ...
                    $notificationResource = (new NotificationApi())->processRow($callback);
