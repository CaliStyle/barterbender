...
class MusicSongApi extends AbstractResourceApi implements ActivityFeedInterface
{
        ...
        if ($params['when']) {
            ...
            switch ($params['when']) {
                case 'today':
                    ...
                    $this->search()->setCondition(' AND (m.time_stamp >= \'' . Phpfox::getLib('date')->convertToGmt($iTimeDisplay) . '\' AND m.time_stamp < \'' . Phpfox::getLib('date')->convertToGmt($iEndDay) . '\')');
                    break;
                case 'this-week':
                    $this->search()->setCondition(' AND m.time_stamp >= ' . (int)Phpfox::getLib('date')->convertToGmt(Phpfox::getLib('date')->getWeekStart()));
                    $this->search()->setCondition(' AND m.time_stamp <= ' . (int)Phpfox::getLib('date')->convertToGmt(Phpfox::getLib('date')->getWeekEnd()));
                    break;
                case 'this-month':
                    $this->search()->setCondition(' AND m.time_stamp >= \'' . Phpfox::getLib('date')->convertToGmt(Phpfox::getLib('date')->getThisMonth()) . '\'');
                    $iLastDayMonth = Phpfox::getLib('date')->mktime(0, 0, 0, date('n'), Phpfox::getLib('date')->lastDayOfMonth(date('n')), date('Y'));
                    $this->search()->setCondition(' AND m.time_stamp <= \'' . Phpfox::getLib('date')->convertToGmt($iLastDayMonth) . '\'');
                    break;
                default:
                    break;
            }
    ...
    function findOne($params)
    {
        ...
        if (!empty($item['module_id'])) {
            if ($aCallback = Phpfox::callback($item['module_id'] . '.getMusicDetails', $item)) {
        ...
        if (Phpfox::isModule('privacy') && !Phpfox::getService('privacy')->check('music_song', $item['song_id'], $item['user_id'], $item['privacy'],
                $item['is_friend'], true)) {
            return $this->permissionError();
        }
        $updateCounter = false;
    ...
    private function processCreate($values)
    {
        ...
        foreach ($uploadList as $file) {
            ...
            Phpfox::getService('core.temp-file')->delete($file['temp_file']);
        }
        if (empty($finalVals)) {
            return $this->error();
        }
    ...
    function getFeedDisplay($feed, $item)
    {
        ...
        $extraSongId = intval($item['extra_song_id']);
    ...
    function sponsor($params)
    {
        ...
        $this->denyAccessUnlessGranted(MusicSongAccessControl::SPONSOR, $item);
