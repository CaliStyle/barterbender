...
class AccountApi extends AbstractResourceApi
{
    ...
    public function updateEmailNotificationSettings($params)
    {
        ...
        //Remove old
        if ($val) {
            $this->database()->delete(':user_notification', 'user_id = ' . $userId . ' AND user_notification = \'' . $var . '\'');
        } else {
    ...
    public function findAllBlockedUser()
    {
        ...
        if ($blocked) {
            $results = array_map(function ($item) {
                ...
                return BlockedUserResource::populate($item)->toArray(['id', 'resource_name', 'module_name', 'full_name', 'avatar', 'is_blocked', 'is_featured']);
            }, $blocked);
        }
        return $this->success($results);
    }
    ...
    private function getProfileSettingOptions($profile = [])
    {
        $options = [];
        $allowOptions = [];
    ...
    private function checkValidVar($var, $val, $accept, $extra = [])
    {
