...
class GroupMemberApi extends AbstractResourceApi
{
    ...
    function findAll($params = [])
    {
        ...
        if (!$group || $group['view_id'] == '2' || ($group['view_id'] != '0' && !$this->groupService->canModerate() && ($this->getUser()->getId() != $group['user_id']))) {
        ...
        if ($group['reg_method'] == 2 && !$this->groupService->isMember($group['page_id']) && !Phpfox::isAdmin() && !$this->groupService->isInvited($group['page_id'])) {
            return $this->permissionError();
        }
        switch ($params['view']) {
            case 'all':
    ...
    function create($params)
    {
        ...
        if ($group['reg_method'] == 2 && !$isInvited) {
            return $this->permissionError();
        }
    ...
    {
        if ($group['reg_method'] == 0 || $isInvited) {
    ...
    public function approveMemberRequest($params)
    {
        ...
        $signup = $this->database()->select('p.*, ps.user_id AS post_user_id')
