    ...
    protected function checkPassword($user, $password)
    {
        ...
        switch ($loginType) {
            ...
            case 'user_name':
                ...
                break;
            default:
                $condition = 'email = \'' . $user['email'] . '\' or user_name = \'' . $user['email'] . '\'';
                break;
        }
        $rawUser = Phpfox::getLib("database")->select("status_id, email")
            ->from(":user")
            ->where($condition)
            ->execute('getRow');
        ...
        if ($rawUser['status_id'] == 1) {
            return self::UN_VERIFY_STATUS;
        }

        list($result, $user) = Phpfox::getService('user.auth')->login($rawUser['email'], $password);
