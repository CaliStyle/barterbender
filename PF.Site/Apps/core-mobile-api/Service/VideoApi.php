...
class VideoApi extends AbstractResourceApi implements MobileAppSettingInterface, ActivityFeedInterface
{
    ...
    function findAll($params = [])
    {
        $params = $this->resolver->setDefined([
            'view', 'q', 'sort', 'profile_id', 'limit', 'page', 'when', 'module_id', 'item_id', 'category'
        ])
        ...
        if ($params['when']) {
            ...
            switch ($params['when']) {
                case 'today':
                    ...
                    $this->search()->setCondition(' AND (video.time_stamp >= \'' . Phpfox::getLib('date')->convertToGmt($iTimeDisplay) . '\' AND video.time_stamp < \'' . Phpfox::getLib('date')->convertToGmt($iEndDay) . '\')');
                    break;
                case 'this-week':
                    $this->search()->setCondition(' AND video.time_stamp >= ' . (int)Phpfox::getLib('date')->convertToGmt(Phpfox::getLib('date')->getWeekStart()));
                    $this->search()->setCondition(' AND video.time_stamp <= ' . (int)Phpfox::getLib('date')->convertToGmt(Phpfox::getLib('date')->getWeekEnd()));
                    break;
                case 'this-month':
                    $this->search()->setCondition(' AND video.time_stamp >= \'' . Phpfox::getLib('date')->convertToGmt(Phpfox::getLib('date')->getThisMonth()) . '\'');
                    $iLastDayMonth = Phpfox::getLib('date')->mktime(0, 0, 0, date('n'), Phpfox::getLib('date')->lastDayOfMonth(date('n')), date('Y'));
                    $this->search()->setCondition(' AND video.time_stamp <= \'' . Phpfox::getLib('date')->convertToGmt($iLastDayMonth) . '\'');
                    break;
                default:
                    break;
            }
    ...
    function findOne($params)
    {
        ...
        if (isset($item['module_id']) && $item['module_id'] != 'video' && !empty($item['item_id']) && Phpfox::isModule($item['module_id'])) {
            if ($item['module_id'] == 'pages' && !Phpfox::getService('pages')->hasPerm($item['item_id'],
                    'pf_video.view_browse_videos')) {
                return $this->permissionError();
            } else {
                if (Phpfox::hasCallback($item['module_id'],
                        'checkPermission') && !Phpfox::callback($item['module_id'] . '.checkPermission',
                        $item['item_id'], 'pf_video.view_browse_videos')) {
                    return $this->permissionError();
                }
            }
        }
        if (Phpfox::isModule('privacy') && !Phpfox::getService('privacy')->check('v', $item['video_id'], $item['user_id'], $item['privacy'],
                $item['is_friend'], true)) {
            return $this->permissionError();
        }
        $updateCounter = false;
    ...
    public function processCreate($values, $checkPerm = false)
    {
        ...
        if (!empty($values['file'])) {
            ...
            if (!empty($encoding->value->encoded)) {
                ...
                if ($iMethodUpload == 0 && $this->getSetting()->getAppSetting('v.pf_video_ffmpeg_path')) {
            ...
            if (isset($values['status_info'])) {
                $values['status_info'] = text()->clean($values['status_info']);
            }
            $id = Phpfox::getService('v.process')->addVideo($values);
            return $id;
        }
        return false;
    }
    ...
    function validateUrl($params)
    {
        ...
        $result = $this->processValidateUrl($sUrl);
        if ($result == false) {
    ...
    private function processValidateUrl($sUrl)
    {
        ...
        if ($parsed = Phpfox::getService('link')->getLink($sUrl)) {
            ...
            $result = [
                ...
                'default_image' => (strpos($parsed['default_image'], 'http') === false ? 'https:' : '') . $parsed['default_image'], // todo @ApiDev should validate protocol is http ? https.
    ...
    public function getFeedDisplay($feed, $item)
    {
        ...
        if (empty($item['duration'])) {
            $item['duration'] = $this->database()->select('v.duration')
                ->from(':video', 'v')
    ...
    function sponsor($params)
    {
        ...
        $item = $this->loadResourceById($id, true);
        $this->denyAccessUnlessGranted(VideoAccessControl::SPONSOR, $item);
