...
class EventApi extends AbstractResourceApi implements ActivityFeedInterface, MobileAppSettingInterface
{
    ...
    public function __construct()
    {
        ...
        $this->browserService = Phpfox::getService('event.browse');
    ...
    function findAll($params = [])
    {
        ...
        $defaultSortWhen = Phpfox::getParam('event.event_default_sort_time', 'ongoing');
        $when = !empty($params['when']) ? $params['when'] : $defaultSortWhen;
        $sort = $params['sort'];
        $view = $params['view'];
        ...
        if ($params['bounds']) {
            ...
            if (isset($bounds['south'], $bounds['north'])) {
                $this->search()->setCondition(' AND m.location_lat >= ' . $bounds['south'] . ' AND m.location_lat != 0');
                $this->search()->setCondition(' AND m.location_lat <= ' . $bounds['north']);
            }
            if (isset($bounds['west'], $bounds['east'])) {
                if ($bounds['west'] > $bounds['east']) {
                    $this->search()->setCondition(' AND m.location_lng <= ' . $bounds['west'] . ' AND m.location_lng != 0');
                    $this->search()->setCondition(' AND m.location_lng >= ' . $bounds['east']);
                } else {
                    $this->search()->setCondition(' AND m.location_lng >= ' . $bounds['west'] . ' AND m.location_lng != 0');
                    $this->search()->setCondition(' AND m.location_lng <= ' . $bounds['east']);
                }
