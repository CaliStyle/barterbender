...
class UserApi extends AbstractResourceApi implements MobileAppSettingInterface
{
    ...
    public function getUserInfo($params)
    {
        ...
        if (!$this->getAccessControl()->isGranted(UserAccessControl::VIEW_PROFILE_INFO, $resource)) {
        ...
        if ($this->getAccessControl()->isGranted(UserAccessControl::VIEW_BASIC_INFO, $resource)) {
            ...
            if (empty($user['dob_setting']) && $this->getSetting()->getAppSetting('user.default_privacy_brithdate') == 'hide') {
                $user['dob_setting'] = 3;
            }
            ...
            $data['sections']['basic_info']['fields']['space_used'] = [
                ...
                'value' => ($this->getSetting()->getUserSetting('user.total_upload_space') === 0
                    ? $oLocation->translate('space_total_out_of_unlimited', ['space_total' => \Phpfox_File::instance()->filesize($totalSpaceUsed)])
                    : $oLocation->translate('space_total_out_of_total_unit',
                        ['space_total' => \Phpfox_File::instance()->filesize($totalSpaceUsed), 'total_unit' => \Phpfox_File::instance()->filesize($totalUploadSpace)])),
            ];
        }
        $customData = Phpfox::getService('custom')->getForDisplay('user_main', $id, $user['user_group_id']);
        foreach ($customData as $customKey => $value) {
            ...
            if ($value && is_array($value) && $value['is_active']) {
                ...
                if (is_array($value['value'])) {
                    ...
                    $result = in_array($value['var_type'], ['radio', 'select']) ? $oLocation->translate($value['value']) : $value['value'];
                }
    ...
    function findAll($params = [])
    {
        ...
        foreach ($rawParams as $key => $param) {
            ...
            if (strpos($key, 'custom_') === 0) {
                ...
                if (!empty($finalParam)) {
                    $customs[str_replace('custom_', '', $key)] = $finalParam;
                }
            }
        }
    ...
    public function findOne($params)
    {
        ...
        $extraOnly = $this->resolver->resolveSingle(['only_extra'], false);
        ...
        if (!empty($params['is_me'])) {
            if ($user['view_id'] == 1) {
                return $this->permissionError($this->getLocalization()->translate('your_account_is_pending_approval'));
            }
            //Check banned
                ...
                if (isset($aBanned['ban_data_id'])) {
                    if (isset($aBanned['is_expired']) && $aBanned['is_expired'] == 0
                        ...
                        if (isset($aBanned['reason']) && !empty($aBanned['reason'])) {
                            $aBanned['reason'] = html_entity_decode(Phpfox::getLib('parse.output')->parse($aBanned['reason']), ENT_QUOTES);
                            ...
                            return $banMessage = $this->getLocalization()->translate('global_ban_message');
                        }
                        ...
                        return $this->permissionError($banMessage);
                    } else {
        ...
        if (!$getSimple) {
            $this->denyAccessUnlessGranted(UserAccessControl::VIEW_PROFILE, $user, null, $this->getLocalization()->translate('sorry_information_of_this_user_isn_t_available_for_you'));
    ...
    public function getPostTypes($id)
    {
        ...
        if (!Phpfox::getService('user.privacy')->hasAccess($id, 'feed.share_on_wall') || !Phpfox::getService('user.privacy')->hasAccess($id, 'feed.view_wall')
        ...
        if ($userId == $id || ($this->getSetting()->getUserSetting('profile.can_post_comment_on_profile'))) {
            ...
            if ($this->getUser()->getId() == $id && $this->getSetting()->getAppSetting('feed.enable_check_in') && $this->getSetting()->getAppSetting('core.google_api_key')) {
                $postOptions[] = [
                    'value'       => 'post.checkin',
    ...
    public function getPermissionWithUser($params)
    {
        ...
        if ($user['profile_page_id'] > 0) {
            ...
            $isFriend = (Phpfox::isModule('friend')) ? Phpfox::getService('friend')->isFriend($currentUserId, $id) : false;
    ...
    public function removeCover($params)
    {
        ...
        if (!$user) {
            return $this->notFoundError($this->getLocalization()->translate('unable_to_find_the_user_you_plan_to_edit'));
        }
    ...
    private function processCancelAccount($aVal)
    {
        ...
        if (!isset($aVal['password']) && !Phpfox::getUserBy('fb_user_id') && !Phpfox::getUserBy('janrain_user_id')) {
            return $this->error($this->getLocalization()->translate('please_enter_your_password'));
        }
    ...
    public function sendRegistrationSms($params)
    {
        ...
        if (Phpfox::getLib('phpfox.verify')->sendSMS($params['phone'], $message)) {
            return $this->success([]);
        }
    ...
    public function pokeUser($params)
    {
        ...
        if (Phpfox::getService('poke.process')->sendPoke($id)) {
