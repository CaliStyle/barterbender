...
class AccountSettingForm extends \Apps\Core_MobileApi\Api\Form\User\AccountSettingForm
{
    ...
    public function buildForm()
    {
        ...
        if ($this->setting->getAppSetting('user.split_full_name')) {
            ...
            $this->addField('full_name', TextType::class, [
                ...
                'required'    => true
            ], [new RequiredValidator(), new StringLengthValidator(1, $this->setting->getAppSetting('user.maximum_length_for_full_name'))], $sectionName);
        }
        $this->addField('user_name', TextType::class, [
            ...
            ->addField('email', EmailType::class, [
                'label'       => 'email',
                'editable'    => $this->setting->getUserSetting('user.can_change_email'),
                'description' => $this->setting->getUserSetting('user.can_change_email') ? $this->local->translate('changing_your_email_address_requires_you_to_verify_your_new_email') : '',
                'inline'      => true,
        ...
        if ($this->setting->getUserSetting('user.can_edit_currency')) {
            $this->addField('default_currency', ChoiceType::class, [
                'options'     => $this->getCurrencies(),
                'label'       => 'preferred_currency',
                'description' => $this->local->translate('show_prices_and_make_purchases_in_this_currency'),
                'required'    => true
            ], null, $sectionName);
        }
        ...
        // Membership Field
        if ($this->setting->isApp('Core_Subscriptions') && $this->setting->getAppSetting('subscribe.enable_subscription_packages') && (int)\Phpfox::getUserBy('user_group_id') != 1) {
            $this->addMembershipPackageField(false, 'package', 'membership');
        }

        if ($this->setting->getUserSetting('user.can_delete_own_account')) {
    ...
    {
        $text = Phpfox::getService(ParseInterface::class)->cleanOutput($text);
