    ...
    <div class="item-outer">
        ...
        <div class="item-name">
            <span class="item-icon" >
                <img src="{$aUser.full_path}" alt="p-reaction-icon" width="16px" >
            </span>
            {$aUser|user}
                ...
                {else}
                    <div class="item-actions">
                        <div class="dropdown js_unlike_pages_{$aUser.page.page_id}" {if empty($aUser.page.is_liked)}style="display:none;"{/if}>
                            ...
                            <ul class="dropdown-menu dropdown-menu-right">
                                <li>
                                    <a role="button" onclick="$.ajaxCall('like.delete', 'type_id=pages&amp;item_id={$aUser.page.page_id}&is_popup=1'); return false;">
                                        <span class="mr-1 ico ico-thumbdown"></span>{_p var='unlike'}
                                    </a>
                                </li>
                            </ul>
                        </div>
                        <button class="js_like_pages_{$aUser.page.page_id} btn btn-default btn-sm btn-gradient item-icon-like" {if !empty($aUser.page.is_liked)}style="display:none !important;"{/if} onclick="$.ajaxCall('like.add', 'type_id=pages&item_id={$aUser.page.page_id}&is_popup=1');">
