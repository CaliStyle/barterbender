    ...
    initCollectionSelection: function () {
        setTimeout(function () {
            if ($(PStatusBg.sListCollectionId).length) {
                ...
                $(PStatusBg.sBackgroundPost).unbind('append').bind('append', function (event) {
                ...
                if ($(PStatusBg.sInputDiv).closest('.activity-feed-status-form-active').length) {
...
PF.event.on('on_document_ready_end', PStatusBg.checkStatusOversize);
PF.event.on('on_page_change_end', PStatusBg.checkStatusOversize);
