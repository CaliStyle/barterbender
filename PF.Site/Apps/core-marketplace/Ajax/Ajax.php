...
{
    public function reopenExpiredListing()
    {
        Phpfox::isUser(true);

        if (!Phpfox::getUserParam('marketplace.can_reopen_own_expired_listing') && !Phpfox::getUserParam('marketplace.can_reopen_expired_listings')) {
            return \Phpfox_Error::set(_p('hack_attempt'));
        }
        if (empty(Phpfox::getParam('marketplace.days_to_expire_listing'))) {
            return \Phpfox_Error::set(_p('marketplaces_not_expired'));
        }
        $iId = $this->get('id');
        $aMarketplace = Phpfox::getService('marketplace')->getForEdit($iId, true);
        if (empty($aMarketplace)) {
            return \Phpfox_Error::set(_p('marketplace_not_found'));
        }

        if ((PHPFOX_TIME - (Phpfox::getParam('marketplace.days_to_expire_listing') * 86400)) <= $aMarketplace['time_stamp']) {
            return \Phpfox_Error::set(_p('marketplace_not_expired'));
        }

        if (!Phpfox::getUserParam('marketplace.can_reopen_expired_listings') && (Phpfox::getUserParam('marketplace.can_reopen_own_expired_listing') && $aMarketplace['user_id'] != Phpfox::getUserId())) {
            return \Phpfox_Error::set(_p('marketplace_can_not_reopen_other_listings'));
        }

        if (Phpfox::getService('marketplace.process')->reopenListing($iId)) {
