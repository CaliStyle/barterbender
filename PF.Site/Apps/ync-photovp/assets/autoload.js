...
var yncphotovp = {
    ...
    init: function () {
        ...
        if ($commentForm.length) {
            yncphotovp.addCommentFormObserver($commentForm);
        } else {
    ...
    createNextPreButtons: function(iCurrentPhotoId) {
        if (this.imageCount > 1) {
            var current = 0, $container = this.getPopup().find('.ync-photovp-image-holder');
            if (this.images.length > 1) {
                $.each(this.images, function (index, photo) {
                    if (photo.photo_id == iCurrentPhotoId) {
                        current = index;
                    }
                });
            }
            if (this.images[current - 1]) {
                var preButton = $('<a>',{
                    id: 'ync_photovp_previous_photo',
                    class: 'button photo_btn',
                    href: this.images[current - 1].link,
                    html: '<i class="ico ico-angle-left"></i>'
                });
                preButton.each(yncphotovp.bindPhotoLinks);
                preButton.appendTo($container)
            }
            if (this.images[current + 1]) {
                var nextButton = $('<a>',{
                    id: 'ync_photovp_next_photo',
                    class: 'button photo_btn',
                    href: this.images[current + 1].link,
                    html: '<i class="ico ico-angle-right"></i>'
                });
                nextButton.each(yncphotovp.bindPhotoLinks);
                nextButton.appendTo($container)
            }
        }
    },
    prepareRelatedPhotosFromAlbum: function() {
        if ($('body#page_photo_album, body#page_photo_index, body#page_advancedphoto_album').length) {
            var imageCount = 0, images = [];
            $('.photo-listing-item').each(function () {
                var t = $(this);
                imageCount++;
                images.push({
                    photo_id: t.data('photo-id'),
                    link: t.data('url')
                });
            });
            this.images = images;
    ...
    prepareRelatedPhotos: function(aPhotos) {
        this.images = aPhotos;
        this.imageCount = aPhotos.length;
    },
    clearRelatedPhotos: function() {
        this.imageCount = 0;
        this.images = [];
    },
    ...
    addCommentFormObserver: function($commentForm) {
        ...
        var callback = function (e) {
            yncphotovp.updateCommentListHeight($commentForm);
        };
