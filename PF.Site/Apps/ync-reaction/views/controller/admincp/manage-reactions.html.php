...
?>
{if count($aReactions)}
<form method="post" id="manage_reactions" action="{url link='admincp.yncreaction.manage-reactions'}">
    <div class="panel panel-default">
        <div class="panel-heading">
            <div class="panel-title" style="position: relative">
                <a href="{url link='admincp.app' id='YNC_Reaction'}">{_p var='manage_reactions'}</a>
            </div>
        </div>
        <div class="table-responsive flex-sortable">
            <table class="table table-bordered" id="_sort" data-sort-url="{url link='admincp.yncreaction.reactions-order'}">
                <thead>
                <tr>
                    <th class="w40"></th>
                    <th class="w40"></th>
                    ...
                    <tr class="tr" data-sort-id="{$aItem.id}">
                        ...
                        <td class="t_center w60">
                            {if $aItem.view_id != 2}
                                ...
                                <div class="link_menu">
                                    <ul>
                                        <li><a href="{url link='admincp.yncreaction.add-reaction' id=$aItem.id}">{_p var='edit'}</a></li>
                                        <li><a href="{url link='admincp.yncreaction.manage-reactions' delete=$aItem.id}" class="sJsConfirm">{_p var='delete'}</a></li>
                                    </ul>
                                </div>
                            {/if}
                        </td>
                        ...
                        <td class="td-flex">
                            {$aItem|yncreaction_color_title}
                        </td>
                        <td class="t_center w140">
                            {if $aItem.view_id != 2}
                                <div class="js_item_is_active"{if !$aItem.is_active} style="display:none;"{/if}>
                                    <a href="#?call=yncreaction.toggleActiveReaction&amp;id={$aItem.id}&amp;active=0" class="js_item_active_link" title="{_p var='Deactivate'}"></a>
                                </div>
                                <div class="js_item_is_not_active"{if $aItem.is_active} style="display:none;"{/if}>
                                    <a href="#?call=yncreaction.toggleActiveReaction&amp;id={$aItem.id}&amp;active=1" class="js_item_active_link" title="{_p var='Activate'}"></a>
                                </div>
                            {/if}
                        </td>
                    </tr>
