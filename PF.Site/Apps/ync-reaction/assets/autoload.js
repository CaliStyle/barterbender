...
$Ready(function () {
    ...
    //init tooltip
    $('.ync-reaction-item[data-toggle="tooltip"]').tooltip();
    ...
    $(document).on('click', '[data-toggle="ync_reaction_toggle_cmd"]', function (event) {
        ...
        if (!$('body').hasClass('_is_guest_user')) {
            if (element.parents('.comment-mini-content-commands').length) {
                ...
                oDisplayLink.find('.ync-reacted-icon-outer').html(_liked ? '<img src="' + react_icon + '" class="ync-reacted-icon" oncontextmenu="return false;"/>' : '');
            // <strong class=ync-reaction-title style="color:\#' + obj.reaction_color + '">' + obj.reaction_title + '</strong>'
                oDisplayLink.find('.ync-reaction-title').html(_liked ? obj.reaction_title : '');
                oDisplayLink.find('.ync-reaction-title').css('color', _liked ? '#' + obj.reaction_color : 'black');
            }
            else {
