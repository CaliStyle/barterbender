...
class AddReactionController extends Phpfox_Component
{
    ...
    public function process()
    {
        ...
        if ($iId = $this->request()->getInt('id')) {
            $bIsEdit = true;
            $aEditItem = Phpfox::getService('yncreaction')->getForEdit($iId);
            if (!$aEditItem) {
            ...
            $iTotalReactions = Phpfox::getService('yncreaction')->countReactions();
        ...
        if ($aVals = $this->request()->getArray('val')) {
            if ($aVals = $this->_validate($aVals)) {
                if ($bIsEdit) {
                    $aVals['id'] = $iId;
                    if (Phpfox::getService('yncreaction.process')->add($aVals, 'title', true)) {
                        $this->url()->send('admincp.app', ['id' => 'YNC_Reaction'],
                            _p('reaction_updated_successfully'));
                    }
                } else {
                    if (Phpfox::getService('yncreaction.process')->add($aVals)) {
                        $this->url()->send('admincp.app', ['id' => 'YNC_Reaction'], _p('reaction_added_successfully'));
                    }
                }
            }
        }
        ...
        $this->template()
            ...
            ->setBreadCrumb(_p("Reaction"), $this->url()->makeUrl('admincp.app', ['id' => 'YNC_Reaction']))
            ->setBreadCrumb($sTitle)
            ->setHeader([
                'jscript/admin.js' => 'app_ync-reaction',
                'colorpicker/js/colpick.js' => 'static_script',
                'css/colorpicker.css' => 'app_ync-reaction',
            ])
            ->assign([
                'sTitle' => $sTitle,
                'bIsEdit' => $bIsEdit,
