...
<script type="text/javascript">
    function onSubmitCreateCustomField() {
        ...
        if (is_edit) {
            $.ajaxCall('ynmember.updateField', $('#js_custom_field').serialize(), 'post');
        } else {
            $.ajaxCall('ynmember.addField', $('#js_custom_field').serialize(), 'post');
        }
        return false;
    }
</script>
...
$(document).on('click', '.js_jc_delete_current_option', function(){l}
    ...
    $Core.jsConfirm({l}message: '{$phrase}'{r}, function() {l}
        ...
        $.ajaxCall('ynmember.deleteOption', 'id=' + aParams['id'] );
    {r});
	return false;
{r});
