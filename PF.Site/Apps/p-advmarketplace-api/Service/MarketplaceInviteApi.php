...
use Apps\P_AdvMarketplaceAPI\Api\Form\MarketplaceInviteForm;
...
class MarketplaceInviteApi extends AbstractResourceApi
{
    ...
    {
        parent::__construct();
        $this->marketplaceService = Phpfox::getService('advancedmarketplace');
    ...
    private function processCreate($aVals)
    {
        ...
        $rListing = MarketplaceResource::populate($aListing);
        if (!$this->getAccessControl()->isGranted(MarketplaceAccessControl::EDIT, $rListing) && !$this->getAccessControl()->isGranted(MarketplaceAccessControl::VIEW, $rListing)) {
        ...
        if (isset($aVals['emails'])) {
            ...
            foreach ($aEmails as $sEmail) {
                ...
                $sLink = Phpfox_Url::instance()->permalink('advancedmarketplace', $iId, $aListing['title']);

                $sMessage = $this->getLocalization()->translate('full_name_invited_you_to_view_the_advancedmarketplace_listing_title', array(
                ...
                if ($aListing['post_status'] == '1') {
                    $aCachedEmails[$sEmail] = true;

                    $this->database()->insert(':advancedmarketplace_invite', array(
                            'listing_id' => $iId,
                            'type_id' => 1,
        ...
        if (isset($aVals['user_ids']) && is_array($aVals['user_ids'])) {
            ...
            foreach ($aUsers as $aUser) {
                ...
                if (Phpfox::isModule('friend') && !Phpfox::getService('friend')->isFriend(
                        Phpfox::getUserId(),
                        $aUser['user_id'])
                ) {
                    continue;
                ...
                $sLink = Phpfox_Url::instance()->permalink('advancedmarketplace', $aListing['listing_id'], $aListing['title']);

                $sMessage = $this->getLocalization()->translate('full_name_invited_you_to_view_the_advancedmarketplace_listing_title', [
                    'full_name' => Phpfox::getUserBy('full_name'),
                    'title' => $oParseInput->clean($aListing['title'], 255),
                    'link' => $sLink
                ], $aUser['language_id']);
                ...
                if ($bSent) {
                    ...
                    (Phpfox::isModule('request') ? Phpfox::getService('request.process')->add('advancedmarketplace_invite', $iId,
                        $aUser['user_id']) : null);
                }
            }
        }
    ...
    function loadResourceById($id, $returnResource = false)
    {
        $invite = $this->database()->select('ei.*, ei.invited_user_id as user_id')
            ->from(':advancedmarketplace_invite', 'ei')
