...
class CommissionRule extends \Phpfox_Service
{
    ...
    {
        $activityPointAppObject = Phpfox::isAppActive('Core_Activity_Points') ? \Core\Lib::appInit('Core_Activity_Points') : null;
        $valid = !empty($activityPointAppObject->version) && $activityPointAppObject->version >= '4.7.7';
        $aRules = db()->select('r.*,rm.user_group_id, rm.rulemap_id,rm.is_active')
            ...
            ->where('rm.user_group_id=' . (int) $iUserGroupId.' AND r.module_id IN(\''.$this->_activeModule.'\')'.($bActive ? ' AND rm.is_active = 1 ' : '') . (!$valid ? ' AND r.rule_name != "purchase_activity_points"' : ''))
            ->execute('getSlaveRows');
        if(count($aRules)){
