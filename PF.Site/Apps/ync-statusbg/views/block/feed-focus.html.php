...
<div class="activity_feed_content_text{if isset($aFeed.comment_type_id) && $aFeed.comment_type_id == 'poll'} js_parent_module_feed_{$aFeed.comment_type_id}{/if} {if !empty($aFeed.custom_class)}{$aFeed.custom_class}{/if} {if !empty($aFeed.status_background)}ync-statusbg-feed" style="background-image: url('{$aFeed.status_background}');"{else}"{/if}>
    ...
    <div class="activity_feed_content_status">{if mb_strlen(strip_tags($aFeed.feed_status)) > 200}{if strpos($aFeed.feed_status, '<br />') >= 200}{$aFeed.feed_status|feed_strip|shorten:200:'feed.view_more':true|split:55|max_line}{else}{$aFeed.feed_status|feed_strip|split:55|max_line|shorten:200:'feed.view_more':true}{/if}{else}{$aFeed.feed_status|feed_strip|split:55|max_line}{/if}</div>
    {/if}
