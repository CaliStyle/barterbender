$Core.ultimatevideo =
{
	...
	actioncategory: function(oObj, sAction, sUrl)
	{
		this.sUrl = sUrl;
		aParams = $.getParams(oObj.href);	
		sMessage = $(oObj).data('message');
		$('.dropContent').hide();

		switch (sAction)
		{
			case 'edit':
				$.ajaxCall('ultimatevideo.editcategory','id=' +  aParams['id']);
				break;
			case 'delete':
				var message = $(oObj).data('message');
                $Core.jsConfirm({message: message}, function() {
                    $.ajaxCall('ultimatevideo.AdminDeleteCategory','id=' +  aParams['id']);
                }, function(){});
				break;	
			case 'showcustomfields':
				tb_show("Custom Group", $.ajaxBox('ultimatevideo.showPopupCustomGroup', 'height=300&width=530&category_id=' + aParams['id']));
				break;
			case 'sub':
                $.ajaxCall('ultimatevideo.getsubcategory','id=' +  aParams['id']);
                break;
			default:
	...
	actionCustomField: function(oObj, sAction, sUrl)
	{
		...
		{
			case 'edit':
				$.ajaxCall('ultimatevideo.editcustomfieldgroup	','id=' +  aParams['id'])
				break;
			case 'delete':
                $Core.jsConfirm({message: oTranslations['are_you_sure']}, function() {
					if (aParams['type'] == 'group')
					{
						$.ajaxCall('ultimatevideo.AdminDeleteCustomFieldGroup', 'id=' + aParams['id']);
					}
					else
					{
						$.ajaxCall('ultimatevideo.deleteField', 'id=' + aParams['id']);
					}					
				});
				break;
			default:
				if (aParams['type'] == 'group')
				{
					$.ajaxCall('ultimatevideo.toggleActiveGroup', 'id=' + aParams['id']);
				}
				else
				{
					$.ajaxCall('ultimatevideo.toggleActiveField', 'id=' + aParams['id']);
				}				
				break;	
		}
	...
	{
		var id = $(obj).data('id');
		$.ajaxCall('ultimatevideo.editcustomfieldgroup	','id=' +  id);
	}
}
...
var SetUpMenu = function () {
    ...
    $('.js_drop_down_link').click(function()
    {
        ...
        setTimeout(function(){
			$('#js_drop_down_cache_menu .link_menu').find('li').click(function(){
				$('#js_drop_down_cache_menu').remove();
			});
		},200);
...
$Ready(function () {
	if($('#ynuv_custom_field_manage').length != 0){
		$Core.ultimatevideo.addSort();
		$Core.ultimatevideo.init_action_custom();
	}
