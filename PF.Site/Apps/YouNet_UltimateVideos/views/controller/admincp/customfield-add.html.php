    ...
    <form onsubmit="return onSubmitValid(this);" action="" method="post" id="js_add_group_name_form"
        ...
        <div class="panel-body">
            {foreach from=$aLanguages item=aLanguage}
                ...
                <input type="text" class="ynuv_add_group_name form-control"
        ...
        {if isset($bIsEditGroup) && $bIsEditGroup }
                    ...
                    {foreach from=$aGroup.customfield key=iKey item=iField}
                        <tr>
                            ...
                            <td align="center">
                                ...
                                /
                                <a href="javascript:void(0)"
                                   onclick="deleteCustomField({$iField.field_id});">{_p('Delete')}</a>
                            </td>
                        </tr>
                    {/foreach}
                </table>
            ...
            <div class="js_mp_parent_holder" id="js_mp_holder">
                {if isset($bIsEditGroup) && $bIsEditGroup}
                    <a href="#"
                       onclick="tb_show('{_p('add_custom_field')}', $.ajaxBox('ultimatevideo.AdminAddCustomFieldBackEnd', 'height=300&width=300&action=add&iGroupId={$aGroup.group_id}')); return false;">{_p('add_custom_field')}</a>
                {/if}
            </div>
        {/if}
