...
class Process extends Phpfox_service
{
    ...
    public function add($aVals)
    {
        ...
        $sVarName = 'ynultimatevideo_' . $sVarName;
    ...
    public function addValue($aValues, $iVideoId)
    {
        $aInsert = array('video_id' => $iVideoId);
    ...
    public function updateValue($aValues, $iVideoId)
    {
        $aUpdate = array();
        ...
        foreach ($aValues as $iFieldId => $aValue) {
            ...
            $aFields = $this->database()->select('cv.value_id')
                ...
                ->where('cv.field_id = ' . $iFieldId . ' AND cv.video_id = ' . $iVideoId)
                ->execute('getSlaveRows');

            if (count($aFields) <= 0) {
                $this->addValue(array($iFieldId => $aValue), $iVideoId);
                continue;
            }
            ...
            if (is_array($aValue)) {
                ...
                $this->database()->delete($this->_sTableValue, 'field_id = ' . $iFieldId . ' AND business_id = ' . $iVideoId);
                ...
                $this->database()->update($this->_sTableValue, $aUpdate, ' video_id = ' . $iVideoId . ' AND field_id = ' . $iFieldId);
            }
        }
    ...
    private function _addOptions($iFieldId, &$aVals)
    {
        ...
        if (isset($aVals['current'])) {
            foreach ($aVals['current'] as $sVarName => $aVal)
                $aExisting[] = str_replace('core.ynultimatevideo_cf_option_' . $aVals['field_id'] . '_', '', $sVarName);
        }
        ...
        foreach ($aVals['option'] as $iKey => $aOptions) {
            ...
            foreach ($aOptions as $sLang => $aOption) {
                ...
                $sPhraseVar = 'ynultimatevideo_cf_option_' . $iFieldId . '_' . $iSeqNumber;
