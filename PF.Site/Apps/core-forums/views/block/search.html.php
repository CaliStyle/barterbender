...
<div id="js_forum_adv_search_wrapper" class="advance_search_form" style="display: none;">
    ...
    <div class="form-group clearfix advance_search_form_button">
        ...
        <div class="pull-right">
            <a class="btn btn-default" href="{if isset($sResetUrl)}{$sResetUrl}{else}{if isset($sView) && $sView != ''}{url link='forum.search' view=$sView}{else}{url link='forum'}{/if}{/if}">{_p var='reset'}</a>
            <button class="btn btn-primary ml-1" name="search[submit]" id="adv_search_user"><i class="ico ico-search-o mr-1"></i>{_p var='search'}</button>
        </div>
    </div>
</div>
