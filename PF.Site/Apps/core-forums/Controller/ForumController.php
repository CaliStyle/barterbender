...
class ForumController extends Phpfox_Component
{
    ...
    public function process()
    {
        ...
        $sView = Phpfox::getLib('parse.output')->cleanScriptTag($this->request()->get('view'));
        ...
        if ($iResult || isset($bForceResult)) {
            ...
            list($iCnt, $aThreads) = Phpfox::getService('forum.post')->callback($aCallback)->isSearch($isSearch)->isAdvSearch($bIsAdvSearch)
            ...
            list($iCnt, $aThreads) = Phpfox::getService('forum.thread')->isSearch($bIsSearch)->isAdvSearch($bIsAdvSearch)
                ->isTagSearch($bIsTagSearch)
                ->isNewSearch(($sView == 'new' ? true : false))
                ->isSubscribeSearch(($sView == 'subscribed' ? true : false))
                ->isModuleSearch($bIsModuleTagSearch)
        ...
        if ($bIsSearch && !isset($aForum['forum_id'])) {
            ...
            if (Phpfox::getParam('forum.rss_feed_on_each_forum')) {
                if ($aParentModule === null) {
                    $this->template()->setHeader('<link rel="alternate" type="application/rss+xml" title="' . _p('forum') . ': ' . _p($aForum['name']) . '" href="' . $this->url()->makeUrl('forum',
            ...
            if ($aParentModule === null) {
                ...
                $this->template()->setTitle(Phpfox_Locale::instance()->convert(_p($aForum['name'])))
