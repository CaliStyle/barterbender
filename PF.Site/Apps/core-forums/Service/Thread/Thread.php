...
class Thread extends Phpfox_Service
{
    ...
    public function get(
        ...
        $this->database()->union()->unionFrom('ft');
        ...
        if (Phpfox::getParam('forum.forum_database_tracking')) {
            $this->database()
                ->select('ftr.item_id AS is_seen, ftr.time_stamp AS last_seen_time, ')
                ->leftJoin(Phpfox::getT('track'), 'ftr', 'ftr.item_id = ft.thread_id AND ftr.user_id = ' . Phpfox::getUserId() . ' AND ftr.type_id=\'forum_thread\'')
                ->group('ft.thread_id');
        }

        $aThreads = $this->database()->select('ft.*, fs.subscribe_id AS is_subscribed, f.name AS forum_name, f.name_url AS forum_url, f.is_closed AS forum_is_closed, ' . Phpfox::getUserField() . ', ' . Phpfox::getUserField('u2', 'last_') . '')
    ...
    {
        $sCacheId = $this->cache()->set('forum_thread_pending_sponsor');
        if(false === ($aItems = $this->cache()->get($sCacheId)))
        {
            $aRows = db()->select('m.thread_id')
                ->from(Phpfox::getT('forum_thread'),'m')
                ->join(Phpfox::getT('better_ads_sponsor'),'s', 's.item_id = m.thread_id')
                ->where('m.order_id != 2 AND s.is_custom = 2 AND s.module_id = "forum_thread"')
                ->execute('getSlaveRows');
            $aItems = array_column($aRows, 'thread_id');
    ...
    public function getThreadPermission(&$aRow, $aCallback = null)
    {
        ...
        if (!isset($aRow['forum_is_closed']) || !$aRow['forum_is_closed']) {
            ...
            {
                $aRow['canSponsor'] = !$aRow['view_id'] && !$aRow['is_announcement'] && !defined('PHPFOX_IS_GROUP_VIEW') && Phpfox::getUserParam('forum.can_sponsor_thread');
                $bCanPurchaseSponsor = $this->canPurchaseSponsorItem($aRow['thread_id']);
                $aRow['canPurchaseSponsor'] = !$aRow['view_id'] && !$aRow['is_announcement'] && (Phpfox::getUserParam('forum.can_purchase_sponsor') && Phpfox::getService('forum')->getSponsorPrice()) && !defined('PHPFOX_IS_GROUP_VIEW') && (Phpfox::getUserId() == (int)$aRow['user_id']) && $bCanPurchaseSponsor;
            }
     ...
     */
    public function getAnnoucements($iForumId = null, $iGroupId = null, $iLimit = 10)
    {
        if ($iForumId !== null) {
    ...
    public function getForEdit($iId)
    {
        ...
        } elseif ((int)$aThread['order_id'] == 1) {
            $aThread['type_id'] = 'sticky';
        }elseif ((int)$aThread['order_id'] == 2)
        {
            $aThread['type_id'] = 'sponsor';
        }
    ...
    public function getRecentDiscussions(
        ...
        $aThreads = $this->database()->select('ft.*, fpt.text, fpt.text_parsed, ' . Phpfox::getUserField() . ', ' . Phpfox::getUserField('u2', 'last_') . '')
            ->join(Phpfox::getT('user'), 'u', 'u.user_id = ft.user_id')
    ...
    public function getRandomSponsored($iLimit = 4, $iCacheTime = 5)
    {
        ...
        if (($sThreadIds = $this->cache()->get($sCacheId, $iCacheTime)) === false) {
            $sThreadIds = '';
            $aThreadIds = $this->database()->select('ft.thread_id')
                ->from($this->_sTable, 'ft')
                ->join(Phpfox::getT('user'), 'u', 'u.user_id = ft.user_id')
                ->join(Phpfox::getT('better_ads_sponsor'), 's', 's.item_id = ft.thread_id')
                ->where('ft.view_id = 0 AND ft.order_id = 2 AND ft.group_id = 0 AND s.module_id = \'forum_thread\' AND s.is_active = 1 AND s.is_custom = 3')
                ->order('rand()')
        ...
        $aThreads = $this->database()->select('ft.*, fpt.text, fpt.text_parsed, ' . Phpfox::getUserField() . ', s.*')
            ->from($this->_sTable, 'ft')
            ->join(Phpfox::getT('user'), 'u', 'u.user_id = ft.user_id')
            ->join(Phpfox::getT('forum_post'), 'fp', 'ft.thread_id = fp.thread_id AND fp.title != \'\'')
            ->join(Phpfox::getT('forum_post_text'), 'fpt', 'fpt.post_id = fp.post_id')
            ->join(Phpfox::getT('better_ads_sponsor'), 's', 's.item_id = ft.thread_id AND s.module_id = \'forum_thread\' AND s.is_custom = 3')
