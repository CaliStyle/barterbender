            ...
            switch ($aVals['type_id']) {
                ...
                case 'advancedphoto':
                    if($feed = Phpfox::getService('ynfeed')->callback($aFeedCallback)->getFeed($aVals['feed_id'])) {
                        ...
                        $lastPhotoId = Phpfox::getService('ynfeed.photo.process')->processPhotoForFeed($aVals['feed_id'], $editedPhotos, $deletedPhotos, $uploadedPhotos, $aFeedCallback, ['privacy' => $aVals['privacy']]);

                        Phpfox::getService('ynfeed.process')->update($aVals['type_id'], $aVals['item_id'], $aVals['privacy'],
                            $aVals['privacy_comment']);
                        $extraInfoMerge = array_merge($aVals, ['is_update' => 1]);
                        if (!empty($lastPhotoId) && $lastPhotoId != $aVals['item_id']) {
                            $extraInfoMerge['replaced_item_id'] = $lastPhotoId;
                        }
                        Phpfox::getService('ynfeed')->setExtraInfo(array_merge($aVals, $extraInfoMerge));
                        Phpfox::getService('ynfeed.photo.process')->updateFeedTagData($aVals['type_id'], $aVals['item_id'], ['new_item_id' => $lastPhotoId]);
