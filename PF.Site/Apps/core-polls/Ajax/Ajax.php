...
class Ajax extends Phpfox_Ajax
{
    ...
    public function addVoteInFeed()
    {
        Phpfox::isUser(true);
        $aVals = $this->get('val');
        if(!isset($aVals['poll_' . $aVals['poll_id'] . '_answer']))
        {
            return Phpfox_Error::set(_p('poll_you_must_choose_at_least_one_option'));
        }
        Phpfox::getService('poll.process')->addVote(Phpfox::getUserId(), (int)$aVals['poll_id'], $aVals['poll_' . $aVals['poll_id'] . '_answer']);

        Phpfox::getBlock('poll.feed-rows',[
            'this_feed_id' => $aVals['poll_id'],
            'custom_param_'. $aVals['poll_id'] => Phpfox::getService('poll')->getPollByUrl($aVals['poll_id']),
            'sponsor_feed_id' => 0
        ]);
        $sContent = $this->getContent(false);
        $this->call('$Core.poll.processVoteInFeed(' . $aVals['poll_id'] . ',' . json_encode(base64_encode($sContent)). ');');
    }
