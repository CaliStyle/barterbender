...
$Core.poll =
    {
        ...
        submitPoll: function (bCanChangePoll, iPollId) {
            ...
            var bIsInFeed = $(formId).find('input[name="val[is_feed]"]').length;
            if (!bIsInFeed) {
                if (bCanChangePoll) {
                    $(this).parent().hide();
                }
                $(this).parents('.p_4:first').find('.js_poll_image_ajax:first').show();
                $('#js_poll_form_' + iPollId).ajaxCall(bIsInFeed ? 'poll.addVoteInFeed' : 'poll.addVote');
            }
            else if (bIsInFeed)
            {
                $('#js_poll_form_' + iPollId).ajaxCall('poll.addVoteInFeed');
            }
            return false;
        },
