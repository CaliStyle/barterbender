	...
	<div class="item-wrapper">
		<div class="item-outer-info">
			<div class="item-inner">
				...
				<div class="core-feed-description line-3 item_view_content">{$aPoll.description|stripbb|feed_strip|split:55|max_line}</div>
			</div>
            ...
            <form class="poll_form" method="post" action="{url link='current'}" id="js_poll_form_{$aPoll.poll_id}">
                ...
                <div class="action-vote-bottom">
                    ...
                    <div class="poll-group-submit">
                        <a href="javascript:void(0);" class="item-vote-number" onclick="$Core.box('poll.pageVotes', 900, 'poll_id={$aPoll.poll_id}'); return false;">{$aPoll.total_votes|short_number} {if $aPoll.total_votes == 1}{_p('poll_total_vote')}{else}{_p('poll_total_votes')}{/if}</a>
                        {if (Phpfox::getUserId() != $aPoll.user_id) || (Phpfox::getUserId() == $aPoll.user_id && Phpfox::getUserParam('poll.can_vote_in_own_poll'))}
                            {if !$aPoll.user_voted_this_poll || ($aPoll.user_voted_this_poll && Phpfox::getUserParam('poll.poll_can_change_own_vote'))}
                            <div class="item-submit">
                                <a href="javascript:void(0);" class="btn btn-sm btn-default" onclick="$Core.poll.submitPoll({if !Phpfox::getUserParam('poll.poll_can_change_own_vote')}true{else}false{/if}, {$aPoll.poll_id});">{if !$aPoll.user_voted_this_poll}{_p var='poll_vote_now'}{else}{_p var='poll_vote_again'}{/if}</a>
                            </div>
                            {/if}
                        {/if}
                    </div>
