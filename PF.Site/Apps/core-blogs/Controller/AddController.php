...
class AddController extends Phpfox_Component
{
    ...
    public function process()
    {
        ...
        if (!empty($sModule) && Phpfox::hasCallback($sModule, 'getItem')) {
            ...
            if ($sModule && $iItemId && Phpfox::hasCallback($sModule, 'viewBlog')) {
                $aCallback = Phpfox::callback($sModule . '.viewBlog', $iItemId);
                if ($aCallback === false) {
                    return Phpfox_Error::display(_p('Cannot find the parent item.'));
                }
        ...
        if (($aVals = $this->request()->getArray('val')) || $bIsPublish) {
            ...
            if ($bIsPublish) {
                ...
                if(Phpfox::isModule('privacy') && !empty($aVals['blog_id']) && ((int)$aVals['privacy'] == PRIVACY_CUSTOM))
                {
                    $friendList = Phpfox::getService('privacy')->get('blog', $aVals['blog_id']);
                    if(!empty($friendList))
                    {
                        $aVals['privacy_list'] = array_column($friendList,'friend_list_id');
                    }
                }
            }
