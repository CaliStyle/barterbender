...
class ViewController extends Phpfox_Component
{
    ...
    public function process()
    {
        ...
        if (isset($aCategories)) {
            ...
            foreach ($aCategories as $iKey => $aCategory) {
                $sCategories .= ($iKey != 0 ? ',' : '') . ' <a href="' . (isset($aCategory['user_id']) ? $this->url()->permalink($aItem['user_name'] . '.blog.category',
                        ...
                        _p($aCategory['category_name'])) : $this->url()->permalink('blog.category',
                        $aCategory['category_id'],
        ...
        if (!empty($aItem['image_path'])) {
            $aItem['image'] = Phpfox::getService('blog')->getImageUrl($aItem['image_path'], $aItem['server_id'], self::IMG_SUFFIX);
            $this->template()
        ...
        $metaDescription = !empty($aItem['text']) ? $aItem['text'] : Phpfox::getParam('blog.blog_meta_description');
        $metaKeywords = !empty($this->template()->getKeywords($aItem['title'])) ? $this->template()->getKeywords($aItem['title']) : Phpfox::getParam('blog.blog_meta_keywords');
