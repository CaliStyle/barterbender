...
class Category extends Core_Service_Systems_Category_Category
{
     ...
     */
    public function getCategory($iId, $bGetActive = false)
    {
        $aWhere = [
            'bc.category_id' => intval($iId)
        ];
        if ($bGetActive) {
            $aWhere = array_merge($aWhere, [
                'bc.is_active' => 1
            ]);
        }
    ...
    public function getCategoriesByBlogId($iId, &$aCategories = null)
    {
        ...
        foreach ($aCategories as $aCategory) {
            ...
            $aBreadcrumb[] = array(
                'category_name' => Phpfox::getLib('locale')->convert(_p($aCategory['name'])),
            ...
            if ($bDropDown) {
                ...
                if ($bAnchor === true) {
                    $sCategories .= '<li><a href="' . Phpfox::permalink('blog', $aCategory['category_id'],
                            $aCategory['name']) . '" class="js_blog_category" id="js_blog_category_' . $aCategory['category_id'] . '">' . Phpfox::getLib('locale')->convert(_p($aCategory['name'])) . '</a>' . $this->_get($aCategory['category_id'],
                            $bAnchor) . '</li>';
                } else {
                    $sCategories .= '<li><img src="' . Phpfox::getLib('template')->getStyle('image',
                            'misc/draggable.png') . '" alt="" /> <span class="js_blog_category" id="js_sortable_category_' . $aCategory['category_id'] . '">' . Phpfox::getLib('locale')->convert(_p($aCategory['name'])) . '</span>' . $this->_get($aCategory['category_id'],
                            $bAnchor) . '</li>';
                }
            }
        }
