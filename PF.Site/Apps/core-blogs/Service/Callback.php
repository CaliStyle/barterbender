...
class Callback extends Phpfox_Service
{
    ...
    public function getActivityFeed($aRow, $aCallback = null, $bIsChildItem = false)
    {
        ...
        if (((defined('PHPFOX_IS_PAGES_VIEW') && defined('PHPFOX_PAGES_ITEM_TYPE') && !Phpfox::getService(PHPFOX_PAGES_ITEM_TYPE)->hasPerm(null,
                        'blog.view_browse_blogs'))
                || (!defined('PHPFOX_IS_PAGES_VIEW') && $aBlog['module_id'] == 'pages' && Phpfox::isAppActive('Core_Pages') && !Phpfox::getService('pages')->hasPerm($aBlog['item_id'],
                        'blog.view_browse_blogs')))
        ...
        $sHtmlCategories = '';
        $blogLink = Phpfox::permalink('blog', $aBlog['blog_id'], $aBlog['title']);
        if (!empty($aCategories)) {
            ...
            if (!empty($aCategories)) {
                ...
                $sDot = '';
                if($iCountCategories > 5)
                {
                    $aCategories = array_slice($aCategories,0, 5);
                    $sDot = '<li><a href="'. $blogLink . '">...</a></li>';
                }
        ...
        if (!defined('PHPFOX_IS_PAGES_VIEW') && (($aBlog['module_id'] == 'groups' && Phpfox::isAppActive('PHPfox_Groups')) || ($aBlog['module_id'] == 'pages' && Phpfox::isAppActive('Core_Pages')))) {
    ...
    public function addComment($aVals, $iUserId = null, $sUserName = null)
    {
        ...
        Phpfox::getService('comment.process')->notify(array(
                ...
                'owner_subject' => array('full_name_commented_on_your_blog_title',
                    array('full_name' => Phpfox::getUserBy('full_name'), 'title' => $aBlog['title'])),
                'owner_message' => array('full_name_commented_on_your_blog_message',
                    array('full_name' => Phpfox::getUserBy('full_name'), 'link' => $sLink, 'title' => $aBlog['title'])),
                'owner_notification' => 'comment.add_new_comment',
                'notify_id' => 'comment_blog',
                'mass_id' => 'blog',
                'mass_subject' => (Phpfox::getUserId() == $aBlog['user_id'] ? array('full_name_commented_on_gender_blog',
                    array(
                        'full_name' => Phpfox::getUserBy('full_name'),
                        'gender' => Phpfox::getService('user')->gender($aBlog['gender'], 1)
                    )) : array('full_name_commented_on_blog_full_name_s_blog',
                    array('full_name' => Phpfox::getUserBy('full_name'), 'blog_full_name' => $aBlog['full_name']))),
                'mass_message' => (Phpfox::getUserId() == $aBlog['user_id'] ? array('full_name_commented_on_gender_blog_message',
                    array(
                        'full_name' => Phpfox::getUserBy('full_name'),
                        'gender' => Phpfox::getService('user')->gender($aBlog['gender'], 1),
                        'link' => $sLink,
                        'title' => $aBlog['title']
                    )) : array('full_name_commented_on_blog_full_name_s_blog_message', array(
                    'full_name' => Phpfox::getUserBy('full_name'),
...
//        ];
    ...
    public function getGroupMenu($aPage)
    {
        ...
        $aMenus[] = [
            'phrase' => _p('Blogs'),
            'url' => Phpfox::getService('groups')->getUrl($aPage['page_id'], $aPage['title'], $aPage['vanity_url']) . 'blog/',
            'icon' => 'module/blog.png',
            'landing' => 'blog'
        ];
