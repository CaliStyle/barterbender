...
class IndexController extends Phpfox_Component
{
    public function process()
    {
        ...
        if ($this->request()->get('req2') == 'delete' && ($iDeleteId = $this->request()->getInt('req3'))) {
            if (Phpfox::getService('ynblog.permission')->canDeleteBlog($iDeleteId, true) && Phpfox::getService('ynblog.process')->deleteBlog($iDeleteId)) {
                if ($iProfileId = $this->request()->getInt('profile')) {
                    ...
                    $this->url()->send($aUser['user_name'] . '.ynblog', [], _p('blog_successfully_deleted'));
                } else {
        ...
        $this->template()->setTitle(($bIsProfile ? _p('full_name_s_blogs', array('full_name' => $aUser['full_name'])) : _p('advanced_blogs')));

        if ($bIsProfile) {
            section(_p('Blogs'), url('/' . $aUser['user_name'] . '/ynblog'));
        } else {
            section(_p('Blogs'), url('/ynblog'));
        }
        ...
        $aSearchFields = [
            ...
            'search_tool' => [
                ...
                'sort' => [
                    ...
                    'featured' => ['advblog.is_featured', _p('featured')]
                ],
                'show' => [10, 20, 30]
            ]
        ];
        ...
        if (!isset($aParentModule['module_id']) && user('yn_advblog_add_blog')) {
            sectionMenu(_p('write_new'), url('/' . $sCustomUrl . '/add'));
        }

        if ($this->request()->get('sort') && $this->request()->get('sort') == 'featured') {
            $this->search()->setCondition(' AND advblog.is_featured=1 ');
        }
        ...
        if (!empty($aSearch)) {
            if (!empty(trim($aSearch['author_name']))) {
                $this->search()->setCondition('AND u.full_name LIKE \'%' . trim($aSearch['author_name']) . '%\'');
            }
        ...
        if ($this->request()->get(($bIsProfile === true ? 'req3' : 'req2')) == 'category') {
            if ($aBlogCategory = Phpfox::getService('ynblog.category')->getCategory($this->request()->getInt(($bIsProfile === true ? 'req4' : 'req3')))) {
                ...
                $categoryId = $this->request()->getInt(($bIsProfile === true ? 'req4' : 'req3'));
                $categoryIds = Phpfox::getService('ynblog.category')->getRelatedCategories($categoryId);
                if(!empty($categoryIds)) {
                    $this->search()->setCondition('AND ac.category_id IN ('. $categoryIds .')');
                }

                $this->template()->setTitle(_p($aBlogCategory['name']));
        ...
        if (isset($aParentModule) && isset($aParentModule['module_id'])) {
            ...
            if (in_array($sView, ['saved', 'favorite', 'draft', 'my']) || ($sView == 'pending' && Phpfox::getUserParam('yn_advblog_approve'))) {
                ...
                if (count($aModules)) {
                    $this->search()->setCondition('AND advblog.module_id NOT IN ("' . implode('","',
                            $aModules) . '")');
                }
            } else {
                $aModules = ['ynblog'];
                ...
                $this->search()->setCondition('AND advblog.module_id IN ("' . implode('","', $aModules) . '")');
            }
        }
        ...
        if ($sView == 'favorite' && Phpfox::isUser()) {
            foreach ($aItems as $key => $aItem) {
                $aItems[$key]['permission_enable'] = 1;
            }
        }

        (($sPlugin = Phpfox_Plugin::get('ynblog.component_controller_index_process_middle')) ? eval($sPlugin) : false);

        $this->template()->setMeta('keywords', Phpfox::getParam('ynblog.ynblog_meta_keywords'));
        $this->template()->setMeta('description', Phpfox::getParam('ynblog.ynblog_meta_description'));
        if ($bIsProfile) {
        ...
        $this->setParam('sModeViewId', 'p-blog-listing');

        foreach ($aItems as $key => $aItem) {
            $this->template()->setMeta('keywords', $this->template()->getKeywords($aItem['title']));
            $aItems[$key]['image_url'] = Phpfox::getService('ynblog.helper')->getImagePath($aItem['image_path'], $aItem['server_id'], '_500', $aItem['is_old_suffix']);
        ...
        $this->template()->assign(array(
                ...
                'aCategories' => Phpfox::getService('ynblog.category')->getForUsers(0, 1, 1),
                'bShowCommand' => $bIsSearch,
            )
        )
        ...
        if ($sView == 'favorite') {
            $aModerationMenu[] = array(
                'phrase' => _p('ynblog_unfavorited_all'),
                'action' => 'un_favorited'
            );
        ...
        $this->template()->setPhrase(['search'])
            ->assign(['bShowModerator' => $bShowModerator]);
        ...
        if (defined('PHPFOX_IS_PAGES_VIEW') && PHPFOX_IS_PAGES_VIEW && defined('PHPFOX_PAGES_ITEM_TYPE')) {
            ...
            $this->template()
                ...
                ->setBreadCrumb(_p('ynblog'), $aParentModule['url'] . 'ynblog/');
        }
