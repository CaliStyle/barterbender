...
class Ajax extends Phpfox_Ajax
{
    ...
    public function publishBlog()
    {
        ...
        if ($iBlogId) {
            ...
            if (Phpfox::getService('ynblog.permission')->canEditBlog($iBlogId) && $aBlog['post_status'] == 'draft') {
                ...
                (user('yn_advblog_automatically_approve')) ? $this->alert(_p('blog_successfully_public_please_waiting_approval_from_administrator')) : $this->alert(_p('blog_publish_successfully'));
                $this->call('setTimeout(function(){ location.reload(); },3000);');
            } else {
                $this->alert(_p('you_do_not_have_permission_to_approve_this_blog'));
            }
    ...
    public function updateFavorite()
    {
        ...
        if ($bFavorite) {
            ...
            $sHtml = '<a class="btn btn-default btn-sm btn-icon" onclick="$Core.ajaxMessage();ynadvancedblog.updateFavorite(' . $iBlogId . ',0);return false;"><i class="ico ico-star" aria-hidden="true"></i>' . _p('favorited') . '</a>';
            $iFavoriteTotal++;
            $iFavoriteTotal = ($iFavoriteTotal >= 100) ? '99+' : $iFavoriteTotal;
            $this->call('$("#total_favorite_blog").text(' . $iFavoriteTotal . '); $("#ynadvblog-detail-favorite-blog-' . $iBlogId . '").html(\'' . $sHtml . '\');');
        } else {
            $this->call("$('#js_blog_id_" . $iBlogId . "').remove();");
            Phpfox::getService('ynblog.process')->deleteFavorite(Phpfox::getUserId(), $iBlogId);
            $sHtml = '<a class="btn btn-default btn-sm btn-icon" onclick="$Core.ajaxMessage();ynadvancedblog.updateFavorite(' . $iBlogId . ',1);return false;"><i class="ico ico-star-o" aria-hidden="true"></i>' . _p('favorite') . '</a>';
            $iFavoriteTotal--;
            $iFavoriteTotal = ($iFavoriteTotal >= 100) ? '99+' : $iFavoriteTotal;
            $this->call('$("#total_favorite_blog").text(' . $iFavoriteTotal . '); $("#ynadvblog-detail-favorite-blog-' . $iBlogId . '").html(\'' . $sHtml . '\');');
        }
    ...
    public function updateFollow()
    {
        ...
        if ($bFollow) {
            ...
            $sHtml = '<button class="btn btn-sm btn-default btn-icon" onclick="ynadvancedblog.updateFollow(' . $iBloggerId . ',0);return false;" title="' . _p('following') . '"><i class="ico ico-check" aria-hidden="true"></i>' . _p('Following') . '</button>';
            $iNewFollowTotal++;
            ...
            $sHtml = '<button class="btn btn-sm btn-primary btn-icon" onclick="ynadvancedblog.updateFollow(' . $iBloggerId . ',1);return false;" title="' . _p('follow') . '"><i class="ico ico-plus" aria-hidden="true"></i>' . _p('Follow') . '</button>';
            $iNewFollowTotal--;
    ...
    public function updateFollowLink()
    {
        ...
        if ($bFollow) {
            Phpfox::getService('ynblog.process')->addFollow(Phpfox::getUserId(), $iBloggerId);
            $sHtml = '<a class="btn btn-sm btn-default btn-icon" onclick="ynadvancedblog.updateFollowLink(' . $iBloggerId . ',0);return false;" title="' . _p('following') . '"><i class="ico ico-check" aria-hidden="true"></i>' . _p('Following') . '</a>';
            $iNewFollowTotal++;
            $sNewFollowTotal = $iNewFollowTotal . '&nbsp;' . ($iNewFollowTotal == 1 ? _p('follower') : _p('followers'));
            ...
            Phpfox::getService('ynblog.process')->deleteFollow(Phpfox::getUserId(), $iBloggerId);
            $sHtml = '<a class="btn btn-sm btn-primary btn-icon" onclick="ynadvancedblog.updateFollowLink(' . $iBloggerId . ',1);return false;"><i class="ico ico-plus" aria-hidden="true" title="' . _p('follow') . '"></i>' . _p('Follow') . '</a>';
            $iNewFollowTotal--;
            $iNewFollowTotal = ($iNewFollowTotal < 0 ? 0 : $iNewFollowTotal);
            $sNewFollowTotal = $iNewFollowTotal . '&nbsp;' . ($iNewFollowTotal == 1 ? _p('follower') : _p('followers'));
        ...
        $this->call('ynadvancedblog.updateFollowButton(' . $iBloggerId . ',' . $bFollow . ');');
    }
    ...
    public function updateSavedBlog()
    {
        ...
        if ($bSavedBlog) {
            Phpfox::getService('ynblog.process')->addSavedBlog(Phpfox::getUserId(), $iBlogId);
            $sHtml = '<a class="p-option-button p-advblog-btn-save active" title="' . _p('unsave_this_ynblog') . '" href="javascript:void(0)" onclick="ynadvancedblog.updateSavedBlog(' . $iBlogId . ', 0);return false;"><i class="ico ico-bookmark-o"></i></a>';
            $iTotalSaved++;
            $this->call('$("#total_saved_blog").show(); $("#total_saved_blog").text(' . $iTotalSaved . '); $(".js_ynblog_saved_blog_' . $iBlogId . '").addClass(\'active\');$(".js_ynblog_saved_blog_' . $iBlogId . '").html(\'' . $sHtml . '\');');
            $this->call('ynadvancedblog.updateDetailSavedBlogBtn(' . $iBlogId . ', 1);');
        } else {
            Phpfox::getService('ynblog.process')->deleteSavedBlog(Phpfox::getUserId(), $iBlogId);
            $sHtml = '<a class="p-option-button p-advblog-btn-save" title="' . _p('save_this_ynblog') . '" href="javascript:void(0)" onclick="ynadvancedblog.updateSavedBlog(' . $iBlogId . ', 1);return false;"><i class="ico ico-bookmark-o"></i></a>';
            $iTotalSaved--;
            $this->call('$("#total_saved_blog").text(' . $iTotalSaved . '); $(".js_ynblog_saved_blog_' . $iBlogId . '").html(\'' . $sHtml . '\');');
            $this->call('ynadvancedblog.updateDetailSavedBlogBtn(' . $iBlogId . ', 0);');
        }
    }
        ...
        if ($iBlogId) {
            if (Phpfox::getService('ynblog.process')->deleteBlog($iBlogId)) {
                if ($bIsDetail) {
                    ...
                    $this->call('setTimeout(function() {js_box_remove($(\'.js_box_close\'));$(\'.acp-header-section a:eq(3)\').trigger(\'click\')},2000);');
                }
            } else {
    ...
    public function moderation()
    {
        ...
        switch ($sAction) {
            case 'approve':
                ...
                $sMessage = _p('ynblog_blogs_actions',['action' => 'approved']);
                break;
            case 'delete':
                ...
                $sMessage = _p('ynblog_blogs_actions',['action' => 'deleted']);
                break;
            case 'feature':
                ...
                $sMessage = _p('ynblog_blogs_actions',['action' => 'featured']);
                break;
            case 'unfeature':
                if (!user('yn_advblog_feature')) {
                    $sMessage = _p('you_do_not_have_permission_on_this_action');
                ...
                $sMessage = _p('ynblog_blogs_actions', ['action' => 'un-featured']);
                break;
            case 'un_favorited':
                foreach ((array)$aIds as $iId) {
                    Phpfox::getService('ynblog.process')->deleteFavorite(Phpfox::getUserId(), $iId);
                }
                $sMessage = _p('ynblog_blogs_actions',['action' => 'un-favorite']);
                break;
            case 'un_saved':
                foreach ((array)$aIds as $iId) {
                    Phpfox::getService('ynblog.process')->deleteSavedBlog(Phpfox::getUserId(), $iId);
                }
                $sMessage = _p('ynblog_blogs_actions',['action' => 'un-saved']);
                break;
            case 'deny':
                if (!user('yn_advblog_approve')) {
                    $sMessage = _p('you_do_not_have_permission_on_this_action');
                ...
                $sMessage = _p('ynblog_blogs_actions',['action' => 'denied']);
                break;
            case 'publish':
                foreach ((array)$aIds as $iId) {
                ...
                $sMessage = _p('ynblog_blogs_actions',['action' => 'public']);
                break;
