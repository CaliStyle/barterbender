        ...
        <table class="table table-bordered" id="_sort" data-sort-url="{url link='ynblog.admincp.category.order'}">
            ...
            <tbody>
                ...
                <tr class="{if is_int($iKey/2)} tr{else}{/if}" data-sort-id="{$aCategory.category_id}">
                    ...
                    <td class="t_center w140">{if $aCategory.used > 0}<a href="{permalink module='ynblog.category' id =$aCategory.category_id title=$aCategory.name|clean}" id="js_category_link{$aCategory.category_id}">{$aCategory.used}</a>{else}{_p var='0'}{/if}</td>
                    <td class="on_off w80">
