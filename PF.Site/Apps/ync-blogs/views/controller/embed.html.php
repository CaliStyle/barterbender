...
<div class="moderation_row js_blog_parent clearfix">
    <div class="ynadvblog_item">
        <div class="ynadvblog_avatar">
            <a title="{$aItem.title|clean}" href="{permalink module='ynblog' id=$aItem.blog_id title=$aItem.title}" class="item_image{if empty($aItem.text)} full{/if}" style="background-image: url(<?php echo Phpfox::getService('ynblog.helper')->getImagePath($this->_aVars['aItem']['image_path'], $this->_aVars['aItem']['server_id'], $this->_aVars['aItem']['is_old_suffix'], '_400'); ?>)">
            </a>
