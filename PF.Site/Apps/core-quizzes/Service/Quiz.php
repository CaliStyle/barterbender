...
class Quiz extends \Phpfox_Service
{
    ...
    public function getQuestionsByQuizId($iQuizzId)
    {
        if(empty($iQuizzId))
        {
            return false;
        }
        $aQuestions = $this->database()->select('qa.*')
            ->from(Phpfox::getT('quiz_question'), 'qa')
            ->where('qa.quiz_id = ' . (int)$iQuizzId)
            ->order('qa.question_id ASC')
    ...
    public function checkStructure($aQuestions)
    {
        ...
        foreach ($aQuestions as &$aQuestion) {
            ...
            elseif($iValidAnswers < $iMinAnswers || $iValidAnswers > $iMaxAnswers) {
                $aError[6] = _p('you_need_to_add_a_minimum_of_min_and_a_maximum_of_max_questions_per_quiz_you_submitted_total', ['min' => $iMinAnswers, 'max' => $iMaxAnswers, 'total' => $iValidAnswers]);
            }
    ...
    public function getQuizByUrl($iQuizId, $iUser = false, $bIsTakingQuiz = false, $iLimit = 5, $iPage = 1)
    {
        ...
        if ($iUser == false && $bIsTakingQuiz == false) {
            ...
            foreach ($aResults as $aResult) {
                // add the user info
                if(!isset($aTotalResults[$aResult['user_id']])) {
    ...
    {
        $sCacheId = $this->cache()->set('quiz_pending_sponsor');

        if(false === ($aIds = $this->cache()->get($sCacheId)))
        {
            $aItems = db()->select('quiz_id')
                ->from(Phpfox::getT('quiz'),'m')
                ->join(Phpfox::getT('better_ads_sponsor'),'s', 's.item_id = m.quiz_id')
                ->where('m.is_sponsor = 0 AND s.is_custom = 2 AND s.module_id = "quiz"')
                ->execute('getSlaveRows');

            $aIds = array_column($aItems,'quiz_id');
    ...
    public function getPermissions(&$aRow)
    {
        ...
        $aRow['iSponsorInFeedId'] = Phpfox::isModule('feed') && (Phpfox::getService('feed')->canSponsoredInFeed('quiz', $aRow['quiz_id']) === true);

        $aRow['canPurchaseSponsor'] = $aRow['canSponsorInFeed'] =  $aRow['canSponsor'] = false;
        if(Phpfox::isAppActive('Core_BetterAds'))
        {
            $aRow['canSponsorInFeed'] = (Phpfox::isModule('feed') && (($aRow['user_id'] == Phpfox::getUserId() && Phpfox::getUserParam('feed.can_purchase_sponsor')) || Phpfox::getUserParam('feed.can_sponsor_feed')) && Phpfox::getService('feed')->canSponsoredInFeed('quiz', $aRow['quiz_id']));
            $aRow['canSponsor'] = (Phpfox::getUserParam('quiz.can_sponsor_quiz'));

            $bCanPurchaseSponsor = $this->canPurchaseSponsorItem($aRow['quiz_id']);
            $aRow['canPurchaseSponsor'] = ($aRow['user_id'] == Phpfox::getUserId() && Phpfox::getUserParam('quiz.can_purchase_sponsor_quiz') && $bCanPurchaseSponsor);
        }
    ...
    public function getQuizToEdit($iQuiz)
    {
        ...
        if (!(($iCurrent == $aQuiz[0]['user_id'] && Phpfox::getUserParam('quiz.can_edit_own_questions')) ||
            Phpfox::getUserParam('quiz.can_edit_others_questions'))
        ) {
            return $aQuiz;
        }

        $parseOut = Phpfox::getLib('parse.output');
        // now get the answers
        $sQuestions = '';
        foreach ($aQuiz as $iKey => $aQuestion) {
            $sQuestions .= 'OR qa.question_id = ' . $aQuestion['question_id'] . ' ';
            $aQuiz[$iKey]['question'] = $parseOut->clean($aQuestion['question']);
        }
        ...
        // glue them
        foreach ($aAnswers as $aAnswer) {
            $aAnswer['answer'] = $parseOut->clean($aAnswer['answer']);
            foreach ($aQuiz as $iKey => $aQuestions) {
                if ($aQuestions['question_id'] == $aAnswer['question_id']) {
                    $aQuiz[$iKey]['answers'][] = $aAnswer;
                }
            }
        }
        $aFull = $aQuiz[0];
    ...
    public function getFeatured($iLimit = 4, $iCacheTime = 5)
    {
        ...
        if (($sQuizIds = $this->cache()->get($sCacheId, $iCacheTime)) === false) {
            $sQuizIds = '';
    ...
    public function getSponsored($iLimit = 4, $iCacheTime = 5)
    {
        ...
        if (($sQuizIds = $this->cache()->get($sCacheId, $iCacheTime)) === false) {
            ...
            $aQuizIds = $this->database()->select('q.quiz_id')
                ->from($this->_sTable, 'q')
                ->join(Phpfox::getT('user'), 'u', 'u.user_id = q.user_id')
                ->join(Phpfox::getT('better_ads_sponsor'), 's', 's.item_id = q.quiz_id')
                ->where('q.view_id = 0 AND q.is_sponsor = 1 AND s.module_id = \'quiz\' AND s.is_active = 1 AND s.is_custom = 3')
                ->order('rand()')
                ->limit(Phpfox::getParam('core.cache_total'))
                ->execute('getSlaveRows');
            foreach ($aQuizIds as $key => $aId) {
        ...
        $aQuizzes = $this->database()->select('q.*, q.total_view as total_view_quiz, ' . Phpfox::getUserField(). ', s.*')
            ->from($this->_sTable, 'q')
            ->join(Phpfox::getT('user'), 'u', 'u.user_id = q.user_id')
            ->join(Phpfox::getT('better_ads_sponsor'), 's', 's.item_id = q.quiz_id AND s.module_id = \'quiz\' AND s.is_active = 1 AND s.is_custom = 3')
            ->where('q.quiz_id IN (' . implode(',', $aQuizIds) . ')')
            ->group('q.quiz_id')
            ->limit($iLimit)
            ->execute('getSlaveRows');
    ...
    public function getUploadParams() {
        ...
        $aCallback = [
            ...
            'label' => _p('banner'),
            'is_required' => Phpfox::getUserParam('quiz.is_picture_upload_required'),
        ];
        return $aCallback;
    }
    /**
