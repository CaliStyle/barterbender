...
{if isset($bIsViewingQuiz)}
	...
	<div class="item-comment mb-2">
       ...
       <span class="item-total-view">
           <span><b>{$aQuiz.total_view|number_format}</b> {if $aQuiz.total_view == 1}{_p('view')}{else}{_p('views_lowercase')}{/if}</span>
           <span><b>{$aQuiz.total_play|number_format}</b> {if $aQuiz.total_play == 1}{_p('quiz_total_play')}{else}{_p('quiz_total_plays')}{/if}</span>
       </span>
       {/if}
    </div>
								...
								{foreach from=$aQuiz.question key=iQuestionId name=questions item=aQuestion}
									<div class="quiz_questions_inner {if isset($aAnswers) && !isset($aAnswers[$iQuestionId])}bg-danger{/if}">
										...
										<div class="quiz_answers">
											<div class="quiz_answers_title fw-bold mb-1">{$aQuestion.question|clean}</div>
												...
												<div class="quiz_answer">
													<div class="radio">
														<label>
															...
															<span title="{$sAnswer}">{$sAnswer|clean}</span>
														</label>
													</div>
												</div>
											{/foreach}
