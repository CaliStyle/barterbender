...
class Client
{
    ...
    public function apiPost($path, $data)
    {
        ...
        $url  = self::API_URL . curl_escape($curl, $path);
        curl_setopt_array($curl, array(
            CURLOPT_URL        => $url,
            CURLOPT_POST       => true,
    ...
    {
        $endPoint = isset($parentId) ? str_replace('{parent_id}', $parentId, '/me/drive/items/{parent_id}/children') : '/me/drive/root/children';
        ...
        $folder = $this->apiPost($endPoint, (object) $properties);
     ...
     * @param null $params
     */
    private function _createFile($filePath, $remotePath, $options = array())
    {
        $fileName = basename($filePath);
        //Create upload session
        $uploadSessionEndPoint = str_replace(['{path}'], [$remotePath . '/' . $fileName] , 'me/drive/root:/{path}:/createUploadSession');
        $result = (array)$this->apiPost($uploadSessionEndPoint, [
            "item" => [
        ...
        while ($i < $numFragments) {
            ...
            if(defined('CURL_SSL_VERIFYPEER_STATUS')) {
                curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, CURL_SSL_VERIFYPEER_STATUS);
            }
        ...
        return !empty($output['id']) && in_array($responseCode, [200, 201]) ? $output : false ;
    }
     ...
     */
    public function createFile($filePath, $remotePath, array $options = array())
    {
        return $this->_createFile($filePath, $remotePath, $options);
    }
    ...
    {
        $objectId = null !== $objectId ? $objectId : 'me/drive/root/children';
    ...
    {
        $endPoint = str_replace('{item-id}', $objectId, 'me/drive/items/{item-id}/children');

        $result  = (array)$this->apiGet($endPoint);

        if(empty($result['value']) || count($result['value']) == 0) {
            return false;
        }
