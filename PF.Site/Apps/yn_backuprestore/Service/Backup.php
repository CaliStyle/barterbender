...
class Backup extends Phpfox_Service
{
    ...
    public function getQuantity($aVal = array())
    {
        ...
        if (isset($aVal['from_date']) && $aVal['from_date']) {
            list($year, $month, $day) = explode('-', $aVal['from_date']);
            $iFromTimestamp = Phpfox::getLib('date')->mktime(0, 0, 0, $month, $day, $year);
            $where[] = "creation_timestamp >= $iFromTimestamp";
        }
        if (isset($aVal['to_date']) && $aVal['to_date']) {
            list($year, $month, $day) = explode('-', $aVal['to_date']);
            $iToTimestamp = Phpfox::getLib('date')->mktime(23, 59, 59, $month, $day, $year);
