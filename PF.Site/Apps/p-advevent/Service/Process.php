...
class Process extends Phpfox_Service
{
    ...
    public function add($aVals, $sModule = 'fevent', $iItem = 0)
    {
        ...
        $aSql = array(
            ...
            'range_value' => (!empty($aVals['range_value']) ? (int)$aVals['range_value'] : 0),
            'range_type' => (!empty($aVals['range_type']) ? (int)$aVals['range_type'] : 0),
        ...
        if (isset($aVals['location_lat']) && isset($aVals['location_lng'])) {
            $aSql['lat'] = $aVals['location_lat'];
            $aSql['lng'] = $aVals['location_lng'];
            $aSql['gmap'] = serialize([
                'latitude' => $aVals['location_lat'],
                'longitude' => $aVals['location_lng'],
            ]);
        }
    ...
    public function address2coordinates($sAddress)
    {
        ...
        $aResponse = json_decode(Phpfox::getLib('request')->send($apiaddress, [], 'GET', $_SERVER['HTTP_USER_AGENT']), true);
