...
group('/fevent', function () {
    ...
    route('/map', function(){
        if (Phpfox::getUserParam('fevent.can_create_event')) {
            sectionMenu(_p('add_new_event'), url('/fevent/add'));
        }
...
if (!$sDefaultEventPhoto) {
    $sDefaultEventPhoto = setting('core.path_actual') . 'PF.Site/Apps/p-advevent/assets/image/no_image.png';
}
Phpfox::getLib('setting')->setParam('fevent.fevent_default_photo', $sDefaultEventPhoto);
