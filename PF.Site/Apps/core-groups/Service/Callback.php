...
class Callback extends \Phpfox_Service
{
    ...
    public function getActivityFeed($aItem, $aCallback = null, $bIsChildItem = false)
    {
        ...
        $aRow['full_name'] = Phpfox::getService('ban.word')->clean($aRow['full_name']);
        ...
        $aReturn = [
            ...
            'feed_info' => _p('groups_user_created_group', [
                'title' => '<a href="' . $sLink . '" title="' . \Phpfox::getLib('parse.output')->clean($aRow['title']) . '">' . \Phpfox::getLib('parse.output')->clean(\Phpfox::getLib('parse.output')->shorten($aRow['title'],
                        50, '...')) . '</a>',
                'full_name' => '<span class="user_profile_link_span" id="js_user_name_link_' . $sFullName . '">' . (Phpfox::getService('user.block')->isBlocked(null, $aRow['user_id']) ? '' : '<a href="' . Phpfox::getLib('url')->makeUrl('profile', [$aRow['user_name']])) . '">' . $sFullName . '</a></span>'
            ]),
    ...
    public function getNotificationComment($aNotification)
    {
        ...
        $sPhrase = _p('user_commented_on_the_group_title', ['users' => $sUsers, 'title' => $sTitle]);
    ...
    public function deleteComment($iId)
    {
        $this->database()->update(Phpfox::getT('pages_feed_comment'),
            ['total_comment' => ['= total_comment -', 1]], 'feed_comment_id = ' . (int)$iId);
    }
