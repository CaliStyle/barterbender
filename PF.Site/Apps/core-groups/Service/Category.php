...
class Category extends Phpfox_Pages_Category
{
    ...
    )
    {
        ...
        foreach ($aTypes as $iKey => $aType) {
            ...
            if ($sView) {
                $aTypes[$iKey]['link'] = Phpfox::permalink(['groups.category', 'view' => $sView], $aType['type_id'],
                    $aType['name']);
            } else {
    ...
    public function getLatestPages($iId, $userId = null, $iPagesLimit = 8, $sView = '')
    {
        ...
        if (!empty($userId)) {
            ...
            if ($sView == 'friend' || Phpfox::getParam('core.friends_only_community')) {
                $sFriendsList = '0';
                if (Phpfox::getUserId()) {
        ...
        if (($userId != Phpfox::getUserId() || $userId === null) && Phpfox::hasCallback('groups', 'getExtraBrowseConditions')) {
