...
class ViewController extends Phpfox_Component
{
    public function process()
    {
        ...
        //Check group privacy
        if ($aPage['reg_method'] == 2 && !Phpfox::getService('groups')->isMember($aPage['page_id']) && !Phpfox::isAdmin() && !Phpfox::getService('groups')->checkCurrentUserInvited($aPage['page_id'], $decodedEmail)) {
        ...
        if ($bCanViewPage && $sCurrentModule == 'members') {
            ...
            if (isset($aPage['text']) && !empty($aPage['text'])) {
                $this->template()->setMeta('description', $aPage['title'] . '. ' . $total_members . '. ' . $aPage['text']);
            } else {
