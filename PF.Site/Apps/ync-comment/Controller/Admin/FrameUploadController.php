...
class FrameUploadController extends Phpfox_Component
{
    ...
    public function process()
    {
        ...
        $aParams = Phpfox::getService('ynccomment')->getUploadParams(['id' => $iId]);
        $aParams['type'] = 'ynccomment';
        ...
        $iImageId = db()->insert(Phpfox::getT('ynccomment_stickers'), array(
            'set_id' => $iId,
            'image_path' => $aFile['name'],
            'server_id' => Phpfox_Request::instance()->getServer('PHPFOX_SERVER_ID')
        ));
        if ($iImageId) {
            db()->updateCounter('ynccomment_sticker_set', 'total_sticker', 'set_id', $aStickerSet['set_id']);
            if (empty($aStickerSet['thumbnail_id'])) {
                db()->update(':ynccomment_sticker_set', ['thumbnail_id' => $iImageId],
