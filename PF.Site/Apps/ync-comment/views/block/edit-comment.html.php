...
?>
<div class="ync-comment-box-edit js_ync_edit_comment_holder js_ync_comment_quick_edit_holder_{$aComment.comment_id} {if !empty($sType) && $sType != 'preview'}has-photo-sticker{/if}">
    <form method="post" action="#" class="js_adv_comment_feed_form form" data-edit-id="{$aComment.comment_id}">
        <div class="item-edit-content">
            <div class="item-box-input">
                ...
                <input type="hidden" name="val[comment_id]" value="{$aComment.comment_id}" class="js_feed_comment_comment_id" />
                ...
                <textarea rows="1" name="val[text]" class="form-control ync-comment-textarea-edit js_ync_comment_textarea_edit" id="js_ync_comment_quick_edit_{$aComment.comment_id}" placeholder="{if $aComment.parent_id}{_p var='write_a_reply'}{else}{_p var='write_a_comment'}{/if}">{$aComment.text}</textarea>
                ...
                <div class="ync-comment-group-icon dropup js_comment_group_icon">
                    <div title="" class="item-icon icon-photo js_ync_comment_attach_photo js_hover_title" data-feed-id="0" data-edit-id="{$aComment.comment_id}">
                    ...
                    <div title="" class="item-icon icon-sticker js_ync_comment_attach_sticker js_comment_icon_sticker_{$aComment.comment_id} js_hover_title" data-sticker_next="0" data-feed-id="0" data-edit-id="{$aComment.comment_id}"><i class="ico "><svg class="sticker-o" version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px"
                    ...
                    {/if}
                    <div title="" class="item-icon icon-emoji js_ync_comment_attach_emoticon js_hover_title" data-feed-id="0" data-edit-id="{$aComment.comment_id}"><i class="ico ico-smile-o"></i><span class="js_hover_info">{_p var='insert_an_emoji'}</span></div>
                </div>
            </div>
            {if !empty($aForms)}
                {template file='ynccomment.block.preview-attach'}
            {/if}
        </div>
    </form>
    <div class="ync-comment-group-btn-icon">
        <div class="ync-comment-btn">
            <button class="btn btn-default btn-xs" onclick="return ynccomment.unsetAllEditComment({$aComment.comment_id});">{_p var='cancel'}</button>
            <button class="btn btn-primary btn-xs" onclick="$(this).closest('.js_ync_edit_comment_holder').find('form:first').submit(); return false;">{_p var='submit'}</button>
        </div>
        <div class="item-edit-cancel">
            <span class="js_ync_focus_edit_comment">
                {_p var='press_esc_to'} <a href="#" onclick="return ynccomment.unsetAllEditComment({$aComment.comment_id});" class="item-cancel">{_p var='cancel__l'}</a> {_p var='edit__l'}
            </span>
            <span class="js_ync_not_focus_edit_comment hide">
                <a href="#" onclick="return ynccomment.unsetAllEditComment({$aComment.comment_id});" class="item-cancel">{_p var='cancel__l'}</a>
            </span>
        </div>
    </div>
</div>
<script type="text/javascript">
    $Behavior.onLoadEditCommentForm = function() {l}
        ynccomment.attachFunctionTagger('.js_ync_comment_quick_edit_holder_{$aComment.comment_id} .js_ync_comment_textarea_edit');
        ynccomment.initFocusTextarea('.js_ync_comment_textarea_edit');
    {r}
</script>