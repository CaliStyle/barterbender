...
{if (isset($showOnlyComments))}
    {if Phpfox::isModule('comment') && (isset($aFeed.comments) && count($aFeed.comments))}
        {if $iShownTotal < $aFeed.total_comment}
            <div class="ync_comment_viemore_holder">
                <div class="ync-comment-viewmore" id="js_feed_comment_pager_{$aFeed.type_id}{$aFeed.item_id}">
                    <a href="{url link='ynccomment.comments'}?type={$aFeed.type_id}&id={$aFeed.item_id}&page=1{if defined('PHPFOX_FEED_STREAM_MODE')}&stream-mode=1{/if}&time-stamp={$aFeed.comments.0.time_stamp}&shown-total={$iShownTotal}&total-comment={$aFeed.total_comment}" class="item-viewmore ajax" onclick="$(this).addClass('active');">
        ...
        {foreach from=$aFeed.comments name=comments item=aComment}
            {template file='ynccomment.block.mini'}
        {/foreach}
    {/if}
{else}
    ...
    {/if}
        <div class="js_feed_comment_border ync-comment-content" {if isset($sFeedType) &&  $sFeedType == 'view'}id="js_item_feed_{$aFeed.feed_id}"{/if}>
                ...
                <div class="comment_mini_content_border">
                    {if !isset($aFeed.feed_mini)}
                        <div class="feed-options-holder item-options-holder hide" data-component="feed-options">
                            ...
                            {template file='ynccomment.block.link'}
                        </div>
                    {/if}
                    ...
                    <div class="comment-mini-content-commands">
                        ...
                        <div class="js_comment_like_holder" id="js_feed_like_holder_{$aFeed.feed_id}">
                            <div id="js_like_body_{$aFeed.feed_id}">
                                {template file='ynccomment.block.like-display'}
                            </div>
                        </div>
                        {/if}
                    ...
                    <div class="ync-comment-wrapper"> <!--ync-comment-wrapper-->
                                ...
                                <div id="js_feed_comment_post_{$aFeed.feed_id}" class="js_feed_comment_view_more_holder">
                                    {if isset($aFeed.comments) && $iShownTotal = count($aFeed.comments)}
                                            {if Phpfox::getParam('comment.comment_page_limit') != null && Phpfox::isModule('comment') && $aFeed.total_comment > Phpfox::getParam('comment.comment_page_limit')}
                                                <div class="ync-comment-viewmore" id="js_feed_comment_pager_{$aFeed.type_id}{$aFeed.item_id}">
                                                    <a href="{url link='ynccomment.comments'}?type={$aFeed.type_id}&id={$aFeed.item_id}&page=1{if defined('PHPFOX_FEED_STREAM_MODE')}&stream-mode=1{/if}&time-stamp={$aFeed.comments.0.time_stamp}&shown-total={$iShownTotal}&total-comment={$aFeed.total_comment}" class="ajax item-viewmore"  onclick="$(this).addClass('active');">
                                            ...
                                            {/if}
                                            <div class="ync-comment-container">
                                                <div id="js_feed_comment_view_more_{$aFeed.feed_id}"{if isset($sFeedType) &&  $sFeedType == 'view'}class="js_ync_comment_comment_items"{else} class="js_ync_comment_limit js_ync_comment_comment_items" data-limit="{if ($thisLimit = setting('ynccomment.ynccomment_comment_show_on_activity_feeds'))}{$thisLimit}{/if}"{/if}>
                                                    {foreach from=$aFeed.comments name=comments item=aComment}
                                                        {template file='ynccomment.block.mini'}
                                                    {/foreach}
                            ...
                            {else}
                                {if isset($aFeed.comments.0) && ($aFeed.comments.0.view_id == '0' || (Phpfox::isUser() && (Phpfox::getUserParam('comment.can_moderate_comments') || $aFeed.comments.0.user_id == Phpfox::getUserId())))}
                                    <div class="ync-comment-container">
                                        <div id="js_feed_comment_view_more_{$aFeed.feed_id}"{if isset($sFeedType) &&  $sFeedType == 'view'}class="js_ync_comment_comment_items"{else} class="js_ync_comment_limit js_ync_comment_comment_items" data-limit="{if ($thisLimit = setting('ynccomment.ynccomment_comment_show_on_activity_feeds'))}{$thisLimit}{/if}"{/if}>
                                            {foreach from=$aFeed.comments name=comments item=aComment}
                                                {template file='ynccomment.block.mini-single'}
                                            {/foreach}
                                        </div><!-- // #js_feed_comment_view_more_{$aFeed.feed_id} -->
                                    </div>
                                {else}
                                    <div class="ync-comment-container">
                                        <div class="ync-comment-item">
                        ...
                        {else}
                            {if Phpfox::isModule('comment')
                                ...
                                {/if}
                                <div class="ync-comment-footer js_feed_comment_form_holder">
                                    <div class="ync-comment-box-container">
                                        <div class="js_feed_ync_comment_form" {if isset($sFeedType) &&  $sFeedType == 'view'} id="js_feed_comment_form_{$aFeed.feed_id}"{/if}>
                                            <div class="js_comment_feed_textarea_browse"></div>
                                            <div class="{if isset($sFeedType) &&  $sFeedType == 'view'} feed_item_view{/if}">
                                                <form method="post" action="#" class="js_adv_comment_feed_form form">
                                                    ...
                                                    <div class="">
                                                        ...
                                                        <div class="item-outer">
                                                            ...
                                                            <div class="item-inner">
                                                                <div class="ync-comment-box js_ync_comment_box">
                                                                    <div class="item-edit-content">
                                                                        <div class="item-box-input">
                                                                            <textarea rows="1" name="val[text]" class="form-control ync-comment-textarea-edit js_ync_comment_feed_textarea" id="js_feed_comment_form_textarea_{$aFeed.feed_id}" placeholder="{_p var='write_a_comment'}" autocomplete="off"></textarea>
                                                                            ...
                                                                            <div class="ync-comment-group-icon dropup js_comment_group_icon">
                                                                                <div title="" class="item-icon icon-photo js_ync_comment_attach_photo js_hover_title" data-feed-id="{$aFeed.feed_id}">
                                                                                ...
                                                                                {if !empty(Phpfox::getService('ynccomment.stickers')->countActiveStickerSet())}
                                                                                    <div title="" class="item-icon icon-sticker js_ync_comment_attach_sticker js_comment_icon_sticker_{$aFeed.feed_id} js_hover_title" data-sticker_next="0" data-feed-id="{$aFeed.feed_id}">
                                                                                        <i class="ico ">
                                                                                ...
                                                                                {/if}
                                                                                <div title="" class="item-icon icon-emoji js_ync_comment_attach_emoticon js_hover_title" data-feed-id="{$aFeed.feed_id}"><i class="ico ico-smile-o"></i><span class="js_hover_info">{_p var='insert_an_emoji'}</span></div>
                                                                            </div>
                                                                        </div>
                                                                    </div>
                                                                </div>
                                    ...
                                    <div class="ync-comment-group-btn-icon-empty" style="display: none">
                                    </div>
                                </div>
                            {else}
