...
<div class="js_view_more_reply hide">
    {if $iLoadMoreTotal > 0}
        <div class="ync-comment-viewless">
            <a data-hide-total="{$iLoadMoreTotal}" href="#" onclick="ynccomment.hideLoadedReplies(this, {$aComment.comment_id}); return false;" class="item-viewless">
                {if $iLoadMoreTotal == 1}
    ...
    <div class="ync-comment-viewmore js_ync_replies_viewmore_{$aComment.comment_id}">
        <?php
            $this->_aVars['aLastReply'] = end($this->_aVars['aReplies']);
        ?>
        <a href="{url link='ynccomment.replies'}?is_feed={$isFeed}&comment_type_id={$aComment.type_id}&item_id={$aComment.item_id}&comment_id={$aComment.comment_id}&time-stamp={$aLastReply.time_stamp}&max-time={$iMaxTime}&shown-total={$iShownTotal}&total-replies={$aComment.child_total}" class="item-viewmore ajax" onclick="$(this).addClass('active');">
