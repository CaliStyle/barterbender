...
var ynccomment_admin = {
    ...
    dropzoneQueueComplete: function () {
        ...
        if (!iTotalError) {
            window.location.href = getParam('sBaseURL') + 'admincp/ynccomment/add-sticker-set/' + '?id=' + set_id;
        } else {
            $('#js_sticker_set_submit').removeClass('disabled').removeAttr('disabled');

            setTimeout(function(){
                $('#ynccomment-dropzone').find('.dz-preview:not(.dz-error)').remove();
                ynccomment_admin.refreshStickers(set_id);
            },500);
        }
    },
        ...
        if (typeof response.errors != 'undefined') {
            for (var i in response.errors) {
                if (response.errors[i]) {
                    $Core.dropzone.setFileError('ynccomment', file, response.errors[i]);
                    return;
                }
            }
            $('#ynccomment-dropzone').removeClass('dz-started');
        }
        return file.previewElement.classList.add('dz-success');
    },
    resetFileToQueue: function() {
        var _i, _len,
            _files = $Core.dropzone.instance['ynccomment'].files;
        if (_files.length) {
    ...
    refreshStickers: function(id) {
        $.ajaxCall('ynccomment.refreshStickers','id='+id);
        return true;
    }
};
...
$Ready(function(){
    setTimeout(function(){
        ynccomment_admin.initCanvasGif('.ync_comment_gif:not(.ync_built)');
    },1000);
    if($('#js_sticker_set_form').length)
    {
        $('#js_sticker_set_form').off('submit').on('submit',function(){
            ...
            $Core.ajax('ynccomment.addStickerSet',{
                type: 'POST',
                params: {
                ...
                success: function(sOutput) {
                    ...
                    if (typeof response.id != 'undefined') {
                        if (_files.length) {
                            ...
                            $Core.dropzone.instance['ynccomment'].processQueue();
                        } else {
                            $('#js_sticker_set_submit').removeClass('disabled').removeAttr('disabled');
                            if ($('#js_sticker_set_id').val() > 0) {
                                ...
                                window.location.href = getParam('sBaseURL') + 'admincp/ynccomment/add-sticker-set/' + '?id=' + response.id;
                            }
                        }
                    }
                }
    ...
    if (typeof $Core.dropzone.instance['ynccomment'] != 'undefined') {
        $Core.dropzone.instance['ynccomment'].on('processingqueue', function(files){
            if (!files.length) {
                var set_id = $('#js_sticker_set_id').val();
                window.location.href = getParam('sBaseURL') + 'admincp/ynccomment/add-sticker-set/' + '?id=' + set_id;
            }
